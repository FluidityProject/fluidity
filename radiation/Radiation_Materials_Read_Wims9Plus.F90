...
module radiation_materials_read_wims9plus
   ...
   public :: read_wims9plus

contains

   ! --------------------------------------------------------------------------

   subroutine read_wims9plus(wims9plus_filename, &
                             dataset_radmat, &
      ...
      !!< Read a wims9plus formatted material data set file, which also may include the data objects DELAYED, POWER and VELOCITY
      !!< Due to the formatting of the wims9plus file associated with the scatter data this will only work with
      ...
      character(len=*), intent(in) :: wims9plus_filename      
      ...
      logical :: wims9plus_file_exists,exit_if_eof
      integer, dimension(:), allocatable :: keyword_find ! the specific keyword integer ids to find
      character(len=record_len) :: line_string
      ! the keyword list to use to search through the wims9plus input file, the 21 is due to the scatter moment data
      character(len=21), dimension(:), allocatable :: keyword_list

      ewrite(1,*) 'Read in wims9plus format data with max keyword record length ',record_len
      ...
      ! initiliase the base module via creating the wims9plus keyword list
      call keyword_list_initialise_wims9plus(number_of_scatter_moments, &
                                             keyword_list)
            
      ! get a free io unit for wims9plus file
      wims9plus_file_unit = free_unit()
      
      inquire(file=trim(wims9plus_filename),exist=wims9plus_file_exists)
      
      no_wims9plus_file: if (.not. wims9plus_file_exists) then
         
         ewrite(-1,*) "Input error: wims9plus file ",trim(wims9plus_filename)
         FLExit("was not found")
         
      end if no_wims9plus_file
      
      ! open a sequential access file
      open(unit   = wims9plus_file_unit, &
           file   = trim(wims9plus_filename), &
           status = 'old', &
           form   = 'formatted', &
           action = 'read', &
           access = 'sequential', &
      ...
      check_open_fine: if (iostat /= 0) then
         ...
         ewrite(-1,*) "Error opening wims9plus file ",trim(wims9plus_filename)
         FLAbort("IOSTAT returned non zero")
      
      end if check_open_fine
      
      ! calculate the number of radmats, energy groups, number of scatter moments, number of delayed groups, number of wims9plus non thermal groups 
      call deduce_number_of_items_wims9plus(wims9plus_file_unit, &
      ...
      check_material_consistency: if (total_number_of_radmats /= total_number_of_radmats_wims9plus) then
      
         ewrite(-1,*) "Input error: wims9plus file ",trim(wims9plus_filename)," has ",total_number_of_radmats_wims9plus,&
      ...
      check_energy_group_consistency: if (number_of_energy_groups /= number_of_energy_groups_wims9plus) then
      
         ewrite(-1,*) "Input error: wims9plus file ",trim(wims9plus_filename)," has ",number_of_energy_groups_wims9plus,&
      ...
      check_scatter_moment_consistency: if (number_of_scatter_moments /= number_of_scatter_moments_wims9plus) then
      
         ewrite(-1,*) "Input error: wims9plus file ",trim(wims9plus_filename)," has ",number_of_scatter_moments_wims9plus,&
      ...
      check_delayed_group_consistency: if (number_of_delayed_groups /= number_of_delayed_groups_wims9plus) then
      
         ewrite(-1,*) "Input error: wims9plus file ",trim(wims9plus_filename)," has ",number_of_delayed_groups_wims9plus,&
      ...
      physical_material_loop: do pmat = 1,size(dataset_radmat%physical_radmats)
         ...
         radmat_loop: do rmat = 1,size(dataset_radmat%physical_radmats(pmat)%radmats)
            ...
            call calculate_removal_moments_wims9plus(wims9plus_file_unit, &
                                                     line_number, &
                                                     dataset_radmat%physical_radmats(pmat)%radmats(rmat), &
                                                     record_len, &
                                                     keyword_list)
            ...
            geom_dim_loop: do idim = 1,problem_dimension 
               ...
               call read_or_associate_transport_and_diffusion_wims9plus(wims9plus_file_unit, &
                                                                        line_number, &
                                                                        dataset_radmat%physical_radmats(pmat)%radmats(rmat), &
                                                                        idim, &
                                                                        record_len, &
            ...
            call read_fission_data_wims9plus(wims9plus_file_unit, &
                                             line_number, &
                                             dataset_radmat%physical_radmats(pmat)%radmats(rmat), &
                                             number_non_thermal_groups_wims9plus, &
                                             record_len, &
      ...
      need_power: if (read_power_data) then
         ...
         call read_power_data_wims9plus(wims9plus_file_unit, &
                                        dataset_radmat, &
                                        record_len, &
                                        keyword_list)
      ...
      need_delayed_lambda_spectrum: if (read_delayed_lambda_spectrum) then
         ...
         call read_delayed_data_wims9plus(wims9plus_file_unit, &
                                          dataset_radmat, &
                                          record_len, &
                                          keyword_list, &
                                          delayed_lambda_spectrum, &
      ...
      close(wims9plus_file_unit)
            
   end subroutine read_wims9plus

   ! --------------------------------------------------------------------------

end module radiation_materials_read_wims9plus
