...
contains
   ...
   subroutine particle_radmat_read(particle_radmat)
      ...
      character(len=OPTION_PATH_LEN) :: flux_norm_type
      character(len=OPTION_PATH_LEN) :: dataset_name
      character(len=OPTION_PATH_LEN) :: dataset_name_to_read_delayed_lambda
      ...
      call get_option(trim(particle_radmat%option_path)//'/equation/name',equation_type)  
      if (trim(equation_type) == 'TimeDependent') read_velocity_data = .true.
         
      ! see if there is a need for the power data for this object 
      eig_or_time: if (trim(equation_type) == 'Eigenvalue') then
      
         call get_option(trim(particle_radmat%option_path)//'/equation/flux_normalisation/name',flux_norm_type)
         
         check_need_power_eig: if (trim(flux_norm_type) == 'TotalPower') then
            
            read_power_data = .true.
         
         else check_need_power_eig
            
            read_power_data = .false.
         
         end if check_need_power_eig         
      ...
      dataset_name_to_read_delayed_lambda = ''
      have_delayed: if (have_option(trim(particle_radmat%option_path)//'/delayed_neutron_precursor')) then
      
         call get_option(trim(particle_radmat%option_path)//'/delayed_neutron_precursor/read_delayed_lambda_spectrum_from_data_set/name',&
                         &dataset_name_to_read_delayed_lambda)
      ...
      data_set_loop: do dmat = 1,size(particle_radmat%dataset_radmats)
         ...
         call get_option(trim(particle_radmat%dataset_radmats(dmat)%option_path)//'/name',dataset_name)
         
         ewrite(1,*) 'Read in dataset ',trim(dataset_name)
                           
         if (trim(dataset_name) == trim(dataset_name_to_read_delayed_lambda)) read_delayed_lambda_spectrum = .true.
   ...
   subroutine read_in_dataset_radmat(dataset_radmat, &
      ...
      read_data_set_if: if (have_option(trim(dataset_radmat%option_path)//'/from_file/format_radmats')) then

         ! set the radmats file record length used for reading from the options
         call get_option(trim(dataset_radmat%option_path)//'/from_file/format_radmats/maximum_record_length',record_len) 
                  
         call read_format_radmats(trim(dataset_radmat%file_name), &
                                  dataset_radmat, &
                                  delayed_lambda_spectrum, &
                                  read_delayed_lambda_spectrum, &
                                  read_velocity_data, &
