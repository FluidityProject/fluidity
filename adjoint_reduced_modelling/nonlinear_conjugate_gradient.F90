...
module nonlinear_conjugate_gradient
use state_module
...
!  REAL,PARAMETER ::LINERR0=0.5e-17
  ...
  IF (LINITS.EQ.1) THEN
     ...
     ELSE	
        ...
        ENDIF
        print *,'SQG,SQGOLD,SQG1',SQG,SQGOLD,SQG1
        print *,'BATA',BATA
...
!cylinder     	  INISTEP = 0.1*MAXCVA/MAXD
     !          INISTEP = 0.05*MAXCD          
!**clyde     INISTEP = -0.01
     ...
     SUBROUTINE LINESEARCH(LINITS,VALFUN,NUVAL,  &
	  ...
	  LINITS = LINITS+1
       ...
       IF (LINITS.EQ.4) THEN
          ...
          PRINT *,'Finish the choice of the initial three points'
          PRINT *,'NU1,NU2,NU3',NU(1), NU(2),NU(3)
          PRINT *,'F1,F2,F3',F(1),F(2),F(3)
          GOTO 10
       ENDIF
       
       
       ! Has the last line search converged?
       Print *,'VALFUN0=',VALFUN0		
...
10     CONTINUE
	...
	IF(LINITS.GE.4) THEN
	   ...
	   CALL SORTNU(3,NU,F)
	    ...
	    CALL MINQUA(NUVAL,3,NU,F,MINNU,MAXNU,GOLDR2)
            !	   ENDIF
         ENDIF
		print *, '--------------------------'
		print *, 'NU', NU
...
1000            CONTINUE
              ...
              SUBROUTINE MINQUA(NUVAL,NSOR,NU,F,MINNU,MAXNU,GOLDR)
                !---------------------------------------------------------------------------------------
                !
                ...
                !
                PRINT *,'FINISHED SMLIN3 AA,BB,CC:',AA,BB,CC
                PRINT *,'checking curve:'
                print *,'from interpolated curve f1,f2,f3:',  &
                ...
                ELSE
                   NUVAL=-BB/(2.*CC)
                   print *,'before corrections BB,CC,NUVAL=',BB,CC,NUVAL
                ...
                ENDIF
                print *,'finishing minqua NUVAL=',NUVAL
                !     
                RETURN
              END SUBROUTINE MINQUA
              !
              !
              !
              SUBROUTINE SORTNU(NSOR,NU,F)
                
                
                !---------------------------------------------------------------------------------------
                !
                ! Sort vectors NU(1)-NU(NSOR) and F(1)-F(NSOR) in order of increasing NU.
                !
                ! Precondition:
                ! NU and F are Vectors of length NSOR
                ! NU(1)-NU(NSOR) are the abcissae and F(1)-F(NSOR) are the function values 
                ! at the corresponding NU.
                !
                ! Postcondition:
                ! NU(i) and F(i) are sorted in increasing values for NU
                !
                !---------------------------------------------------------------------------------------
                ...
                END DO
                RETURN
      END SUBROUTINE SORTNU
      !....
      
   end  module nonlinear_conjugate_gradient  
