subroutine test_cubic_interpolation
  ...
  do node=1,node_count(old_mesh)
    ...
    old_fields(1)%val(node) = x**3
    old_fields(2)%val(node) = x**2
    old_fields(3)%val(node) = x
  end do

  call cubic_interpolation(old_fields, old_position, new_fields, new_position)

  call vtk_write_fields("data/cubic_interpolation", 0, old_position, old_mesh, sfields=old_fields, vfields=(/old_position/))
  call vtk_write_fields("data/cubic_interpolation", 1, new_position, new_mesh, sfields=new_fields, vfields=(/new_position/))

  fail = .false.
  do node=1,node_count(new_mesh)
    pos = node_val(new_position, node)
    x = pos(1) ; y = pos(2) ; z = pos(3)
    if (.not. fequals(node_val(new_fields(1), node), x**3, 0.05)) then
      write(0,*) "node == ", node
      write(0,*) "node_val(new_fields(1), node) == ", node_val(new_fields(1), node)
      fail = .true.
    end if
    if (.not. fequals(node_val(new_fields(2), node), x**2, 0.05)) then
      write(0,*) "node == ", node
      write(0,*) "node_val(new_fields(2), node) == ", node_val(new_fields(2), node)
      fail = .true.
    end if
    if (.not. fequals(node_val(new_fields(3), node), x, 0.05)) then
      write(0,*) "node == ", node
      write(0,*) "node_val(new_fields(3), node) == ", node_val(new_fields(3), node)
      fail = .true.
    end if
  end do
  call report_test("[cubic interpolation]", fail, .false., "All nodal values should be exact.")
