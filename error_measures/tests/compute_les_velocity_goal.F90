...
!  use metric_assemble
  ...
  call vtk_write_fields("goal_error_adapted", 0, positions, mesh, sfields=(/edgelen/), &
  ...
  do i=1,10
    ...
    call form_gradation_metric(positions, metric) 
    call get_edge_lengths(metric, edgelen)
    call vtk_write_fields("goal_error_adapted", i+1, positions, mesh, sfields=(/les, edgelen/), &
                          vfields=(/velocity/), tfields=(/metric/))
    call adapt_state(state, metric) 
  end do
