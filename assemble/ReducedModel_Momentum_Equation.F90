...
!      use pressure_dirichlet_bcs_cv
     ...
     ! use Bp_Neutral_Net 
      ...
      private
      public :: solve_momentum_reduced,solve_ann_reduced, solve_smolyak, solve_rbf_interpolation, solve_rbf, solve_lag_interpolation, deim_state, deim_state_res,deim_state_resl,deim_number,pod_coef_all_o
     ...
     subroutine solve_momentum_reduced(state, u,p,big_m, ct_m, mom_rhs, ct_rhs, inverse_masslump, &
       ...
       reduced_model_loop: do istate = 1, size(state)
          ...
          if(have_option(trim(u%option_path)//"/prognostic/spatial_discretisation"//&
           ...
           ! if (have_option("/reduced_model/Non_intrusive")) goto 211
...
!                            stop 24
      ...
      !                pod_matrix_adv%val(1:u%dim*size(POD_state,1), u%dim*size(POD_state,1)+1:(u%dim+1)*size(POD_state,1)) = 0.0
       ...
       !               pod_matrix_adv%val(1:u%dim*size(POD_state,1), u%dim*size(POD_state,1)+1:(u%dim+1)*size(POD_state,1)) = 0.0
           ...
           if(.false.) then  ! produce the value of functions
           if (have_option("/reduced_model/Smolyak")) then
               ...
               call get_option(&
               '/reduced_model/pod_basis_formation/pod_basis_count', nsvd)      
               print *, 'timestepsmolyak', timestep
              ...
              if(timestep .eq. 1) then 
                   open(11,file='coef_pod_all_fvalue')
                   pod_coef_all_obv(1,:)=0
                   write(11,*)(pod_coef_all_obv(timestep,:))
                   close(11)
                  else 
                   open(11,file='coef_pod_all_fvalue', position='append',ACTION='WRITE')
                   write(11,*)(pod_coef_all_obv(timestep,:))
                   close(11)
                  endif
             ...
             !add some to pod_coef()
             m=(u%dim+1)*nsvd
                       
             open(40,file='total', ACTION='read') 
             read(40,*) total_nd
             close(40)
             allocate ( xd(1:m,1:total_nd) )
             ! print *,'total',total_nd,m

             open(41,file='xd', ACTION='READ') 
             !print *,'dddxd'
             read(41,*)((xd(i,j),i=1,m),j=1,total_nd) 
             close(41)
             if(timestep>total_nd) stop
             pod_coef_all_obv(timestep,:)=xd(:,timestep)
             ! produce tensor_product grid
             ! print *, 'sssssssssssssssssssssssss'
             call project_full(delta_u, delta_p, pod_sol_velocity, pod_sol_pressure, POD_state(:,:,istate), pod_coef_all_obv(timestep,:))
            ...
            ! snapmean_pressure=>extract_Scalar_field(POD_state(1,2,1),"SnapmeanPressure")
             ...
             deallocate(xd)
                  ...
                  if (have_option("/reduced_model/Non_intrusive")) then
                  ...
                  endif  !if (have_option("/reduced_model/Non_intrusive")) then   
                   ...
                   write(101,*)(pod_coef(i),i=1,(u%dim+1)*size(POD_state,1))
                   close(101)
                   print * ,'pod_coef(i)reducd', pod_coef((u%dim)*size(POD_state,1))
              ...
              !      !pod_coef(:)=pod_coef_all_temp(timestep,:) ! test the rom using full model's results
               ...
               ! if (.not.(have_option("/reduced_model/Smolyak"))) then
                snapmean_velocity=>extract_vector_field(POD_state(1,1,istate),"SnapmeanVelocity")
                snapmean_pressure=>extract_Scalar_field(POD_state(1,2,istate),"SnapmeanPressure")
                !call addto(u, delta_u, dt)
                u%val=snapmean_velocity%val
  ...
  subroutine solve_ann_reduced(state, u,p,big_m, ct_m, mom_rhs, ct_rhs, inverse_masslump, &
       ...
       p => extract_scalar_field(state(1), "Pressure")
        ...
        ! Allocate the change in pressure field
            ...
            IF(.NOT.ALLOCATED(pod_coef_all_temp)) then        
                ...
                pod_coef(:)=pod_coef_all_temp(timestep,:) ! initialize the pod_coef
     reduced_model_loop: do istate = 1, size(state)
              ...
              if (have_option("/reduced_model/Non_intrusive")) then
                   ...
                   if(timestep>ctrl_timestep) then !
                        ...
                        do i=1,  (u%dim+1)*size(POD_state,1) 
                          ...
                          pod_coef(i)=0           
                        !  call  ann_bp_main(total_timestep,timestep,pod_coef(i),i) !recalculate the pod_coef(i) using BP_ANN one by one
                         ! open when do bp_ann nonintrusive
                        enddo 
                    endif                   
                   !save pod_coef, rewrite every timestep 
                  endif ! non-intrusive
                    ...
                    ! stop 1608
                   call get_option("/reduced_model/nonintrusive_pod_basis_count", ann_pod_num)
                  ! do i=1, (u%dim+1) 
                 !  do j=ann_pod_num+1, nsvd
                     !   pod_coef((i-1)*nsvd+j)=0    ! test part of the pod_numbers
                 !  enddo
                  ! enddo
                  ...
                  if(timestep .eq. total_timestep)then 
                         ...
                         !if (have_option("/reduced_model/RBF_interpolation")) then 
                            do i=1,  (u%dim+1)*size(POD_state,1) 
                              ! call rbf_interp_main ((u%dim+1)*size(POD_state,1),i,total_timestep )  !do rbf interpolation after the last timestep of ANN_ROM
                            enddo
                        ! end if !
                       ...
                       !  if (have_option("/reduced_model/Lag_nd_interpolation")) then
                        ...
                        ! endif 
                   deallocate(pod_coef_all_temp)
                  endif! if(timestep .eq. total_timestep)
                   deallocate(pod_coef)
          enddo reduced_model_loop 
        end subroutine solve_ann_reduced
    
       subroutine solve_lag_interpolation(state, u,p,big_m, ct_m, mom_rhs, ct_rhs, inverse_masslump, &
          at_first_timestep, timestep, POD_state, POD_state_deim, snapmean, eps, its,total_timestep)
      ...
      ! logical, optional, intent(in) :: if_optimal
       ...
       type(scalar_field), pointer :: POD_pressure
       real, dimension(:,:), allocatable :: pod_coef_all_lag_inv
       type(pod_matrix_type) :: pod_matrix, pod_matrix_mass, pod_matrix_adv 
       type(pod_rhs_type) :: pod_rhs
       real, dimension(:), allocatable :: pod_coef,pod_coef_dt,pod_coef0
       real, dimension(:,:), allocatable :: pod_sol_velocity, pod_ct_m
       ...
       p => extract_scalar_field(state(1), "Pressure")
        ...
        ! Allocate the change in pressure field
         ...
         allocate(pod_coef((u%dim+1)*size(POD_state,1)))
            ...
            IF(.NOT.ALLOCATED(pod_coef_all_lag_inv)) then        
                    
                    allocate(pod_coef_all_lag_inv((u%dim+1)*size(POD_state,1),total_timestep)) 
                    open(11,file='coef_pod_all_lag_tmp') 
                    !read interpolated coef_pod_all from coef_pod_all_tmp which formed in the last timestep of ann
                    read(11,*)((pod_coef_all_lag_inv(i,j),j=1,total_timestep),i=1,(u%dim+1)*size(POD_state,1))                    
                    close(11)  
              endif
               ! print * ,'aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa'
                pod_coef(:)=pod_coef_all_lag_inv(:,timestep) !give the interpolated pod_coef
             ...
             reduced_model_loop: do istate = 1, size(state)
                   ...
                   ! save pod_coef for all the time levels
                    if(timestep==1) then
                      open(40,file='coef_pod_all_lag')
                      write(40,*)(pod_coef(i),i=1,(u%dim+1)*size(POD_state,1))
                      close(40)
                   else
                    open(101,file='coef_pod_all_lag', position='append',ACTION='WRITE')
                    write(101,*)(pod_coef(i),i=1,(u%dim+1)*size(POD_state,1))
                    close(101)
                    print * ,'pod_coef(i)', pod_coef((u%dim)*size(POD_state,1))
                    endif
                 ...
                 ! if(timestep .eq. total_timestep)then
                  ! do i=1,  (u%dim+1)*size(POD_state,1) 
                    !     call rbf_interp_main ((u%dim+1)*size(POD_state,1),pod_coef,i,total_timestep )  
                 !   enddo  
                 ! endif
                   deallocate(pod_coef)
                   deallocate(pod_coef_all_lag_inv)
          enddo reduced_model_loop
                
        end subroutine solve_lag_interpolation

       subroutine solve_rbf(state, u,p,big_m, ct_m, mom_rhs, ct_rhs, inverse_masslump, &
          at_first_timestep, timestep, POD_state, POD_state_deim, snapmean, eps, its,total_timestep)
      ...
      ! logical, optional, intent(in) :: if_optimal
       ...
       p => extract_scalar_field(state(1), "Pressure")
         ...
         reduced_model_loop: do istate = 1, size(state)
              ...
              if (have_option("/reduced_model/Non_intrusive")) then
                   ...
                   if(timestep>ctrl_timestep) then !
                       ...
                       !   pod_coef(i)=0           
                        !  call  ann_bp_main(total_timestep,timestep,pod_coef(i),i) !recalculate the pod_coef(i) using BP_ANN one by one
                       call rbf_interp_1t_main (timestep,3*nsvd, pod_coef, total_timestep )
                    endif                   
                   !save pod_coef, rewrite every timestep 
                  endif ! non-intrusive
        ...
        subroutine solve_rbf_interpolation(state, u,p,big_m, ct_m, mom_rhs, ct_rhs, inverse_masslump, &
          at_first_timestep, timestep, POD_state, POD_state_deim, snapmean, eps, its,total_timestep)
      ...
      ! logical, optional, intent(in) :: if_optimal
       ...
       type(scalar_field), pointer :: POD_pressure
       real, dimension(:,:), allocatable :: pod_coef_all_rbf_inv
       type(pod_matrix_type) :: pod_matrix, pod_matrix_mass, pod_matrix_adv 
       type(pod_rhs_type) :: pod_rhs
       real, dimension(:), allocatable :: pod_coef,pod_coef_dt,pod_coef0
       real, dimension(:,:), allocatable :: pod_sol_velocity, pod_ct_m
       ...
       p => extract_scalar_field(state(1), "Pressure")
        ...
        ! Allocate the change in pressure field
         ...
         allocate(pod_coef((u%dim+1)*size(POD_state,1)))
            ...
            IF(.NOT.ALLOCATED(pod_coef_all_rbf_inv)) then        
                    
                    allocate(pod_coef_all_rbf_inv((u%dim+1)*size(POD_state,1),total_timestep)) 
                    open(11,file='coef_pod_all_tmp') 
                    !read interpolated coef_pod_all from coef_pod_all_tmp which formed in the last timestep of ann
                    read(11,*)((pod_coef_all_rbf_inv(i,j),j=1,total_timestep),i=1,(u%dim+1)*size(POD_state,1))                    
                    close(11)  
              endif
               ! print * ,'aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa'
                pod_coef(:)=pod_coef_all_rbf_inv(:,timestep) !give the interpolated pod_coef
             ...
             reduced_model_loop: do istate = 1, size(state)
                   ...
                   ! save pod_coef for all the time levels
                    if(timestep==1) then
                      open(40,file='coef_pod_all_rbf')
                      write(40,*)(pod_coef(i),i=1,(u%dim+1)*size(POD_state,1))
                      close(40)
                   else
                    open(101,file='coef_pod_all_rbf', position='append',ACTION='WRITE')
                    write(101,*)(pod_coef(i),i=1,(u%dim+1)*size(POD_state,1))
                    close(101)
                    print * ,'pod_coef(i)', pod_coef((u%dim)*size(POD_state,1))
                    endif
                 ...
                 ! endif
                   deallocate(pod_coef)
                   deallocate(pod_coef_all_rbf_inv)
          enddo reduced_model_loop
                
        end subroutine solve_rbf_interpolation

    subroutine solve_smolyak(state, u,p,big_m, ct_m, mom_rhs, ct_rhs, inverse_masslump, &
          at_first_timestep, timestep, POD_state, POD_state_deim, snapmean, eps, its,total_timestep)
         ...
         reduced_model_loop: do istate = 1, size(state)
              ...
              if (have_option("/reduced_model/Non_intrusive")) then
                    ...
                    if(timestep>ctrl_timestep) then ! 
                        !   pod_coef(i)=0           
                        !   call  ann_bp_main(total_timestep,timestep,pod_coef(i),i) !recalculate the pod_coef(i) using BP_ANN one by one
                        call smolyak_main (timestep,3*nsvd, pod_coef, total_timestep,2 )!smolyak_main (timestep,podnum, pod_coef, total_timestep , sparse_max )
                       !call tensor_product_main(timestep,3*nsvd, pod_coef, total_timestep,2 )
                    endif                   
                        !   save pod_coef, rewrite every timestep 
                  endif !   non-intrusive
