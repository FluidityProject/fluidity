...
contains
  ...
  subroutine assemble_advection_diffusion_cg(t, matrix, rhs, state, dt, velocity_name)
    ...
    ! Grid velocity
    velocity_ptr => extract_vector_field(state, "GridVelocity", stat = stat)
    if(stat == 0) then
      assert(velocity_ptr%dim == mesh_dim(t))
      assert(ele_count(velocity_ptr) == ele_count(t))
      ...
      call addto(velocity, velocity_ptr, scale = -1.0)
    else
      ewrite(2, *) "No grid velocity"
    end if
        ...
        if(field_name /= "Pressure") then
          ...
          if(have_option(trim(path) // "/spatial_discretisation/continuous_galerkin")) then       
            ...
            if(option_count(trim(path) // "/boundary_conditions/type::neumann") > 0 &
              & .and. .not. have_option(trim(path) // "/tensor_field::Diffusivity")) then
              call field_warning(state_name, field_name, &
