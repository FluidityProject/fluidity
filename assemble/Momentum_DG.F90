...
contains
   ...
   !    call zero(Abs_wd)
    ...
    if(has_scalar_field(state, "Sigma_d0")) then
      if(have_a) then
        ...
        FLExit("When Sigma_d0 is switched on,'/mesh_adaptivity/mesh_movement/free_surface/wetting_and_drying/a and d0' needs to be set. ")
     end if
   end if
    ...
    colour_loop: do clr = 1, size(colours) 
      ...
      element_loop: do nnid = 1, len
       ...
       call construct_momentum_element_dg(ele, big_m, rhs, ct_rhs,&
  ...
  subroutine construct_momentum_element_dg(ele, big_m, rhs,ct_rhs, &
    ...
    if((have_absorption.or.have_vertical_stabilization.or.have_wd_abs .or. have_swe_bottom_drag .or. have_sigma) .and. &
