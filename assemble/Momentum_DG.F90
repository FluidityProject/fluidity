...
contains
  ...
  subroutine construct_momentum_interface_dg(ele, face, face_2, ni, &
    ...
    logical, intent(in), optional :: partial_stress
    ...
    real, dimension(2,face_loc(U,face),ele_loc(U,ele)) ::&
         & primal_fluxes_mat
    real, dimension(2,mesh_dim(U),mesh_dim(U), face_loc(U,face),ele_loc(U,ele)) ::&
   ...
   subroutine primal_fluxes_partial_stress
      ...
      integer :: d, d1, d2
      real :: flux_factor
      real, dimension(face_loc(U,face), face_loc(U,face)) :: temp
    ...
    subroutine local_assembly_ip_face
      ...
      if (boundary) then
         do d=1,U%dim
            if (no_normal_flow) then
                do d1=1,U%dim
               !! Internal Degrees of Freedom
               
               !penalty flux
               
               Viscosity_mat(d1,d,u_face_loc,u_face_loc) = &
                    Viscosity_mat(d1,d,u_face_loc,u_face_loc) + &
                    penalty_fluxes_mat(1,:,:)*normal(d1,1)*normal(d,1)

               !! External Degrees of Freedom
               
               !!penalty fluxes

               Viscosity_mat(d1,d,u_face_loc,start:finish) = &
                    Viscosity_mat(d1,d,u_face_loc,start:finish) + &
                    penalty_fluxes_mat(2,:,:)*normal(d1,1)*normal(d,1)
            end do
            else if(dirichlet(d)) then
         ...
         do d=1,U%dim
            if (no_normal_flow) then
                  do j = 1, size(u_face_loc)
                     ...
                     Viscosity_mat(d1,d,u_face_loc(j),1:nele) = &
                          Viscosity_mat(d1,d,u_face_loc(j),1:nele) + &
                          primal_fluxes_mat(1,j,:)*normal(d1,j)*normal(d,j)
              
                     Viscosity_mat(d1,d,1:nele,u_face_loc(j)) = &
                          Viscosity_mat(d1,d,1:nele,u_face_loc(j)) + &
                          primal_fluxes_mat(1,j,:)*normal(d1,j)*normal(d,j) 

               !primal fluxes

                     Viscosity_mat(d1,d,1:nele,start+j-1) = &
                          Viscosity_mat(d1,d,1:nele,start+j-1) + &
                          primal_fluxes_mat(2,j,:)*normal(d1,j)*normal(d,j)
                  end do
               ...
               do i=1,mesh_dim(U)
                  if (no_normal_flow) then
                     do j = 1, size(u_face_loc)
                        do d4=1,U%dim
                           do d2=1,U%dim
                              Viscosity_mat(d2,d4,u_face_loc(j),1:nele) = &
                                   Viscosity_mat(d2,d4,u_face_loc(j),1:nele) + &
                                   primal_fluxes_mat_partial_stress(1,i,d,j,:)&
                                   *normal(d2,j)*normal(i,j)&
                                   *normal(d,j)*normal(d4,j)
                              Viscosity_mat(d2,d4,1:nele,u_face_loc(j)) = &
                                   Viscosity_mat(d2,d4,1:nele,u_face_loc(j)) + &
                                   primal_fluxes_mat_partial_stress(1,d,i,j,:)&
                                   *normal(d2,j)*normal(i,j)&
                                   *normal(d,j)*normal(d4,j)
                              Viscosity_mat(d2,d4,1:nele,start+j-1) = &
                                   Viscosity_mat(d2,d4,1:nele,start+j-1) + &
                                   primal_fluxes_mat_partial_stress(2,d,i,j,:)&
                                   *normal(d2,j)*normal(i,j)&
                                   *normal(d,j)*normal(d4,j)
                           end do
                        end do
                     end do
                  else if (dirichlet(i)) then
    ...
    subroutine local_assembly_cdg_face
      ...
      integer :: i,j,d1,d2,d3,nele,face1,face2,d
      integer, dimension(face_loc(U,face)) :: U_face_loc    
      real, dimension(mesh_dim(U),ele_loc(U,ele),face_loc(U,face)) :: R_mat
      real, dimension(mesh_dim(U),mesh_dim(U),mesh_dim(U),ele_loc(U,ele),face_loc(U,face)) :: RNN_mat
      real, dimension(mesh_dim(U),2,2,face_loc(U,face),face_loc(U,face)) :: add_mat
      ...
      do d=1,U%dim
            ...
            if (no_normal_flow) then
                  ...
                  do j=1,face_loc(U,face)
                     do face1 = 1, 2
                        do face2 = 1, 2
                           do d1 = 1, mesh_dim(U)
                              do d2 = 1, mesh_dim(U)
                                 add_mat(:,face1,face2,j,i) = &
                                      add_mat(:,face1,face2,j,i) + &
                                      &(-1.)**(face1+face2)&
                                      *dot_product((R_mat(d1,:,j)), &
                                      &matmul(kappa_mat(d1,d2,:,:),&
                                      R_mat(d2,:,i)))&
                                      *normal(d,i)*normal(:,j)
                              end do
                           end do
               ...
               do face1 = 1, 2
                  do face2 = 1, 2
                     do d1 = 1, mesh_dim(U)
                        do d2 = 1, mesh_dim(U)
                           add_mat(d,face1,face2,:,:) = add_mat(d,face1,face2,:,:) + &
         ...
         do d2 = s, f
            ...
            Viscosity_mat(d2,d,u_face_loc,u_face_loc) = &
                 &Viscosity_mat(d2,d,u_face_loc,u_face_loc) + &
                 &add_mat(d2,1,1,:,:)
         
            !face1 = 1, face2 = 2
            
            Viscosity_mat(d2,d,u_face_loc,start:finish) = &
                 &Viscosity_mat(d2,d,u_face_loc,start:finish) + &
                 &add_mat(d2,1,2,:,:)
         
            !face1 = 2, face2 = 1
         
            Viscosity_mat(d2,d,start:finish,u_face_loc) = &
                 Viscosity_mat(d2,d,start:finish,u_face_loc) + &
                 &add_mat(d2,2,1,:,:)
            
            !face1 = 2, face2 = 2
         
            Viscosity_mat(d2,d,start:finish,start:finish) = &
                 &Viscosity_mat(d2,d,start:finish,start:finish) + &
                 &add_mat(d2,2,2,:,:)
         end do
      end do
    ...
    subroutine local_assembly_cdg_partial_stress_face
      ...
      integer :: i,j,d1,d2,d3,d4,nele,face1,face2,d
      integer, dimension(face_loc(U,face)) :: U_face_loc    
      real, dimension(ele_loc(U,ele),face_loc(U,face),mesh_dim(U)) :: R_mat
      real, dimension(mesh_dim(U),mesh_dim(U),face_loc(U,face),face_loc(U,face),2,2) :: add_mat

      real, dimension(face_loc(U,face),face_loc(U,face)) :: temp, temp2
      real, dimension(face_loc(U,face),ele_loc(U,ele)) :: tempA
      real, dimension(ele_loc(U,ele), face_loc(U,face),mesh_dim(U)) :: tempB 
               ...
               do face2 = 1, 2
                  do face1 = 1, 2
                     do i=1,face_loc(U,face)
                        do j=1,face_loc(U,face)
                           do d1 = 1, mesh_dim(U)
                              add_mat(d1,d2,j,i,face1,face2) = &
                                   add_mat(d1,d2,j,i,face1,face2) + &
                                   &(-1.)**(face1+face2)*temp2(j,i)&
                                   *normal(d1,j)*normal(d2,i)
                           end do
                        end do
                     end do
                  end do
               end do
            else if (dirichlet(d2)) then
               do face2 = 1, 2
                  do face1 = 1, 2
                     ...
                     do face1 = 1, 2
                        do i=1,face_loc(U,face)
                           do j=1,face_loc(U,face)
                              do d3 = 1, mesh_dim(U)
                                 do d4 = 1, mesh_dim(U)
                                    add_mat(d3,d4,j,i,face1,face2) = &
                                         add_mat(d3,d4,j,i,face1,face2) + &
                                         &(-1.)**(face1+face2)*temp(j,i)&
                                         *normal(d1,j)*normal(d2,i)&
                                         *normal(d3,j)*normal(d4,i)
                                 end do
                              end do
                           end do
                        end do
  ...
  subroutine momentum_DG_check_options
    ...
    state_loop: do i=0, nstates-1
       ...
       if (have_option(trim(dg_path)//"/viscosity_scheme/partial_stress_form") .and. .not. &
         ...
         FLAbort("partial stress form is only implemented for the bassi-rebay and compact discontinuous Galerkin viscosity schemes in DG")
       end if
