...
module advection_diffusion_DG
  ...
  public solve_advection_diffusion_dg, construct_advection_diffusion_dg, &
       &calculate_auxiliary_gradient_dg
...
!!$       Grad_T_mat(:, :, :size(T_ele)) = shape_dshape(q_shape, dt_t, detwei)
  ...
  !************************************************************************
  subroutine calculate_auxiliary_gradient_dg(state, aux_grad)
    ...
    real, allocatable, dimension(:,:,:) :: grad_t_mat, dq_t
    real, allocatable, dimension(:,:) :: q_inv, normal, grad_t_rhs
    real, allocatable, dimension(:) :: detwei, detwei_f

    type(element_type), pointer :: t_shape, q_shape, q_shape_f, t_shape_f, t_shape_f_2

    integer, dimension(:), allocatable :: local_glno, tfield_bc_type

    integer, dimension(:), pointer :: t_ele, q_ele, neigh, t_face_l, q_face_l
    integer :: start, finish, ele, ele_2, face, face_2, ni, dim
    logical :: boundary
    ...
    tfield => extract_scalar_field(state, trim(field_name))
    x => extract_vector_field(state, "Coordinate")

    allocate(grad_t_mat(aux_grad%dim, ele_loc(aux_grad, 1), ele_and_faces_loc(tfield, 1)), &
             grad_t_rhs(aux_grad%dim, ele_loc(aux_grad, 1)), &
             q_inv(ele_loc(aux_grad,1), ele_loc(tfield,1)), &
             detwei(ele_ngi(tfield,1)), &
             dq_t(ele_loc(aux_grad,1), ele_ngi(aux_grad,1), aux_grad%dim), &
             detwei_f(face_ngi(tfield,1)), &
             normal(mesh_dim(tfield), face_ngi(tfield, 1)), &
             local_glno(ele_and_faces_loc(tfield, 1)), &
             tfield_bc_type(surface_element_count(tfield)))

    ! get the fields over the surface containing the bcs
    call get_entire_boundary_condition(tfield, (/"weakdirichlet"/), tfield_bc, tfield_bc_type)


    do ele = 1, element_count(tfield)

      t_ele=>ele_nodes(tfield,ele)
      q_ele=>ele_nodes(aux_grad,ele)

      t_shape=>ele_shape(tfield, ele)
      q_shape=>ele_shape(aux_grad, ele)
      ...
      ! Transform q derivatives into physical space.
      call transform_to_physical(x,ele,&
          & q_shape , dshape=dq_t, detwei=detwei)
      ...
      do ni = 1,size(neigh)
        ...
        ele_2=neigh(ni)
        face = ele_face(tfield, ele, ele_2)
        if(ele_2>0) then
          face_2 = ele_face(tfield, ele_2, ele)
          boundary = .false.
        else
          face_2 = face
          boundary = .true.
        ...
        finish = start+face_loc(tfield, face_2)-1

        local_glno(start:finish)=face_global_nodes(tfield, face_2)

        t_face_l=>face_local_nodes(tfield%mesh, face)
        t_shape_f=>face_shape(tfield, face)
    
        t_shape_f_2=>face_shape(tfield, face_2)
        
        q_face_l=>face_local_nodes(aux_grad%mesh, face)
        q_shape_f=>face_shape(aux_grad, face)
        ...
        start = start+face_loc(tfield,face_2)

      end do

      do dim = 1, aux_grad%dim
        call addto(aux_grad, dim, q_ele, matmul(Q_inv, &
                   (matmul(grad_t_mat(dim,:,:), node_val(tfield, local_glno))+grad_t_rhs(dim,:))))
      end do
    ...
    subroutine bassi_rebay_diffusion
      ...
      do dim=1,mesh_dim(tfield)
         ...
         if(.not.boundary) then
            ...
            grad_t_mat(dim, q_face_l, T_face_l)=&
                 grad_t_mat(dim, q_face_l, T_face_l) &
                 +0.5*shape_shape(q_shape_f, T_shape_f, detwei_f*normal(dim,:))

            ! External face.
            grad_t_mat(dim, q_face_l, start:finish)=&
                 +0.5*shape_shape(q_shape_f, T_shape_f_2, detwei_f*normal(dim,:))

         else

            if(tfield_bc_type(face)==1) then
              grad_t_rhs(dim, q_face_l) = grad_t_rhs(dim, q_face_l) + &
                    matmul(shape_shape(q_shape_f, T_shape_f_2, detwei_f*normal(dim,:)), ele_val(tfield_bc, face))
            else
              grad_t_mat(dim, q_face_l, start:finish)=&
                  +shape_shape(q_shape_f, T_shape_f_2, detwei_f*normal(dim,:))
            end if
  ...
  end subroutine calculate_auxiliary_gradient_dg
  !************************************************************************

!!$  subroutine minimal_test(A,B)
!!$    real, dimension(:,:), intent(inout) :: A
!!$    real, dimension(:,:), intent(in) :: B
!$
!!$    integer, dimension(:), allocatable :: T_face_loc
!!$
!!$    allocate( T_face_loc(3) )
!!$
!!$    t_face_loc= (/ 1, 2, 3 /)
!!$    
!!$    A(:,t_face_loc) = &
