...
module darcy_impes_leaching_chemical_model
  ...
  subroutine finalize_leaching_chemical_model(di) 
   ...
   subroutine allocate_leaching_chemical_prog_sfield_src(di,f)
   ...
   subroutine calculate_oxygen_dissolution(di)
        ...
        if (di%MIM_options%have_MIM(2)) then
           ...
           o2_m => di%generic_prog_sfield(f)%MIM%C_a
        ...
        node_loop: do i=1,di%number_pmesh_node
          ...
          if (di%MIM_options%have_MIM(2)) then
             ...
             di%generic_prog_sfield(f)%MIM%immobile_sfield%sfield%val(i)=o2%val(i)*theta2*Fs/theta_im
   ...
   subroutine allocate_leach_heat_transfer_prog_Temperature_src(di,f)
       type(darcy_impes_type), intent(inout) :: di
       integer, intent(in) :: f
