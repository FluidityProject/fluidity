...
module darcy_transport_model
  ...
  public :: darcy_trans_solve_MIM_saturations_and_mass_transfer_coefficient, &
       ...
       calculate_mobile_velocity_dispersity
...
contains
  ...
  subroutine initialize_MIM_model(di)
      ...
      allocate(di%MIM_options%vel_pmsh(di%number_phase)) 
      allocate(di%MIM_options%vel_elemsh(di%number_phase)) 
      allocate(di%MIM_options%Dispersivity_mim(di%number_phase)) 
      allocate(di%MIM_options%constant_zero_tfield_elementwisemesh)
      call allocate(di%MIM_options%constant_zero_tfield_elementwisemesh, di%elementwise_mesh)      
      call zero(di%MIM_options%constant_zero_tfield_elementwisemesh)
      ...
      do p = 1,di%number_phase
         ...
         if (have_option(trim('/material_phase['//int2str(p-1)//']'//'/MobileImmobileModel/tensor_field::Dispersivity_MIM/'))) then
             di%MIM_options%Dispersivity_mim(p)%ptr => extract_tensor_field(di%state(p), "Dispersivity_MIM", stat = stat)
             if (.not. stat == 0) then  
                FLAbort("failed to extract Dispersivity_MIM under mobile immobile model")
             end if
             
             call zero(di%MIM_options%Dispersivity_mim(p)%ptr)
             
             if (have_option(trim('/material_phase['//int2str(p-1)//']'//'/MobileImmobileModel/tensor_field::Dispersivity_MIM/correlation_Ilankoon'))) then  
             
                 di%MIM_options%vel_pmsh(p)%ptr => extract_scalar_field(di%state(p), "Mobile_velocity_magnitude_pressure_mesh", stat = stat)
                 if (.not. (stat == 0)) then            
                   FLAbort("can't extract the field called Mobile_velocity_magnitude_pressure_mesh under correlation_Lima of mobile immobile model")
                 end if
             
                 di%MIM_options%vel_elemsh(p)%ptr => extract_scalar_field(di%state(p), "Mobile_velocity_magnitude_element_mesh", stat = stat)
                 if (.not. (stat == 0)) then            
                   FLAbort("can't extract the field called Mobile_velocity_magnitude_element_mesh under correlation_Lima of mobile immobile model")
                 end if
                 
             else if (have_option(trim('/material_phase['//int2str(p-1)//']'//'/MobileImmobileModel/tensor_field::Dispersivity_MIM/correlation_Lima'))) then
             
                 di%MIM_options%vel_pmsh(p)%ptr => extract_scalar_field(di%state(p), "dispersivity_pressure_mesh", stat = stat)
                 if (.not. (stat == 0)) then            
                   FLAbort("can't extract the field called dispersivity_pressure_mesh under correlation_Lima of mobile immobile model")
                 end if
             
                 di%MIM_options%vel_elemsh(p)%ptr => extract_scalar_field(di%state(p), "dispersivity_element_mesh", stat = stat)
                 if (.not. (stat == 0)) then            
                   FLAbort("can't extract the field called dispersivity_element_mesh under correlation_Lima of mobile immobile model")
                 end if
             
             else             
                FLAbort("can't find the correlation used to calculate the dynamic dispersivity")
             end if   
                


         else
             di%MIM_options%Dispersivity_mim(p)%ptr => di%MIM_options%constant_zero_tfield_elementwisemesh
             di%MIM_options%vel_pmsh(p)%ptr   => di%constant_zero_sfield_pmesh
             di%MIM_options%vel_elemsh(p)%ptr => di%constant_zero_sfield_elementwisemesh
         end if
 ...
 subroutine finalize_MIM_model(di)
   ...
   do p = 1,di%number_phase
      ...
      nullify(di%MIM_options%vel_pmsh(p)%ptr)
      nullify(di%MIM_options%vel_elemsh(p)%ptr)
      nullify(di%MIM_options%Dispersivity_mim(p)%ptr)
      di%MIM_options%have_MIM(p)=.false.
   end do
   ...
   deallocate(di%MIM_options%vel_pmsh) 
   deallocate(di%MIM_options%vel_elemsh) 
   deallocate(di%MIM_options%Dispersivity_mim) 
   call deallocate(di%MIM_options%constant_zero_tfield_elementwisemesh)
   deallocate(di%MIM_options%constant_zero_tfield_elementwisemesh)
 ...
 subroutine darcy_trans_solve_MIM_saturations_and_mass_transfer_coefficient(di)
  ...
  subroutine darcy_trans_MIM_prog_sfield_allocate_rhs_lhs(di,p,f,shared_rhs, shared_lhs,isub)
         ...
         type(scalar_field) :: MIM_src,MIM_src_s,leach_im_src,theta_s,theta_d,theta_s_old, theta_d_old, src_cv_mass, old_cd, old_cs, dtheta_s, dtheta_s_c_dt
         real :: dt
         real :: isub_e, isub_s
         integer :: i,j
         ...
         call allocate(dtheta_s, di%pressure_mesh)
         call allocate(dtheta_s_c_dt, di%pressure_mesh)
         ...
         if (di%lcsub%have_leach_subcycle) then
             ...
             call set(dtheta_s, theta_s)
             call addto(dtheta_s,theta_s_old, scale=-1.0)
             call set(dtheta_s_c_dt, dtheta_s)
             call scale(dtheta_s_c_dt, 1.0/dt)  
         ...
         !(theta_s_new+alpha*dt-(theta_s_new-theta_s_old))
         call addto(shared_lhs, dtheta_s, scale=-1.0)
         ...
         if (.not.(di%generic_prog_sfield(f)%MIM%chem%if_src_linear)) then
            ...
            !!!!calculate shared part rhs '(theta_s_old*Cs_old)/(theta_s_new+alpha*dt-(theta_s_new-theta_s_old))'
            do j=1,di%number_pmesh_node
              if (theta_s%val(j)<1.0E-7) then
                 shared_lhs%val(j)=0.0
              else
                 shared_lhs%val(j)=1.0/shared_lhs%val(j)! now shared lhs '1/(theta_s_new+alpha*dt-(theta_s_new-theta_s_old))'
              end if
            end do
            ...
            !'(alpha*theta_s_old*Cs_old)/(theta_s_new+alpha*dt-(theta_s_new-theta_s_old))'
            ...
            if(di%generic_prog_sfield(f)%MIM%chem%have_chem) then
              ...
              !!! (theta_s_old*Cs_old)/(theta_s_new+alpha*dt-(theta_s_new-theta_s_old))+(dt*S_immobile)/(theta_s_new+alpha*dt-(theta_s_new-theta_s_old))
               call set(MIM_src, shared_lhs) !shared lhs is '1/(theta_s_new+alpha*dt-(theta_s_new-theta_s_old))'
               call scale(MIM_src,leach_im_src)
               call scale(MIM_src,dt) !(dt*S_immobile)/(theta_s_new+alpha*dt-(theta_s_new-theta_s_old))
               call addto(shared_rhs,MIM_src)

               !add the chemistry source part into rhs
               !***(alpha*dt*S_immobile)/(theta_s_new+alpha*dt-(theta_s_new-theta_s_old))
               call scale(MIM_src,di%MIM_options%mass_trans_coef(p)%ptr)
               ...
               !***(alpha*dt*S_immobile)/(theta_s_new+alpha*dt-(theta_s_new-theta_s_old))+S_mobile
               ! rhs added is (alpha*theta_s_old*Cs_old)/(theta_s_new+alpha*dt-(theta_s_new-theta_s_old))+(alpha*dt*S_immobile)/(theta_s_new+alpha*dt-(theta_s_new-theta_s_old))+S_mobile'
            ...
            !-------the component addto the lhs matrix
            !!!!calculate (alpha*dt)/(theta_s_new+alpha*dt-(theta_s_new-theta_s_old)), which is the shared part           
            call scale(shared_lhs,dt)
            call scale(shared_lhs,di%MIM_options%mass_trans_coef(p)%ptr)
            !***add to lhs
            call set(MIM_src, shared_lhs)
            !calculate (alpha^2*dt)/(theta_s_new+alpha*dt-(theta_s_new-theta_s_old))
            call scale(MIM_src,di%MIM_options%mass_trans_coef(p)%ptr)
            !calculate '-(alpha^2*dt)/(theta_s_new+alpha*dt-(theta_s_new-theta_s_old))+alpha'
            do j=1,di%number_pmesh_node
              if (theta_s%val(j)>1.0E-7) then
                 MIM_src%val(j)=MIM_src%val(j)-di%MIM_options%mass_trans_coef(p)%ptr%val(j)               
              end if
              MIM_src%val(j)=-MIM_src%val(j)
            end do
            !'-(alpha^2*dt)/(theta_s_new+alpha*dt-(theta_s_new-theta_s_old))+alpha+(theta_s_new-theta_s_old)/dt'
            call addto(MIM_src, dtheta_s_c_dt)
            call compute_cv_mass(di%positions,src_cv_mass, MIM_src)
            ...
            node_loop3: do i=1,di%number_pmesh_node
               if (old_cs%val(i)<=1.0D-15 .or. shared_rhs%val(i)<=1.0D-15 .or.  theta_s%val(i)<1.0E-7) then
                  MIM_src%val(i)=0.0

               else
                  MIM_src%val(i)=dt*leach_im_src%val(i)/old_cs%val(i)!(dt*S_NegativeImmobie)/(Cs_old)
            ...
            do i=1,di%number_pmesh_node
             if (theta_s%val(i)>1.0E-7) then

               MIM_src%val(i)=-MIM_src%val(i)+shared_lhs%val(i)!(theta_s_new+alpha*dt-(theta_s_new-theta_s_old))-(dt*S_NegativeImmobie)/(Cs_old)
            
               MIM_src%val(i)=1.0/MIM_src%val(i) !1/((theta_s_new+alpha*dt-(theta_s_new-theta_s_old))-(dt*S_NegativeImmobie)/(Cs_old))
             end if
            end do
            ...
            !!!the shared rhs is '(theta_s_old*Cs_old+dt*S_PositiveImmobile)/(theta_s_new+alpha*dt-(theta_s_new-theta_s_old)-dt*S_NegativeImmobie/Cs_old)'
            call addto(shared_rhs, leach_im_src)

            call scale(shared_rhs,MIM_src)
            
            !'alpha*(theta_s_old*Cs_old+dt*S_PositiveImmobile)/(theta_s_new+alpha*dt-(theta_s_new-theta_s_old)-dt*S_NegativeImmobie/Cs_old)'
            ...
            !'alpha*(theta_s_old*Cs_old+dt*S_PositiveImmobile)/(theta_s_new+alpha*dt-(theta_s_new-theta_s_old)-dt*S_NegativeImmobie/Cs_old)  + S_PositiveMobile)'
            ...
            !!!the shared lhs is
            !!!alpha*dt/(theta_s_new+alpha*dt-(theta_s_new-theta_s_old)-dt*S_NegativeImmobie/Cs_old)
            ...
            node_loop4: do i=1,di%number_pmesh_node
               if (old_cd%val(i)<=1.0D-15) then
                  MIM_src%val(i)=0.0

               else
                  MIM_src%val(i)= leach_im_src%val(i)/old_cd%val(i)  !S_NegativeMobile/Cd_old
               end if
             
            end do node_loop4
         
            call set(MIM_src_s, shared_lhs)
            call scale(MIM_src_s,di%MIM_options%mass_trans_coef(p)%ptr) !alpha^2*dt/(theta_s_new+alpha*dt-(theta_s_new-theta_s_old)-dt*S_NegativeImmobie/Cs_old)

            !****add to lhs matrix, the total part is
            !-(S_NegativeMobile*theta_d/theta_d_old*Cd_old+alpha^2*dt/(theta_s_new+alpha*dt-(theta_s_new-theta_s_old)-dt*S_NegativeImmobie/Cs_old)-alpha)
            ...
            !-(S_NegativeMobile*theta_d/theta_d_old*Cd_old+alpha^2*dt/(theta_s_new+alpha*dt-(theta_s_new-theta_s_old)-dt*S_NegativeImmobie/Cs_old)-alpha)+(theta_s_new-theta_s_old)/dt
            call addto(MIM_src, dtheta_s_c_dt)
            call compute_cv_mass(di%positions,src_cv_mass, MIM_src)
            call addto(di%lhs, src_cv_mass)
        ...
        call deallocate(dtheta_s)
        call deallocate(dtheta_s_c_dt)
  ...
  subroutine darcy_trans_assemble_and_solve_immobile_sfield(di, p, f, shared_rhs, shared_lhs)
       ...
       ! calculate '(alpha*dt*C_d)/(theta_s+alpha*dt-(theta_s_new-theta_s_old))'
       !or '(alpha*dt*C_d)/(theta_s_new+alpha*dt-(theta_s_new-theta_s_old)-theta_s*dt*S_NegativeImmobie/theta_s_old*Cs_old)' with src linearization
       ...
       !calculate '(alpha*dt*C_d+theta_s_old*C_s_old+dt*S_immobile)/(theta_s+alpha*dt-(theta_s_new-theta_s_old))'
       !or '(alpha*dt*C_d+theta_s_old*C_s_old+dt*S_PositiveImmobile)/(theta_s_new+alpha*dt-(theta_s_new-theta_s_old)-theta_s*dt*S_NegativeImmobie/theta_s_old*Cs_old)' with src linearization
  ...
  subroutine leaching_MIM_calculate_fields_and_ratio(di,p,f)
    ...
    subroutine calculate_mobile_velocity_dispersity(di)
       type(darcy_impes_type), intent(inout) :: di
       
       integer :: p, node, vele, id, f
       character(len=OPTION_PATH_LEN) ::  path, path2
       real :: da_val, theta_d, disp, mu, rho, d, Re, Bo, g
       integer, dimension(:), pointer :: elenodes
       type(tensor_field), pointer :: temp_diff
       
       do p = 2,di%number_phase
        path2=trim('/material_phase['//int2str(p-1)//']'//'/MobileImmobileModel/tensor_field')
        if (have_option(trim(path2)//'::Dispersivity_MIM')) then
           call zero(di%MIM_options%Dispersivity_mim(p)%ptr)
           call zero(di%MIM_options%vel_pmsh(p)%ptr)
           call zero(di%MIM_options%vel_elemsh(p)%ptr)
           if (have_option(trim('/material_phase['//int2str(p-1)//']'//'/MobileImmobileModel/tensor_field::Dispersivity_MIM/correlation_Lima'))) then  
               !calculate darcy velocity element mesh
               node_loop1: do node=1,di%number_pmesh_node
                   da_val=norm2(node_val(di%darcy_velocity(p)%ptr,node))
                   !the gravity magnitude
                   g=di%gravity_magnitude
                   ...
                   mu=node_val(di%lviscosity_pmesh,node)
                   !liquid density
                   rho=node_val(di%density(p)%ptr,node)
                   !rock particle diameter
                   d=node_val(di%heap%rock_d,node)
                   ...
                   Bo=9.14e-02*(Re**0.00532)
                   
                   di%MIM_options%vel_pmsh(p)%ptr%val(node)=da_val*d/Bo
                                     
               end do node_loop1
               
               call calculate_galerkin_projection(di%state(p), di%MIM_options%vel_elemsh(p)%ptr)
               
               !calculate dynamic dispersivity
               vol_element_loop1: do vele = 1, di%number_vele
                   disp=minval(ele_val(di%MIM_options%vel_elemsh(p)%ptr,vele))
                   elenodes=>ele_nodes(di%MIM_options%Dispersivity_mim(p)%ptr,vele)
               
                   do id=1,di%positions%dim
                     !set to be isotropic
                 
                     di%MIM_options%Dispersivity_mim(p)%ptr%val(id,id,elenodes)=disp
                                
                   end do
               
               end do vol_element_loop1
               nullify(elenodes)
           else

               !calculate mobile velocity
               node_loop2: do node=1,di%number_pmesh_node
                  da_val=norm2(node_val(di%darcy_velocity(p)%ptr,node))
                  theta_d=node_val(di%MIM_options%mobile_saturation(p)%ptr,node)*node_val(di%porosity_pmesh,node)
                
                  di%MIM_options%vel_pmsh(p)%ptr%val(node)=da_val/theta_d
                
               end do node_loop2  
               call calculate_galerkin_projection(di%state(p), di%MIM_options%vel_elemsh(p)%ptr)
               
               !calculate dynamic dispersivity
               vol_element_loop2: do vele = 1, di%number_vele

                   !note that no flux at the boundary surfaces, only the volume element are considered
                   disp=3.3e-05*minval(ele_val(di%MIM_options%vel_elemsh(p)%ptr,vele)*1000.0)**0.65
               ...
               end do vol_element_loop2
               nullify(elenodes)
           end if
           
           !set the field with dynamic dispersion
           do f=1,size(di%generic_prog_sfield)
              if (di%generic_prog_sfield(f)%MIM%if_dynamic_D) then
                 call set(di%generic_prog_sfield(f)%sfield_diff,di%MIM_options%Dispersivity_mim(p)%ptr)
              end if 
           end do
    ...
    end subroutine calculate_mobile_velocity_dispersity
