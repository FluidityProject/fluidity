...
extern "C" {
#define test_coare_ocean_fluxes_fc F77_FUNC(test_coare_ocean_fluxes, TEST_COARE_OCEAN_FLUXES)  
...
*/
    ...
    report_test("[test_coare_ocean_forcing: heat flux error to ERA40]",fail,warn,errorMessage);
    ...
    error = (sumF-sumF_era40)/sumF;
    sprintf(errorMessage, "Error > .75 Is %g",error);
    report_test("[test_coare_ocean_forcing: salinity flux error to ERA40]",fail,warn,errorMessage);

    fail = true;
    error = (sumTauX-sumTauX_era40)/sumTauX;
    if (abs(error) < .75) fail = false;
    sprintf(errorMessage, "Error > .75 Is %g",error);
    report_test("[test_coare_ocean_forcing: tau x momentum error to ERA40]",fail,warn,errorMessage);

    fail = true;
    error = (sumTauY-sumTauY_era40)/sumTauY;
    if (abs(error) < .75) fail = false;
    sprintf(errorMessage, "Error > .75 Is %g",error);
    report_test("[test_coare_ocean_forcing: tau y momentum error to ERA40]",fail,warn,errorMessage);

    // check we have the right direciton of forcing
    if (r_q > .88) fail = false;
    sprintf(errorMessage, "Correlation coefficient < .88: Is %g",r_q); 
    report_test("[test_coare_ocean_forcing: heat flux correlation]",fail,warn,errorMessage);

    fail = true;
    if (r_f > .88) fail = false;
    sprintf(errorMessage, "Correlation coefficient < .88: Is %g",r_f); 
    report_test("[test_coare_ocean_forcing: freshwater flux correlation]",fail,warn,errorMessage);

    fail = true;
    if (r_tauX > .88) fail = false;
    sprintf(errorMessage, "Correlation coefficient < .88: Is %g",r_tauX); 
    report_test("[test_coare_ocean_forcing: momentum (X) flux correlation]",fail,warn,errorMessage);

    fail = true;
    if (r_tauY > .88) fail = false;
    sprintf(errorMessage, "Correlation coefficient < .88: Is %g",r_tauY); 
    report_test("[test_coare_ocean_forcing: momentum (Y) flux correlation]",fail,warn,errorMessage);
