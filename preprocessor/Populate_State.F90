...
contains
  ...
  contains
    ...
    scalar_field_loop: do j=0, nfields-1
      ...
      !insert tensor field
      if (have_option(trim(state_path_MIM)//'/tensor_field::Dispersivity_MIM')) then
         field_name=trim('Dispersivity_MIM')
         path=trim(state_path_MIM)//'/tensor_field::Dispersivity_MIM'
         call allocate_and_insert_tensor_field(path, state, &
                dont_allocate_prognostic_value_spaces=dont_allocate_prognostic_value_spaces)
         nfields=option_count(trim(state_path_MIM)//"/tensor_field::Dispersivity_MIM/correlation_Lima/scalar_field")
         ! Loop over the scalar fields
         scalar_field_loop1: do j=0, nfields-1
  
             !save path
             path=trim(state_path_MIM)//"/tensor_field::Dispersivity_MIM/correlation_Lima/scalar_field["//int2str(j)//"]"
             ! Get Field name
             call get_option(trim(path)//"/name", field_name)
             ! Reset path to have field name rather than index
             path=trim(state_path_MIM)//"/tensor_field::Dispersivity_MIM/correlation_Lima/scalar_field::"//trim(field_name)
  
             call allocate_and_insert_scalar_field(trim(path), state, field_name=field_name, &
               dont_allocate_prognostic_value_spaces=dont_allocate_prognostic_value_spaces)
               
          end do scalar_field_loop1
          
          nfields=option_count(trim(state_path_MIM)//"/tensor_field::Dispersivity_MIM/correlation_Ilankoon/scalar_field")
         ! Loop over the scalar fields
         scalar_field_loop2: do j=0, nfields-1
  
             !save path
             path=trim(state_path_MIM)//"/tensor_field::Dispersivity_MIM/correlation_Ilankoon/scalar_field["//int2str(j)//"]"
             ! Get Field name
             call get_option(trim(path)//"/name", field_name)
             ! Reset path to have field name rather than index
             path=trim(state_path_MIM)//"/tensor_field::Dispersivity_MIM/correlation_Ilankoon/scalar_field::"//trim(field_name)
  
             call allocate_and_insert_scalar_field(trim(path), state, field_name=field_name, &
               dont_allocate_prognostic_value_spaces=dont_allocate_prognostic_value_spaces)
               
          end do scalar_field_loop2
