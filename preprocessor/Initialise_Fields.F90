...
contains
  ...
  subroutine initialise_tensor_field(field, path, position, time)
    ...
    integer :: i
    logical :: is_isotropic, is_symmetric
  ...
  subroutine initialise_scalar_field_over_regions(field, path, position)
  ...
  subroutine initialise_vector_field_over_regions(field, path, position)
  ...
  subroutine initialise_tensor_field_over_regions(field, path,position)
    !!< Wrapper to initialise_field for prescribed fields in case
    !!< mesh regions are being used
    type(tensor_field), intent(inout) :: field
    !! path should point to either /"path to field"/prescribed/value 
    ...
    do value = 0, nvalues-1
       call zero(tempfield)
       call initialise_field(tempfield, &
            trim(path)//'['//int2str(value)//']', position)
       call apply_region_ids(field, tempfield, &
            trim(path)//'['//int2str(value)//']')
    end do
