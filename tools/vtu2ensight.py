...
def parse_args(argv):
    ...
    parser.add_argument(
        ...
        "--static",
        help=(
            "Use this flag only when a fixed mesh was used. By default a dynamically"
            " varying (adaptive) spatial mesh is assumed."
        ),
        action="store_true",
        dest="static",
        default=False,
    ...
    parser.add_argument(
        "-i",
        help=(
            "Use this flag to set the index of the vtu file you wish to convert. By"
            " default all vtu files with the matching basename are converted."
        ),
        dest="dumpno",
        default=-1,
    )
    parser.add_argument(
        ...
        "--last-dump",
        help=(
            "Use this flag to automatically find the vtu file with the highest dump"
            " number and only convert that opposed to all vtu files. Note: It does not"
            " check the timestamps. If -l and -i are given, -i is neglected."
        ),
        action="store_true",
        dest="lastdump",
        default=False,
