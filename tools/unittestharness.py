...
def unittest_harness(tests, xml_outfile):
    ...
    for test in tests:
        ...
        if (tests_dir / test.name).is_file() is False:
            ...
            skip_list.append(test.name)
            xml_entry = TestCase(
                "", classname=test.name, elapsed_sec="None", status="Skipped"
            )
            ...
            try:
                ...
                if test_output[:4] == "Warn":
                    xml_entry.add_failure_info(
                        message=fail_msg.lstrip(), failure_type="warning"
                    )
                else:
...
if args.efence:
    add_path_to_environment_variable("LD_PRELOAD", Path("/usr/lib/libefence.so.0.0"))
...
if args.clean:
    ...
    if args.xml_output:
        ...
        if sys.version_info.major == 3 and sys.version_info.minor >= 8:
            ...
            assert float(version("junit_xml")) >= 1.9, "Please update junit_xml"
        else:
