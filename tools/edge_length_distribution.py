...
def usage():
#
    parser=OptionParser(usage='Usage: %prog <filename> [options]', add_help_option=True, description="This takes the vtus that begin with filename and plots a histogram of the distribution of edge lengths in those vtus. N.B. i) it will make and put plots in a folder called 'edge_length_distribution_plots' ii) it will write two log files 'time.log' and 'edge_lengths.log' to 'edge_length_distribution_plots', see option --plotonly for more information iii) it does not include checkpoint files")
#
    parser.add_option("-s", "--start", dest="start_vtu", type = "int", default = 0, help="the first dump id of files to be included, default = 0")
#
    parser.add_option("-e", "--end", dest="end_vtu", type = "int", help="the last dump id of files to be included, if not used all vtus with id >= start_vtu will be included")
#
    parser.add_option("-b", "--bins", dest="no_bins", type = "int", default = 10, help="number of bins for the histogram plots, default = 10")
#
    parser.add_option("-m", "--maxmin", dest="plot_maxmin", action="store_true", help = "plots the maximum and minimum edge lengths over time")
#
    parser.add_option("-c", "--cumulative", dest="plot_cumulative", action="store_true", help = "plots a histogram of the cumulative total of edge lengths for all vtus")
#
    parser.add_option("-p", "--plotonly", dest="plot_only", action="store_true", help = "will allow plots to be made from the data in the log files 'time.log' and 'edge_lengths.log' rather than extracting the information from the vtus as this can take a while. Note: you must have run the script once WITHOUT this option otherwise the log files will not exist")
#
    parser.add_option("--pvtu", dest="use_pvtu", action="store_true", help = "uses pvtus instead of vtus")
    return parser
...
def PlotEdgeLengths(edge_lengths_all, time, options):
  ...
  for i in range(len(edge_lengths_all)):
    ...
    title("t = "+str(time[i]))
    grid("True")
    savefig("./edge_length_distribution_plots/edge_length_distribution_"+str(time[i])+".png")
...
def PlotCumulative(edge_lengths_all, options):
  ...
  all_vals=[]
  for edge_lengths in edge_lengths_all: all_vals = all_vals+edge_lengths
...
vtu_type = 'vtu'
if options.use_pvtu == True: vtu_type = 'pvtu'
filelist = GetFiles(filename, vtu_type)
time = []
edge_lengths_all = []

try: os.mkdir("edge_length_distribution_plots")
except OSError: pass
