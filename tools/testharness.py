...
def filter_tests(xml_files):
    ...
    for xml_file in xml_files:
        ...
        if (length_condition or nprocs_condition or excluded_tags_condition
        ...
        tests[xml_file] = {
            "id": prob_classname, "length": prob_length, "nprocs": prob_nprocs,
            "command": prob_command, "variables": {}, "pass_tests": {},
            "warn_tests": {}, "elapsed_time": 0, "stdout": "\n",
            "stderr": "\n"}
        ...
        try:
            for test in parsed_xml.find("warn_tests"):
                assert test.get("language").strip() == "python"
                tests[xml_file]["warn_tests"][test.get("name")] = dedent(
                    test.text)
...
def poll_processes(running_procs, core_counter, serial, return_list,
                   task_string, process_interpreter, oversubscribe):
    # Check if running processes have terminated
    proc_status = [tests[running_xml]["running_proc"].poll()
                   for running_xml in running_procs]
    # Reversed to be able to remove items from lists without skipping any item
    for status, running_xml in zip(reversed(proc_status),
                                   reversed(running_procs)):
        if status is not None:  # Test has terminated
            ...
            current_test["elapsed_time"] += (
                monotonic() - current_test["create_time"])
            ...
            stdout = "".join(
                [line for line in read_stream(running_xml, "stdout_file")])
            stderr = "".join(
                [line for line in read_stream(running_xml, "stderr_file")])
...
def process_error(test_xml, process_interpreter, stdout, stderr):
    ...
    xml_entry = TestCase(
        name=test_xml.stem, classname=tests[test_xml]["id"],
        elapsed_sec=tests[test_xml]["elapsed_time"],
        stdout=tests[test_xml]["stdout"], stderr=tests[test_xml]["stderr"],
    ...
    # Errors/Failures generated in Python variables/tests
        ...
        if any(error in stderr for error
...
def run_tasks(task_string, tests_list, serial, process_interpreter,
              task_function):
    ...
    while tests_list:
        ...
        if (core_counter + task_nprocs > core_avail
                and (task_nprocs <= core_avail or oversubscribe)):
            # Check if some processes have terminated
            core_counter, oversubscribe = poll_processes(
                running_procs, core_counter, serial, return_list, task_string,
                process_interpreter, oversubscribe)
...
def task_make_input(test_xml):
    tests[test_xml]["running_proc"] = subprocess.Popen(
        ["make", "input"], cwd=test_xml.parent, encoding="utf-8",
...
def task_run_commands(test_xml):
    tests[test_xml]["running_proc"] = subprocess.Popen(
        tests[test_xml]['command'], cwd=test_xml.parent, shell=True,
        encoding="utf-8", stdout=tests[test_xml]["stdout_file"],
        stderr=tests[test_xml]["stderr_file"])
...
def task_run_tests(test_xml):
    ...
    tests[test_xml]["running_proc"] = subprocess.Popen(
        [sys.executable, "-c", test_string], cwd=test_xml.parent,
        encoding="utf-8", stdout=tests[test_xml]["stdout_file"],
        stderr=tests[test_xml]["stderr_file"])
...
add_path_to_environment_variable("PYTHONPATH",
                                 fluidity_root / "libspud" / "diamond")
...
if args.just_list:
    ...
    for test_xml in tests.keys():
        ...
        try:
            subprocess.run(
                ["make", "clean"], cwd=test_xml.parent, encoding="utf-8",
...
""")
    if args.valgrind:
        ...
        assert "debugging" in fluidity_version.stderr, (
...
""")
        ...
        for test_xml in tests.keys():
            ...
            tests[test_xml]["command"] = f"""valgrind --tool=memcheck \
...
--log-file=test.log {tests[test_xml]["command"]}"""
    ...
    tests_list = run_tasks("*** Executing 'make input'", list(tests.keys()),
                           True, "Shell", task_make_input)
    if tests_list:
        tests_list = run_tasks("*** Executing test commands from XML files",
                               tests_list, False, "Shell", task_run_commands)
    if tests_list:
        tests_list = run_tasks("*** Executing Python tests", tests_list, True,
                               "Python", task_run_tests)

    for test_xml in tests_list:
        xml_entry = TestCase(
            name=test_xml.stem, classname=tests[test_xml]["id"],
            elapsed_sec=tests[test_xml]["elapsed_time"], status="Success")
