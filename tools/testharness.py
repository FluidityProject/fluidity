...
sys.path.insert(0, os.path.join(
    os.getcwd(), os.path.dirname(sys.argv[0]), os.pardir, "python"))
...
class TestHarness:
    def __init__(self, length="any", parallel="any", exclude_tags=None,
        ...
        rootdir = os.path.abspath(os.path.join(
            os.path.dirname(sys.argv[0]), os.pardir))
        dirnames = []
    ...
    def modify_command_line(self, nprocs):
        ...
        def f(s):
            ...
            if not self.genpbs:
                s = s.replace('mpiexec ', 'mpiexec -n %(nprocs)d ' %
                              {'nprocs': nprocs})
        ...
        if not self.justtest:
            ...
            for nprocs in sorted(list(tests_by_nprocs.keys()), reverse=True):
                for i in range(len(threadlist),
                    ...
                    threadlist.append(multiprocessing.Process(
...
if __name__ == "__main__":
    ...
    parser.add_option("-l", "--length", dest="length",
                      help="length of problem (default=any)", default="any")
    ...
    parser.add_option("--just-test", action="store_true",
                      dest="justtest", default=False)
    parser.add_option("--just-list", action="store_true", dest="justlist")
    parser.add_option("--genpbs", action="store_true", dest="genpbs")
    parser.add_option("-x", "--xml-output", dest="xml_outfile",
                      default="", help="filename for xml output")
    parser.add_option("--exit-failure-count", action="store_true",
                      dest="exit_fails", help="Return failure count on exit")
    ...
    os.environ["PATH"] = os.path.abspath(os.path.join(
        os.path.dirname(sys.argv[0]), "..", "bin")) + ":" + os.environ["PATH"]
    os.environ["PATH"] = os.path.abspath(os.path.join(os.path.dirname(
        sys.argv[0]), "..", "libspud", "bin")) + ":" + os.environ["PATH"]
    try:
        os.environ["LD_LIBRARY_PATH"] = os.path.abspath(os.path.join(os.path.dirname(
            sys.argv[0]), "..", "lib")) + ":" + os.environ["LD_LIBRARY_PATH"]
    except KeyError:
