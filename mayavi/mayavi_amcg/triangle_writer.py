...
class TriangleWriter(HasTraits):
    ...
    def write_node_file(self):
        ...
        if number_of_arrays > 0:
            ...
            boundary_marker = (
                self.grid.point_data.get_array(number_of_arrays - 1).data_type == 6
            )
        ...
        for i in range(points):
            f.write("\n" + repr(i + 1) + " ")  # write point number (starting from 1)

            point = self.grid.get_point(i)
            for j in range(dimensions):
                f.write(repr(point[j]) + " ")

            for j in range(number_of_arrays):
                f.write(repr(arrays[j].get_value(i)) + " ")
    ...
    def write_face_file(self):
        ...
        f.write(repr(faces) + " " + repr(int(boundary_marker)))

        if boundary_marker:
            boundary_marker_array = self.grid.cell_data.get_array(0)

        for i in range(faces):
            f.write("\n" + repr(i + 1) + " ")  # write face number (starting from 1)

            cell = self.grid.get_cell(i)
            for j in range(3):  # nodes per triangle
                point = cell.get_point_id(j)
                f.write(repr(point + 1) + " ")

            if boundary_marker:
                f.write(repr(boundary_marker_array.get_value(i)))
    ...
    def write_ele_file(self):
        ...
        for i in range(tetrahedra):
            f.write(
                "\n" + repr(i + 1) + " "
            )  # write tetrahedron number (starting from 1)
            ...
            for j in range(attributes):
                f.write(repr(attribute_arrays[j].get_value(i)) + " ")
