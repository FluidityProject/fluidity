...
def find_fullset(tree):
    ...
    def traverse(node):
        ...
        if node.tag == RELAXNG + "element" or (
            node.tag == RELAXNG + "choice"
            and all(n.tag != RELAXNG + "value" for n in node)
        ):
            ...
            cache.add(query)
            node = tree.xpath(
                '/t:grammar/t:define[@name="' + node.get("name") + '"]',
                namespaces={"t": RELAXNGNS},
            )[0]
