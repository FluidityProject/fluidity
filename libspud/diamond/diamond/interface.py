...
class Diamond:
    ...
    def __init__(
        ...
        self.scherror = scherror.DiamondSchemaError(
            self, gladefile, schemafile, schematron_file
        )
        ...
        if schematron_file is None:
            ...
            menu.get_children()[3].get_submenu().get_children()[1].set_property(
                "sensitive", False
            )
    ...
    def load_file(self, filename):
        ...
        try:
            ...
            if tree_read is None:
                dialogs.error_tb(
                    self.main_window, 'Unable to open file "' + filename + '"'
                )
                return
    ...
    def on_open(self, widget=None):
        ...
        if self.suffix is None:
            ...
            filter_names_and_patterns[config.schemata[self.suffix][0]] = (
                "*." + self.suffix
            )
        else:
    ...
    def on_increase_font(self, widget=None):
        ...
        self.cellcombo.set_property(
            "font-desc", pango.FontDescription(str(self.fontsize))
        )
        self.redraw_tree()
        return
    ...
    def redraw_tree(self):
        ...
        if self.selected_iter is not None:
            expanded_iters.append(self.selected_iter)
    ...
    def __diff(self, path):
        def run_diff(self, path):
            ...
            self.statusbar.set_statusbar(
                "Diff calculated (took " + str(seconds) + " seconds)"
            )
            return False
    ...
    def set_treestore(self, iter=None, new_tree=[], recurse=False, replace=False):
        ...
        for t in new_tree:
            ...
            if isinstance(t, tree.Tree):
                if t.is_hidden():
                    attrid = t.connect(
                        "on-set-attr", self.on_set_attr, self.treestore.get_path(iter)
                    )
                    dataid = t.connect(
                        "on-set-data", self.on_set_data, self.treestore.get_path(iter)
                    )
                ...
                attrid = t.connect(
                    "on-set-attr", self.on_set_attr, self.treestore.get_path(child_iter)
                )
                dataid = t.connect(
                    "on-set-data", self.on_set_data, self.treestore.get_path(child_iter)
                )
                self.signals[t] = (attrid, dataid)
    ...
    def set_combobox_liststore(
        self, column, cellCombo, treemodel, iter, user_data=None
    ):
        """
    ...
    def expand_tree(self, iter):
        ...
        if choice_or_tree.cardinality == "":
            ...
            iter = self.treestore.insert_after(
                self.treestore.iter_parent(iter),
                iter,
                [new_tree, new_tree.get_current_tree(), liststore],
            )
            attrid = new_tree.connect(
                "on-set-attr", self.on_set_attr, self.treestore.get_path(iter)
            )
            dataid = new_tree.connect(
                "on-set-data", self.on_set_data, self.treestore.get_path(iter)
            )
    ...
    def options_tree_select_func(self, selection, model, path, is_selected, data=None):
        ...
        if (
            isinstance(self.selected_node, mixedtree.MixedTree)
    ...
    def get_spudpath(self, active_tree):
        ...
        while name_tree is not None:
            if "name" in name_tree.attrs and name_tree.get_attr("name") is not None:
                ...
                name_tree.parent is not None
                and name_tree.parent.count_children_by_schemaname(name_tree.schemaname)
                > 1
            ):
                siblings = [
                    x
                    for x in name_tree.parent.children
    ...
    def get_painted_tree(self, iter_or_tree, lock_geometry_dim=True):
        ...
        if not isinstance(iter_or_tree, tree.Tree) and not self.iter_is_active(
            ...
            lock_geometry_dim
            and self.geometry_dim_tree is not None
            and self.geometry_dim_tree.data is not None
        ):
        ...
        def search(iter, node, indent=""):
            while iter:
                if self.treestore.get_value(iter, 0) is node:
                    ...
                    child = search(
                        self.treestore.iter_children(iter), node, indent + "  "
                    )
                    if child:
                        return child
    ...
    def set_geometry_dim_tree(self):
        ...
        iter = self.get_treestore_iter_from_xmlpath(
            "/" + self.tree.name + self.data_paths["dim"]
        )
        if iter is None:
            return
        ...
        if isinstance(painted_tree, mixedtree.MixedTree):
            ...
            if (
                not isinstance(painted_tree.datatype, tuple)
                and painted_tree.datatype != "fixed"
            ):
    ...
    def choice_or_tree_matches(
        self, text, choice_or_tree, recurse, search_active_subtrees=False
    ):
        """
        ...
        if choice_or_tree.is_hidden():
            ...
            if self.choice_or_tree_matches(
                text, choice_or_tree.get_current_tree(), False
            ):
                return True
            elif (
                recurse
                and self.find.search_gui.get_object(
                    "searchInactiveChoiceSubtreesCheckButton"
                ).get_active()
            ):
                for opt in choice_or_tree.choices():
                    if (
                        not search_active_subtrees
                        and opt is choice_or_tree.get_current_tree()
                    ):
                        continue
                    if opt.children == []:
                        self.expand_choice_or_tree(opt)
            ...
            if self.get_painted_tree(choice_or_tree).matches(
                ...
                if (
                    comment is not None
                    and comment.data is not None
                    and text_re.search(comment.data) is not None
                ):
                    return True
                elif recurse:
    ...
    def search_treestore(self, text, iter=None):
        ...
        if self.choice_or_tree_matches(
            text, choice_or_tree, isinstance(choice_or_tree, choice.Choice)
        ):
            yield iter
