...
def print_type(datatype, bracket=True):
    ...
    def type_name(datatype):
        ...
        if (
            datatype_string[:8] == "<class '"
            and datatype_string[len(datatype_string) - 2 :] == "'>"
        ) or (
            datatype_string[:7] == "<type '"
            and datatype_string[len(datatype_string) - 2 :] == "'>"
        ):
    ...
    if isinstance(datatype, plist.List):
        if (
            isinstance(datatype.cardinality, int) and datatype.cardinality == 1
        ) or datatype.cardinality == "":
            ...
            if isinstance(datatype.cardinality, int):
                type_as_printable += (
                    str(datatype.cardinality) + " " + list_type_as_printable + "s"
                )
            else:
