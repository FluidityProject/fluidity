...
class AttributeWidget(gtk.Frame):
    ...
    def key_data_func(self, col, cell_renderer, model, iter, data=None):
        ...
        if (
            not self.node.active
            or self.node.attrs[iter_key][0] is None
            or self.node.attrs[iter_key][0] == "fixed"
        ):
    ...
    def combo_edit_start(self, cell_renderer, editable, path):
        ...
        if (
            isinstance(self.node.attrs[iter_key][0][0], tuple)
            and self.node.get_attr(iter_key) is None
        ):
    ...
    def combo_selected(self, cell_renderer, path, new_text):
        ...
        if (
            isinstance(self.node.attrs[iter_key][0][0], tuple)
            and new_text not in self.node.attrs[iter_key][0][0]
        ):
            if self.node.get_attr(iter_key) is None and new_text == "":
                return
    ...
    def set_fontsize(self):
        self.key_renderer.set_property(
            "font-desc", pango.FontDescription(str(self.fontsize))
        )
        self.entry_renderer.set_property(
            "font-desc", pango.FontDescription(str(self.fontsize))
        )
        self.combo_renderer.set_property(
            "font-desc", pango.FontDescription(str(self.fontsize))
        )
