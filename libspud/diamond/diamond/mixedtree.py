...
class MixedTree:
    def __init__(self, parent, child):
        ...
        self.attrs = dict(
            ...
            + [x for x in list(self.child.attrs.items()) if x[0] not in excluded_attrs]
        )
    ...
    def is_tensor(self, geometry_dim_tree):
        ...
        if (
            "dim1" not in list(self.child.attrs.keys())
        ...
        if (
            self.child.attrs["dim1"][0] != "fixed"
            or self.child.attrs["rank"][0] != "fixed"
        ):
            return False
        ...
        if "dim2" in list(self.child.attrs.keys()):
            ...
            if (
                ...
                or str(self.child.attrs["shape"][0].cardinality)
                != self.child.get_attr("rank")
            ):
                return False

        # Otherwise, the rank must be one and the shape an integer
        elif (
            self.child.get_attr("rank") != "1"
            or self.child.attrs["shape"][0] is not int
        ):
            return False

        # The data for the element must be a list of one or more
        if (
            not isinstance(self.datatype, plist.List)
            or self.datatype.cardinality != "+"
        ):
            return False
    ...
    def is_symmetric_tensor(self, geometry):
        ...
        return (
            dim1 == dim2
            and "symmetric" in list(self.child.attrs.keys())
            and self.child.get_attr("symmetric") == "true"
        )
