...
def long_message(parent, message):
    ...
    message_dialog = gtk.Dialog(
        parent=parent, buttons=(gtk.STOCK_OK, gtk.ResponseType.ACCEPT)
    )
...
def error(parent, message):
    ...
    error_dialog = gtk.MessageDialog(
        parent, 0, gtk.MessageType.WARNING, gtk.ButtonsType.OK, message
    )
...
def console(parent, locals=None):
    ...
    console_dialog = gtk.Dialog(
        parent=parent, buttons=(gtk.STOCK_QUIT, gtk.ResponseType.ACCEPT)
    )
...
def message_box(window, title, message):
    dialog = gtk.MessageDialog(
        window, 0, gtk.MessageType.INFO, gtk.ButtonsType.OK, message
    )
