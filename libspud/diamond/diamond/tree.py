...
class Tree(gobject.GObject):
    ...
    __gsignals__ = {
        "on-set-data": (gobject.SignalFlags.RUN_LAST, gobject.TYPE_NONE, (str,)),
    ...
    def set_attr(self, attr, val):
        ...
        if invalid:
            raise Exception("invalid data: ({}, {})".format(datatype, val))
    ...
    def set_data(self, data):
        ...
        if invalid:
            raise Exception("invalid data: ({}, {})".format(str(self.datatype), data))
        self.data = data
        self.recompute_validity()
            ...
            for tree in outlist:
                if tree.cardinality in ["+", "*"]:
                    inactive_list = [
                        x
                        for x in outlist
                        if x.schemaname == tree.schemaname and x.active is False
                    ]
    ...
    def write(self, filename):
        ...
        xmlTree = etree.tostring(
            self.write_core(None),
            pretty_print=True,
