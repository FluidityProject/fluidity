...
class DataWidget(gtk.VBox):
    ...
    def is_node_editable(self):
        return (
            ...
            and (
                not self.node.is_tensor(self.geometry_dim_tree)
        ...
        for i in range(dim1):
            for j in range(dim2):
                ...
                if not is_symmetric or i >= j:
                    entry.show()
                    entry.connect(
                        "focus-in-event", self.tensor_element_focus_in, jindex, iindex
                    )

                    if self.node.data is None:
                        entry.set_text(datatype.print_type(self.node.datatype.datatype))
                        entry.modify_text(
                            gtk.StateFlags.NORMAL, gdk.color_parse("blue")
                        )
                    else:
                        entry.set_text(
    ...
    def set_data_combo(self):
        ...
        if self.node.data is None:
            ...
            self.data.get_child().modify_text(
                gtk.StateFlags.NORMAL, gdk.color_parse("blue")
            )
            self.data.get_child().modify_text(
                gtk.StateFlags.PRELIGHT, gdk.color_parse("blue")
            )
        ...
        if (
            isinstance(self.node.datatype[0], tuple)
    ...
    def data_entry_store(self):
        ...
        if self.node.data is None and not self.interacted:
            ...
            if value_check is None:
                ...
                if (
                    isinstance(self.node, mixedtree.MixedTree)
    ...
    def data_tensor_store(self):
        ...
        for i in range(dim1):
            for j in range(dim2):
                if (
                    self.interacted[j + i * dim2]
                    and entry_values[j + i * dim2] != ""
                    and (
                        self.node.data is None
        ...
        if not changed:
            ...
            self.node.data is None and False in self.interacted
        ) or "" in entry_values:
            dialogs.error(None, "Invalid value entered")
            return False
    ...
    def combo_focus_child(self, container, widget):
        ...
        if not self.interacted:
            ...
            if self.node.data is None:
                ...
                self.data.get_child().modify_text(
                    gtk.StateFlags.PRELIGHT, gdk.color_parse("black")
                )
    ...
    def set_fontsize(self):
        if self.frame.get_child() is not None:
            if isinstance(self.data, gtk.TextView) or isinstance(
                ...
                self.data.get_child().modify_font(
                    pango.FontDescription(str(self.fontsize))
                )
            elif isinstance(self.data, gtk.Table):
                dim1, dim2 = self.data.get_size()
                for i in range(dim1):
                    for j in range(dim2):
                        self.data.get_children()[i + j * dim1].modify_font(
                            pango.FontDescription(str(self.fontsize))
                        )
