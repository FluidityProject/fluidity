...
class Dom:
    ...
    def __str__(self, indent=""):
        ...
        children = ("\n" + indent).join(
            child.__str__(indent + "  ") for child in self.children
        )
    ...
    def path(self):
        ...
        if self.parent:
            siblings = [
                sibling for sibling in self.parent.elements() if sibling.tag == self.tag
            ]
            if len(siblings) != 1:
...
def get_parent_nodes(tree):
    ...
    if tree.children:
        return utils.flatten([get_parent_nodes(child) for child in tree.children]) + [
            tree
        ]
    else:
        return []
    ...
    if tree.depth == depth:
        ...
        if tree.children:
            return utils.flatten(
                [get_depth_nodes(child, depth) for child in tree.children]
            )
        else:
            return []
...
def compare_value(value1, value2):
    ...
    if value1 is None or value2 is None:
        return 1.0
    return 1.0 - (
        float(len(lcs.lcs(lcs.path(value1, value2)))) / max(len(value1), len(value2))
    )
...
def compare_children(children1, children2, M):
    return float(len(common(children1, children2, M))) / max(
...
def alignchildren(t1, t2, M, E, w, x):
    ...
    s1 = [
        child for child in w.elements() if child in M.left and M.left[child].parent == x
    ]
    s2 = [
        child
