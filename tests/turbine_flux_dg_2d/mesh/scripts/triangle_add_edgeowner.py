...
def nodedupl_recursion(elein, edgein, nodeid, boundary_id):
    ...
    if debug > 1:
        print(
            "Lets loop around nodeid",
            nodeid,
            " starting with ele",
            elein + 1,
            " with boundary edge ",
            edgein + 1,
            " until we reach the next surface edge with id ",
            next_edgein + 1,
        )
    ...
    if (
        triangle.ele_with_edgeids(next_edgein)[0]
        == next_elein_list[len(next_elein_list) - 1]
    ):
        copy_surfaceowner_ids[next_edgein] = (
            triangle.ele_with_edgeids(next_edgein)[1] + 1
        )
    else:
        copy_surfaceowner_ids[next_edgein] = (
            triangle.ele_with_edgeids(next_edgein)[0] + 1
        )
    ...
    nodedupl_recursion(
        next_elein_list[len(next_elein_list) - 1], next_edgein, next_nodeid, boundary_id
    )
...
copy_surfaceowner_ids = [
    -1 for i in range(0, len(triangle.surface_ids))
...
triangle.save_edgefile2(
    copy_edges, copy_surface_ids, copy_surfaceowner_ids, filename + "_edgow.edge"
)
