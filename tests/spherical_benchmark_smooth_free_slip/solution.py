...
A = -0.5*Ra*(R1**(k + l + 2) - R2**(k + l + 2))*R1*R2*g/((R1*R2**(2*l) - R1**(2*l)*R2)*(k + l + 2)*(k - l + 3)*(2*l + 1))
B = -0.5*Ra*(R1**l*R2**(k + 1) - R1**(k + 1)*R2**l)*R1**(l + 3)*R2**(l + 3)*g/((R1**(2*l + 3) - R2**(2*l + 3))*(k + l + 4)*(k - l + 1)*(2*l + 1))
C = -0.5*Ra*(R1**(k + l + 4) - R2**(k + l + 4))*g/((R1**(2*l + 3) - R2**(2*l + 3))*(k + l + 4)*(k - l + 1)*(2*l + 1))
D = -0.5*Ra*(R1**l*R2**(k + 3) - R1**(k + 3)*R2**l)*R1**l*R2**l*g/((R1*R2**(2*l) - R1**(2*l)*R2)*(k + l + 2)*(k - l + 3)*(2*l + 1))
E = g/(eta*(k + l + 4)*(k + l + 2)*(k - l + 3)*(k - l + 1))

# coefficients for pressure solution: p = G r^l + H r^{-l-1} + K r^{k+1}
G = -2*eta*(l+1)*(2*l+3)*C
H = -2*eta*l*(2*l-1)*D
K = -g*(k+2)/((k+1)*(k+2)-l*(l+1))
