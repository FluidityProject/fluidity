...
def report_convergence(file1, file2):
    ...
    errortop_l2_1 = sqrt(
        sum(
            stat1["Fluid"]["FreeSurfaceDifference"]["surface_l2norm%Top"][1:] ** 2
            * stat1["dt"]["value"][1:]
        )
    )
    errortop_l2_2 = sqrt(
        sum(
            stat2["Fluid"]["FreeSurfaceDifference"]["surface_l2norm%Top"][1:] ** 2
            * stat2["dt"]["value"][1:]
        )
    )
    ...
    errorbottom_l2_1 = sqrt(
        sum(
            stat1["Fluid"]["FreeSurfaceDifference"]["surface_l2norm%Bottom"][1:] ** 2
            * stat1["dt"]["value"][1:]
        )
    )
    errorbottom_l2_2 = sqrt(
        sum(
            stat2["Fluid"]["FreeSurfaceDifference"]["surface_l2norm%Bottom"][1:] ** 2
            * stat2["dt"]["value"][1:]
        )
    )
    ...
    error_l2_1 = sqrt(
        sum(
            stat1["Fluid"]["FreeSurfaceDifference"]["surface_l2norm%Both"][1:] ** 2
            * stat1["dt"]["value"][1:]
        )
    )
    error_l2_2 = sqrt(
        sum(
            stat2["Fluid"]["FreeSurfaceDifference"]["surface_l2norm%Both"][1:] ** 2
            * stat2["dt"]["value"][1:]
        )
    )
    ...
    quad1 = quad(
        lambda t: solution.nond_error_amp(stat1, t) ** 2,
        stat1["ElapsedTime"]["value"][0],
        stat1["ElapsedTime"]["value"][-1],
        limit=1000,
    )
    quad2 = quad(
        lambda t: solution.nond_error_amp(stat2, t) ** 2,
