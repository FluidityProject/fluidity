...
for nodeid in nodeid_to_test:
    for k in range(0, len(triangle.nodes)):
        if k + 1 == nodeid:
            continue
        if (
            (triangle.nodes[k][0] - triangle.nodes[nodeid - 1][0]) ** 2
            + (triangle.nodes[k][1] - triangle.nodes[nodeid - 1][1]) ** 2
        ) < 1e-8:
            print(
                "\tFound two nodes at the same position with ids: ",
                nodeid,
                " ",
                triangle.nodes[nodeid - 1],
                " and ",
                k + 1,
                triangle.nodes[k],
                " ",
                ". The latter node will be deleted.",
            )
            ...
            for index, ele in enumerate(triangle.eles):
                triangle.eles[index] = [
                    nodeid if x == k + 1 else x for x in triangle.eles[index]
                ]
                if len(Set(triangle.eles[index])) != len(
                    triangle.eles[index]
                ):  # We dont need elements with nodes k+1 and nodeid
            ...
            for index, edge in enumerate(triangle.edges):
                triangle.edges[index] = [
                    nodeid if x == k + 1 else x for x in triangle.edges[index]
                ]
...
triangle.save_edgefile2(
    triangle.edges,
    triangle.surface_ids,
    triangle.edge_attribute1,
    filename + "_nosup2.edge",
)
