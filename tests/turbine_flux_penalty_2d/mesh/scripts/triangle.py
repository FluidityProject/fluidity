...
def save_nodefile(nodes, dim, filename):
    ...
    f.write(repr(len(nodes)) + " " + repr(dim) + " 0 0\n")
    k = 1
    for node in nodes:
        f.write(repr(k) + " " + "".join([repr(n) + " " for n in node]) + "\n")
        k = k + 1
...
def save_elefile(eles, par1, filename):
    f = open(filename, "w")
    f.write(repr(len(eles)) + " " + repr(len(eles[0])) + " 1\n")
    if len(par1) != len(eles):
        print(
            "Error in save_elefile(): Length of element list is ",
            len(eles),
            "  but length of region_id list is: ",
            len(par1),
        )
        exit()
    k = 1
...
def save_edgefile2(edges, atr1, atr2, filename):
    f = open(filename, "w")
    f.write(repr(len(edges)) + " 2\n")
    if len(atr1) != len(edges) or len(atr2) != len(edges):
        print(
            "Error in save_edgefile(): Length of edge list is ",
            len(edges),
            "  but length of atr1, atr2 list is: ",
            len(atr1),
            ", ",
            len(atr2),
        )
...
def pnode(fnode):
    ...
    if len(nodes) != nbnodes:
        print(
            "Error: actual number of nodes does not corresponds " + str(len(nodes)),
            nbnodes,
        )
        sys.exit(2)
...
def pedges(fedge):
    ...
    if len(edges) != nbedges:
        print(
            "Error: actual number of edges does not corresponds " + str(len(edges)),
            str(nbedges),
        )
        sys.exit(2)
...
def peles(fele):
    ...
    if len(eles) != nbeles:
        print(
            "Error: actual number of eles does not corresponds " + str(len(eles)),
            str(nbeles),
        )
        sys.exit(2)
...
def ele_with_edgeids(edgeids):
    ...
    if len(ret) > 2:
        print(
            "There are ",
            len(ret),
            " elements adjacent to face with id ",
            edgeids,
            ". Only 2 were expected.",
        )
        exit()
    return ret
...
def ele_with_nodeids(nodeids):
    ...
    if len(ret) != 2:
        print(
            "There are ",
            len(ret),
            " elements adjacent to nodes with ids ",
            nodeids,
            ". Only 2 were expected.",
        )
        exit()
...
def has_ele_edge_on_boundaryid(elein, nodeid, boundary_id):
    ...
    for n in eles[elein]:
        ...
        if edge != [] and get_boundaryid_from_edgeid(edge[0]) == boundary_id:
            if debug > 3:
                print(
                    "Found boundary edge with id",
                    edgeid_from_nodeids([n, nodeid])[0] + 1,
                )
            return True
    return False
...
def delete_nodeid(nodeid):
    ...
    for index, ele in enumerate(eles):
        if nodeid in ele:
            print(
                "Error in delete_nodeid(",
                nodeid,
                "). This node is still in use in element ",
                index + 1,
                ": ",
                ele,
                ".",
    ...
    for index, edge in enumerate(edges):
        if nodeid in edge:
            print(
                "Error in delete_nodeid(",
                nodeid,
                "). This node is still in use in edge ",
                index + 1,
                ".",
            )
            exit()
