...
import vtktools
from numpy import abs, cos, dot, exp, sin, size, sqrt, zeros
from pylab import *
...
def IC(X):
    ...
    L = 1.0e6
    T = 10.0 * exp(
        -((X[0] - 2.0 * L / 3.0) ** 2 + (X[1] - L / 3.0) ** 2) / (2.0 * (L / 12.5) ** 2)
    )
    return T
...
def func(X, t):
    ...
    u = (
        0.5
        * cos(3.1415926535897933e-06 * X[1])
        * (p * exp(X[0] * zplus) + q * exp(X[0] * zminus) - 1.0)
    )
    # v = -d psi/d x
    v = (
        -A
        * sin(3.1415926535897933e-06 * X[1])
        * (p * zplus * exp(X[0] * zplus) + q * zminus * exp(X[0] * zminus))
    )
...
def function_analytical(x, y, t_start, t_end):
    ...
    Analytical = 10.0 * exp(
        -((pos[-1, 0] - 2.0 * L / 3.0) ** 2 + (pos[-1, 1] - L / 3.0) ** 2)
        / (2.0 * (L / 12.5) ** 2)
    )
