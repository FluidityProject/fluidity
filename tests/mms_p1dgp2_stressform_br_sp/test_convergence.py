...
#     a_error = stat("MMS_"+str(mesh[0])+".stat")["NS"]["PressureError"]["l2norm"][-1]
#     b_error = stat("MMS_"+str(mesh[1])+".stat")["NS"]["PressureError"]["l2norm"][-1]
    
#     ratio = a_error / b_error
    
#     print mesh[0] + '->' + mesh[1] + ': ', log(ratio, 2)

#     convergence[1] = min(log(ratio, 2), convergence[1])
