...
for file in files:
   ...
   for i in range(0,len(temp)):
      if( (abs(pos[i,0] - x0) < 0.1) & (abs(pos[i,1] - y0) < 0.1) ):
         xyzkkden.append((pos[i,0],pos[i,1],-pos[i,2] + 352332,temp[i],vel[i,0],vel[i,1],vel[i,2],kk[i],1000*den[i],sal[i],phy[i],z[i],nut[i],det[i]))
...
day,mld_1,mld_2=loadtxt("mld_1989.csv", delimiter=",",unpack=True)
mld_1[:] = -1*mld_1[:]
mld_2[:] = -1*mld_2[:]
