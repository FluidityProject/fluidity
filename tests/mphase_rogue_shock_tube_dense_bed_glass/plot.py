...
for fileindex in range(1, 1334, 5):
    ...
    probedpressure_upstream = vtktools.vtu.ProbeData(
        vt, numpy.array([[0.0125, 1.35 - 0.11, 0]]), "Gas::Pressure"
    )
    probedpressure_downstream = vtktools.vtu.ProbeData(
        vt, numpy.array([[0.0125, 1.37 + 0.718, 0]]), "Gas::Pressure"
    )
...
pylab.plot(
    numerical_t, numerical_p_upstream, "g-", label="Numerical - Upstream (Fluidity)"
)
pylab.plot(
    numerical_t, numerical_p_downstream, "r-", label="Numerical - Downstream (Fluidity)"
)
...
pylab.plot(
    ...
    experimental_p_upstream,
    "go",
    label="Experimental - Upstream (Rogue et al., 1998)",
)
pylab.plot(
    experimental_t_downstream,
    experimental_p_downstream,
    "ro",
    label="Experimental - Downstream (Rogue et al., 1998)",
)
