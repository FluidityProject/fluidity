...
def F1l(p):
    ...
    if p <= pl:
        return (
            ...
            * al
            / (gamma - 1.0)
            * (1.0 - (p / pl) ** ((gamma - 1.0) / (2.0 * gamma)))
        )
    else:
        return -(p - pl) * sqrt(
            (2.0 / (gamma + 1.0) / rhol) / (p + (gamma - 1.0) / (gamma + 1.0) * pl)
        )
...
def F1lprime(p):
    ...
    if p <= pl:
        return -al / gamma * (p / pl) ** ((gamma - 1.0) / (2.0 * gamma) - 1.0)
    else:
        return -1.0 * sqrt(
            (2.0 / (gamma + 1.0) / rhol) / (p + (gamma - 1.0) / (gamma + 1.0) * pl)
        ) + -(p - pl) / 2.0 / sqrt(
...
def F3r(p):
    ...
    if p <= pr:
        return (
            ...
            * ar
            / (gamma - 1.0)
            * (1.0 - (p / pr) ** ((gamma - 1.0) / (2.0 * gamma)))
        )
    else:
        return (p - pr) * sqrt(
            (2.0 / (gamma + 1.0) / rhor) / (p + (gamma - 1.0) / (gamma + 1.0) * pr)
        )
...
def F3rprime(p):
    # derivative of the above
    if p <= pr:
        return ar / gamma * (p / pr) ** ((gamma - 1.0) / (2.0 * gamma) - 1.0)
    else:
        return sqrt(
            (2.0 / (gamma + 1.0) / rhor) / (p + (gamma - 1.0) / (gamma + 1.0) * pr)
        ) + (p - pr) / 2.0 / sqrt(
...
if ps < pl:  # rarefaction
    ...
    rhosl = (
        rhol
        * ((gamma - 1.0) / (gamma + 1.0) * pl / ps + 1.0)
        / (pl / ps + (gamma - 1.0) / (gamma + 1.0))
    )  # eqn (3.1.176)
    s1 = ul - al * sqrt(
        (gamma + 1.0) / 2.0 / gamma * ps / pl + (gamma - 1.0) / 2.0 / gamma
    )  # shock speed, eqn (3.1.177)
iesl = ps / rhosl / (gamma - 1.0)
...
if ps < pr:  # rarefaction
    ...
    rhosr = (
        rhor
        * (ps / pr + (gamma - 1.0) / (gamma + 1.0))
        / ((gamma - 1.0) / (gamma + 1.0) * ps / pr + 1.0)
    )  # eqn (3.1.182)
    s3 = ur + ar * sqrt(
        (gamma + 1.0) / 2.0 / gamma * ps / pr + (gamma - 1.0) / 2.0 / gamma
    )  # shock speed, eqn (3.1.183)
iesr = ps / rhosr / (gamma - 1.0)
    ...
    if x / t < us:
        ...
        if ps < pl:
            ...
            if x / t < ul - al:  # left
                ...
                p = pl * (
                    2.0 / (gamma + 1.0)
                    + (gamma - 1.0) / (gamma + 1.0) / al * (ul - x / t)
                ) ** (
                    2 * gamma / (gamma - 1.0)
                )  # eqn (3.1.97)
                u = (
                    2.0 / (gamma + 1.0) * (al + (gamma - 1.0) / 2.0 * ul + x / t)
                )  # eqn (3.1.97)
                rho = rhol * (
                    2.0 / (gamma + 1.0)
                    + (gamma - 1.0) / (gamma + 1.0) / al * (ul - x / t)
        ...
        if ps < pr:
            ...
            if x / t > ur + ar:  # right
                ...
                u = (
                    2.0 / (gamma + 1.0) * (-ar + (gamma - 1.0) / 2.0 * ur + x / t)
                )  # eqn (3.1.98)
                rho = rhor * (
                    2.0 / (gamma + 1.0)
                    - (gamma - 1.0) / (gamma + 1.0) / ar * (ur - x / t)
