...
#.spm file must have 6 face selections named 'Face1', 'Face2',....'Face6', and 2 material selections named 'mat1' and 'mat2'
...
with open(sys.argv[1],"r") as f:
	...
	for i in lines:
        	...
        	if i.strip() == "mat1":				#find lines with selection info	
            		start = lines.index(i)  
			selectionlines = [item.rstrip('\n') for item in lines[start:start + 11]] #lines of interest into list               
			#print selectionlines
			mat1 = ([int(i) for i in selectionlines[3].split()])
			mat2 = ([int(i) for i in selectionlines[10].split()])	
			#print mat1, mat2
		...
		for i in lines[2:]:
			if i != lines[1] and i != lines[0] and i != " ":	#ignore blank lines and nodes per element lines	
				...
				if count-1 in mat1:			#assign material ID
					...
					matID = "2"
					f2.write(countstr + " " + node_string + " " + matID + '\n') #adding element no., nodes and newline to .ele file
