...
with open(sys.argv[1],"r") as f3:				#open up file given in command line and read
    	...
    	facelines = [item.rstrip('\n') for item in f3lines[startline:startline + 46]] #put lines of interest into list
    	total_faces = str((int(facelines[2]) + int(facelines[9]) + int(facelines[16]) + int(facelines[23]) + int(facelines[30]) + int(facelines[37]) + int(facelines[44])) / 2)		#find total number of faces (half the values given in .spm file due to 									additional orientation info)
	...
	interface = ([int(i) for i in facelines[45].split()])
	Interface = [[interface[i],interface[i+1]] for i in range(0,len(interface)-1,2)]
	count = 0
