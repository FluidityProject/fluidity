...
_sage_const_2p5 = RealNumber("2.5")
_sage_const_0p25 = RealNumber("0.25")
...
pavg = N(
    integrate(integrate(p, x, _sage_const_0, pi), y, _sage_const_0, pi)
    / (pi**_sage_const_2)
)
p -= pavg
...
tau_xx1 = _sage_const_2 * nu * diff(u1, x) - (
    _sage_const_2p0 / _sage_const_3p0
) * nu * (diff(u1, x) + diff(v1, y))
tau_xy1 = nu * (diff(u1, y) + diff(v1, x))
tau_yy1 = _sage_const_2 * nu * diff(v1, y) - (
    _sage_const_2p0 / _sage_const_3p0
) * nu * (diff(u1, x) + diff(v1, y))
tau_yx1 = nu * (diff(u1, y) + diff(v1, x))

tau_xx2 = _sage_const_2 * nu * diff(u2, x) - (
    _sage_const_2p0 / _sage_const_3p0
) * nu * (diff(u2, x) + diff(v2, y))
tau_xy2 = nu * (diff(u2, y) + diff(v2, x))
tau_yy2 = _sage_const_2 * nu * diff(v2, y) - (
    _sage_const_2p0 / _sage_const_3p0
) * nu * (diff(u2, x) + diff(v2, y))
tau_yx2 = nu * (diff(u2, y) + diff(v2, x))

Su1 = (
    vfrac1 * rho1 * u1 * diff(u1, x)
    + vfrac1 * rho1 * v1 * diff(u1, y)
    - diff(vfrac1 * tau_xx1, x)
    - diff(vfrac1 * tau_xy1, y)
    + vfrac1 * diff(p, x)
)
Sv1 = (
    vfrac1 * rho1 * u1 * diff(v1, x)
    + vfrac1 * rho1 * v1 * diff(v1, y)
    - diff(vfrac1 * tau_yx1, x)
    - diff(vfrac1 * tau_yy1, y)
    + vfrac1 * diff(p, y)
)
Su2 = (
    vfrac2 * rho2 * u2 * diff(u2, x)
    + vfrac2 * rho2 * v2 * diff(u2, y)
    - diff(vfrac2 * tau_xx2, x)
    - diff(vfrac2 * tau_xy2, y)
    + vfrac2 * diff(p, x)
)
Sv2 = (
    vfrac2 * rho2 * u2 * diff(v2, x)
    + vfrac2 * rho2 * v2 * diff(v2, y)
    - diff(vfrac2 * tau_yx2, x)
    - diff(vfrac2 * tau_yy2, y)
    + vfrac2 * diff(p, y)
)
