...
def usage():
    print("Usage:")
    print(
        "plotpoint.py -r radius --file=detector_filename --csv=basename(optional)"
        " --save=filenamebase -t timestepsize(optional) --d0=d0 -b begintimestep -e"
        " endtimestep\n"
    )
    print(
        "Specifying a timestepsize will change the timestep for the analytical solution"
        " only."
    )
    print(
        "--save=true will save the plots as images instead of plotting them on the"
        " screen."
    )
    print(
        "-r radius. Specifies the point where the measurements will be performed. Can"
        " be positive or negative."
    )
    print(
        "--csv=baseame instead of the detectors use csv files created with paraview"
...
def analytic_solution(r, t):
    ...
    eta = eta - 1
    eta = eta - r**2 / (R**2) * (
        (1.0 - A**2) / ((1.0 - A * cos(omega * t)) ** 2) - 1.0
    )
    eta = h0 * eta
...
def main(argv=None):
    ...
    try:
        opts, args = getopt.getopt(
            sys.argv[1:], "v:t:r:b:e:", ["file=", "csv=", "save=", "d0="]
        )
    ...
    timeplot_yvalue_analytical = (
        []
    )  # these are the analytical scalar values at the specified point
    r_index = (
        -1
    )  # this is the index to access the detector or csv point closest to the user specified point
    ...
    for t in range(0, len(timesteps)):
        ...
        # use csv data
            ...
            timeplot_yvalue_analytical.append(
                analytic_solution(xcoords[i], timesteps[t]) + offset + handoffset
            )

    # Plot result of all timesteps
    ax2.plot(timeplot_xvalue, timeplot_yvalue, "--", label="Numerical solution")
    ax2.plot(
        timeplot_xvalue, timeplot_yvalue_analytical, "-", label="Analytical solution"
    )
