...
def usage():
    print("Usage:")
    print(
        "plotfs_detec.py -t timestep --file=detector_filename --csv=basename(optional)"
        " --save=filenamebase\n"
    )
    print(
    ...
    print(
        "--csv=basename instead of the detectors use csv files created with paraview"
...
def bathymetry_function(X):
    ...
    return (
        h0 * (R**2 - r2) / (R**2) + 0.5 - h0 * (R**2 - Rmesh**2) / (R**2)
    )  # positive number
...
def main(argv=None):
    ...
    for t in range(0, len(timesteps)):
        ...
        for i in range(-250, 250):  # number of points for the analytical solution
            ...
            fsvalues_ana.append(
                analytic_solution(xcoords[-1] * 1000, t * timestep_ana)
                + offset
                + handoffset
            )
        ...
        if verbose:
            ...
            print(
                "Error in the domain center: ",
                abs(fsvalues_ana[len(fs) / 2] - fsvalues[len(fs) / 2]),
            )
            ...
            integral = (
                integral - fsvalues_ana[0] / 2 - fsvalues_ana[len(xcoords) - 1] / 2
            )
    ...
    if verbose:
        print(
            "Ups, program ended unexpected. Are you sure you specified the correct time"
            " for the -v argument?"
        )
