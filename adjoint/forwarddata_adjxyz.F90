...
!    USA
#include "fdebug.h"
SUBROUTINE FORWARDDATA_ADJXYZ(X,Y,Z,NONODS,NPHASE,TOTELE,NLOC,NDGLNO,  &
     D3,ITSTIME,DIRNAME)
   use FLDebug
  IMPLICIT NONE
  ...
  CHARACTER(4098) FILENAME_ADJXYZ,FILENAME
  INTEGER I,K1,K2

    WRITE(FILENAME_ADJXYZ, 10) ITSTIME
10  FORMAT('/adjxyz_data',I5.5)
    ewrite(3,*) trim(DIRNAME)
    ewrite(3,*) trim(FILENAME_ADJXYZ)
    FILENAME=trim(DIRNAME)//trim(FILENAME_ADJXYZ)
...
END SUBROUTINE FORWARDDATA_ADJXYZ


SUBROUTINE FORWARDDATA_ADJSND(SNDGLN,FSNDID,NONODS,NPHASE,STOTEL,SNLOC,   &
                              D3,ITSTIME,DIRNAME)
  ...
  INTEGER ::ITSTIME
  CHARACTER(40) DIRNAME
  CHARACTER(4098) FILENAME_ADJFS,FILENAME
  INTEGER I,K1,K2


  WRITE(FILENAME_ADJFS, 10) ITSTIME
10 FORMAT('/adjfs_data',I5.5)
...
END SUBROUTINE FORWARDDATA_ADJSND
