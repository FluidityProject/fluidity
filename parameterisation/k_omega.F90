...
subroutine komega_momentum_diagnostics(state)          !A! called in Momentum_Diagnostic_Fields.F90
  ...
  !A!call komega_blending_functions(state, advdif=.false.)!A! blending functions for k-omega SST
...
subroutine komega_calculate_rhs(state)
  ...
  type(scalar_field), dimension(2) :: src_abs_terms !A! 3 to 2 since no buoyancy term in k-omega
