...
contains
  ...
  integer :: ntime, nits, nits_internal, ndpset, noit_dim, &
       nits_flux_lim_volfra, nits_flux_lim_comp

  real :: dt, patmos, p_ini, t_ini, t_beta, v_beta, t_theta, v_theta, u_theta, domain_length
  ...
  real, dimension( : ), allocatable :: den, satura, volfra, comp, t, p, cv_p, volfra_pore, cv_one
...
!!!!!!!!
  ...
  allocate( comp_absorb( cv_nonods * nphase, nphase, nphase ))
  ...
  ! This bit is hard-coded and need to be generalised - initialising
  ! COMP e SUF_COMP_BC to be under equilibrium conditions
  if( ncomp > 1 )then
     comp = 0.
     ...
     write(357, * ) 'comp :', iphase, icomp, comp(( icomp - 1 ) * nphase * cv_nonods + ( iphase - 1 ) * cv_nonods + 1 : &
          ( icomp - 1 ) * nphase * cv_nonods + iphase * cv_nonods )

     iphase = 2
     comp(( icomp - 1 ) * nphase * cv_nonods + ( iphase - 1 ) * cv_nonods + 1 : &
          ( icomp - 1 ) * nphase * cv_nonods + iphase * cv_nonods ) = &
          K_Comp( 2, 1, 2 ) / ( K_Comp( 2, 1, 2 ) + 1. )
     write(357, * ) 'comp :', iphase, icomp, comp(( icomp - 1 ) * nphase * cv_nonods + ( iphase - 1 ) * cv_nonods + 1 : &
          ( icomp - 1 ) * nphase * cv_nonods + iphase * cv_nonods )
     ...
     comp(( icomp - 1 ) * nphase * cv_nonods + ( iphase - 1 ) * cv_nonods + 1 : &
          ( icomp - 1 ) * nphase * cv_nonods + iphase * cv_nonods ) = &
          1. - K_Comp( 2, 1, 2 ) / ( K_Comp( 2, 1, 2 ) + 1. )

     suf_comp_bc = 0.
     suf_comp_bc( 1 ) = 1.
     suf_comp_bc( stotel * cv_snloc * ( iphase - 1 ) * icomp + 1 ) = 1.

     write(357, * ) 'comp :', iphase, icomp, comp(( icomp - 1 ) * nphase * cv_nonods + ( iphase - 1 ) * cv_nonods + 1 : &
          ( icomp - 1 ) * nphase * cv_nonods + iphase * cv_nonods )
     write(357, * ) 'suf_comp_bc :', stotel, cv_snloc, suf_comp_bc
