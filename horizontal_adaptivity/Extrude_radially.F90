...
module hadapt_extrude_radially
  ...
  subroutine extrude_radially(shell_mesh, option_path, out_mesh)
    ...
    real:: constant_sizing, depth, min_layer_frac
      ...
      do column=1, size(r_meshes)
        ...
        if (sizing_is_constant) then
          call compute_r_nodes(r_meshes(column), depth, node_val(shell_mesh, column), r_shell, &
            min_layer_frac, sizing=constant_sizing) ! Return r_shell, will be needed for working out face id's later
        else
          if (have_option(trim(option_path)//"/from_mesh/extrude/regions["//&
                                      int2str(r)//"]/sizing_function/list")) then
            call compute_r_nodes(r_meshes(column), depth, node_val(shell_mesh, column), r_shell, &
              min_layer_frac, sizing_vector=sizing_vector)
          else if (sigma_layers) then
            call compute_r_nodes(r_meshes(column), depth, node_val(shell_mesh, column), r_shell, &
              min_layer_frac, number_sigma_layers=number_sigma_layers)
          else
            call compute_r_nodes(r_meshes(column), depth, node_val(shell_mesh, column), r_shell, &
              min_layer_frac, sizing_function=sizing_function)
          end if
        end if
  ...
  subroutine compute_r_nodes(r_mesh, depth, xyz, r_shell, min_layer_frac, sizing, &
    ...
    do
      ...
      r=r - delta_r
      if (r<(r_shell-depth+min_layer_frac*delta_r)) exit
      node=node+1
