...
Discretising the diffusion term for discontinuous Galerkin and finite 
volume methods is a challenge since 
...
\begin{equation}
  \matC_{ij}=\int_\Omega \phi_i\grad\psi_j \label{pgmatrix}
\end{equation}
...
\begin{split}
  ...
  \matC_{ij}=\int_\Omega \phi_i\grad\psi_j, \quad
...
gradient matrix as
\begin{equation*}
  \matC_{ij}=\int_\Omega \phi_i\grad\psi_j 
...
one is used to solve for a preliminary $\dvec\bmu_*^{n+1}$:
...
$\dvec\bmu^{n+1}$ after the pressure correction:
...
Note that $A$ depends on $\dvec\bmu$ itself. There are two possibilities when 
to evaluate $A$, either $A(\dvec\bmu^n)$ or at the latest velocity in the 
...
$A\left(\dvec\bmu^{n+\thetanl}\right)\dvec\bmu^{n+\theta}$.
...
\begin{split}
  ...
  \dvec\bmu^{n+1} &= \dvec\bmu_*^{n+1}
  -\Delta t  M^{-1} \matC \Delta\dvec p
\end{split}
\end{equation*}
Note that by construction the obtained $\dvec\bmu^{n+1}$ and 
...
$\dvec p^{n+\tfrac 12}$ and $\dvec\bmu^{n+1}$ from the previous 
...
matrix, so in general we do not want to explicitly construct this. A way to
avoid this is to use the lumped mass matrix $\mat{M}_L$
as an approximation
\begin{equation*}
