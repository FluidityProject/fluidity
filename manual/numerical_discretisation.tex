...
\section{Algorithm for Particle Advection (Lagrangian Trajectories)}
\label{sec:lagrangian_trajectories}
\index{Lagrangian trajectories} \index{detectors!Lagrangian}
Static Detectors can be set in the code at specific positions where the user wants
to know the values of certain variables at each time step. They are virtual
probes in the simulation that are fixed in space.

Alternatively, particles, which move with the flow and track specific attributes
through time, can also be included. The configuration of (static) detectors and (lagrangian) particles in
\fluidity\ is detailed in chapter
\ref{chap:configuration}. This section summarises the method
employed in the calculation of the new position of each particle as it moves with the flow.

Particles are advected using an explicit Runge-Kutta method, defined as
...
is used to track the particle across elements and, in parallel runs, across partitions. 
This method allows a particle leaving an element to be traced without resorting to an iterative procedure,
and is based on the observation that each stage of the Runge-Kutta scheme can be considered as a linear substep.

Starting from an initial position P in Figure~\ref{fig:guided_search_menthod}, a linear step in physical space would take the particle to position Q. 
...
In general, the particle will have left the element. However, we can determine the new containing element by inspecting the local coodinates of the arrival point 
...
The final position T of the particle can thus be evaluated with a fixed number of substeps. 

\begin{figure}[ht]
  \centering
  \xfig{numerical_discretisation_images/guided_search_method}
  \caption{A sketch representing the Guided Search method used in combination with an explicit Runge-Kutta algorithm to advect particles with the flow.}
  \label{fig:guided_search_menthod}
\end{figure}

\section{Utilizing Particles for Multi-Material Simulations}
\label{sec:particles_multi_material}

Particle attributes can be interpolated onto a Eulerian mesh, allowing modelling of multi-material simulations based on particles. To do this, particles are initialized with attribute values of 1 or 0 (where a value of 1 represents one material, and a value of 0 represents another material). These attributes are then transferred onto a dual control-volume mesh through a weighted distance interpolation algorithm. The nodes of the mesh then record the ratio of these attribute values, as:

\begin{equation}
P_{ratio}=\frac{\sum P_{value}}{N_{particles}},
\end{equation}
\noindent where $P_{value}$ represents individual attribute values transferred to a node, $N_{particles}$ represents the total number of particles transferring attributes to the same node, and $P_{ratio}$ represents the ratio value for that node, which determines the ratio of one material property relative to another. This allows particle attributes to determine various properties of a material, such as density and/or viscosity.
