...
\paragraph{First Order (W)ENO} \label{sec:foweno}

As described previously in section \ref{sec:ND_hermite_weno_limiter} above, the ENO and WENO methods generate local higher order reconstructions consistent with the volumetric data over a restricted domain, then preferentially choose the smoothest, either alone (ENO) or through a weighted sumation (WENO). In this case we apply the method for control volumes dual to a linear continuous mesh by calculating reconstructions matching the volumes which share an element. This regenerates a linear function within each control volume. The actual face value to be used is found by evaluating the linear function for the donor cell at the facet quadrature point. For more information on ENO methods see the works of \cite{Harten1987c} among others.
