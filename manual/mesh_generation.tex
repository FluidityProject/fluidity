...
\section{Extruded meshes}\label{sect:extruded_meshes}
\index{mesh!extruded}
...
// first extrude the surface
//y
Point(1) = {origin_x,origin_y,0,1};
Extrude {0,domain_y,0} {
  Point{1}; Layers{ny};
}

//z
Extrude {0,0,domain_z,1} {
 Line{1}; Layers{nz};
}

//x
Extrude {domain_x,0,0,1} {
  Surface{5}; Layers{nx};
}

// Now delete all the middle entities
Delete {
  Volume{1};
  Surface{26, 18, 22, 14};
  Line{21, 12, 13, 17};
...
//Add what ever you would like in between
//define the surfaces and lines as before

// In 3D use the frontal algo to make sure that the periodic surface mesh is
// not modified when meshing the volume
Mesh.Algorithm3D = 4;

  \end{lstlisting}
  \caption{An example of how to produce a 3D periodic channel with a complicated middle section}
\end{example}
...
\subsection{fldecomp}
This function is used to decompose a mesh into several parts in order to run in parallel.  For more information, see \ref{decomp_meshes_parallel}.
...
\end{lstlisting}
where BASENAME is the triangle mesh base name (excluding extensions). This will create PARTS partition triangle meshes together with PARTS .halo files. 
