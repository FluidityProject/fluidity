...
triangle files. The second, newer and more convenient but still experimental
way is to directly load the Gmsh file into \fluidity. To enable this,
...
used - see section \ref{mesh!meshing tools!fldecomp}\ for details. \lstinline[language=bash]+periodise+
does not yet correctly process Gmsh files so if you need to run
\lstinline[language=bash]+periodise+ you will have to convert the Gmsh file
to triangle format first.
...
This script converts Gmsh mesh files into triangle format. 
% Its use is now
