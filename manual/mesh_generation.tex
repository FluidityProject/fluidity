...
(\url{http://geuz.org/gmsh/}), as well as being supplied with
...
\begin{itemize}
\item \lstinline+--dx+  constant interval spacing
\item \lstinline+--variable_dx+  interval spacing defined with a python function
\item \lstinline+--region_ids+ python function defining the region ID at each point
\item \lstinline+--reverse+ reverse order of mesh
\end{itemize}
...
\lstinline[language=Bash]{gmsh2triangle_large} is used via:
...
The \lstinline[language = Bash]+--2d+ flag can be used to instruct \lstinline+gmsh2triangle_large+
...
Where foo is the triangle file base name (\ie \lstinline+foo.node+ etc.)
...
%Running a periodic mesh in parallel is relatively straight forward. To begin with, fluidity must be compiled with Zoltan. To check if the Zoltan libraries are installed type:

%\begin{lstlisting}[language=bash]
%dpkg -l libzoltan*
%\end{lstlisting}

%If the libraries are not present install them with:
...
of steps are required in order to provide flredecomp (required to make the
