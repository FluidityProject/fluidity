...
\begin{center}
  ...
  clean\_mayavi\_mesh       & \ref{sec:clean_mayavi_mesh} \\
	...
	encode                  		& \ref{sec:encode} 	        	\\
        ...
        flboundadapt			        & \ref{sec:flboundadapt}		\\
...
\subsubsection{flboundadapt}
\label{sec:flboundadapt}
flbounadapt (only available if \fluidity is compiled with libgmsh support) performs a remeshing of an input geometry based on an input options file (which may be a checkpoint) and outputs the resulting mesh. It is run from the command line:

\begin{lstlisting}[language = Bash]
flboundadapt [options] INPUT MESH OUTPUT
\end{lstlisting}
where \lstinline[language = Bash]+INPUT+ is the name of the input options file, \lstinline[language = Bash]+MESH+ is the name of the input Gmsh geometry (without the \lstinline[language = Bash]+.geo+ suffix) and \lstinline[language = Bash]+OUPUT+ is the name of the generated mesh. The flag \lstinline[language = Bash]+-v+ flag enables verbose output and the flag \lstinline[language = Bash]+-h+ flag displays the help message.
