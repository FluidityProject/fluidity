...
\fluidity\ is a very actively developed code. The code is distributed
via Subversion (SVN). If you are not familiar with subversion you may be
...
There are three repositories of \fluidity: release, stable and trunk. The release version
is aimed at general users of \fluidity. It is the least frequently updated, does not 
contain the all of latest features, but is very well tested. The manual and other 
resources are also set up to support this version. Expect updates to this to be once or twice per year. 
The stable tag is well tested and has passed all available tests. It will contain 
most of the latest features and expect it to be updated every 2-3 weeks. The manual
may not be completely up-to-date and there may be bugs and errors. The trunk is the latest, cutting-edge
version and is used by developers. This is updated very frequently; expect updates
to this several times per day and this version may not pass all tests.
...
discussion so you can better understand it:
...
Note the \lstinline[language=Bash]+-q+ will not list files that are in the directory
but not in the repository - configuring and compiling produce a lot of files!

To get specific information on the difference between a local file and
the central repository use the following:
\begin{lstlisting}[language=Bash]
svn diff a_dir/a_file
\end{lstlisting}
...
Once \fluidity\ has been built and run, result files will be generated, that will need visualising
