...
\subsubsection{Buoyancy and hydrostacy}
\index{density!reference}
...
\end{equation}
Here, we have separated the pressure $p_0$, due to the weight of a fluid of reference density $\rho_0$ below a reference level at $z=0$ near the top, from a perturbation pressure $p'$. This
perturbation pressure still contains the hydrostatic pressure due to the
perturbation density, but also any external pressure applied at the top, the
additional weight of $\rho g\eta$ in places where the top of the domain $\eta$
does not coincide with $z=0$ (e.g. in the case of a free surface), and any
non-hydrostatic parts of the pressure.
...
$\rho'\bmg$. Note, that this step does not require any assumptions about what
sort of boundary condition is applied at the top.
...
coordinates, its gradient does not simply balance with the buoyancy, and a
horizontal gradient $\rho_0g\nabla_{\text{H}}\eta$ remains. In \fluidity{}
