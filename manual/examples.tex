...
\subsection{Overview}
This examples demonstrates the capabilities of simulating wetting and drying processes in \fluidity\ .
...
This earthquake reached a magnitude of 7.8 (Mw) and the resulting tsunami hit a lightly populated part of the Okushiri island, Japan with a runup height of up to 30m.

To investigate the danger of such events, the Research Institute for Electric Power Industry (CRIEPI) in Abiko, Japan constructed a $1/400$ laboratory model of the area around the island \cite{androsov2008tsunami}. 
The following configuration simulates this laboratory setup and uses the experimental measurements to benchmark \fluidity\ .

\subsection{Configuration}
The configuration resembles the experimental setup as close as possible. 
The considered domain is a basin with walls on the sides but the left where the water level can be enforced. 
The basin measures $5.448\mbox{m} \times 3.402\mbox{m}$ and the bathymetry and coastal topography closely correspond to measurement data, see Figure.
...
The mesh used for the simulation is vertically aligned, one element deep and horizontally unstructured mesh consisting of $19,464$ tetrahedral elements with increased resolution near the inundation areas, see Figure \ref{fic:monai_inputwave} (b). 
...
The plot shows the measurements of the laboratory experiment compared to the values from the simulation.
  at the gauge station plotted against the laboratory measurements are shown in Figure \ref{fic:monai_results} (b). 
\begin{figure}
\begin{center}
...
\item Change the viscosity parameter. If we had honey oceans would that makes any difference to the danger of tsunamis events?
\item Remove the fourth order stabilization (active the "remove\_stabilisation\_term" in /material\_phase::water/scalar\_field::Pressure/prognostic/reference\_node). 
      The simulation will run for some while and the terminate. If you look at the results, can you figure out what the reason is why blows up? 
      (Hint: In \fluidity\ the water elevation is a function from the horizontal position to the height, i.e. in a plane each x, y coordinate has a water elevation values assigned to it)
\end{enumerate}
