...
\begin{tabular}{|l|l|c|l|}
  ...
  \ref{sect:tephra_settling} Tephra settling & \texttt{tephra\_settling} & 4 & ? hr. \\
  \hline
\end{tabular}
...
\subsection{Overview}
At the bottom of the world's oceans lies layer after layer of ash deposited from past volcanic eruptions. This process of deposition is important because it forms a timeline from which scientists can learn more about the timing and frequency of such significant geophysical events. Ash (or tephra) descending through the ocean is predicted to travel at a terminal velocity given by Stokes' law because of the viscous drag acting on each individual particle, and typical deposition times are therefore expected to be anywhere from one day to one year (depending on particle size) for ocean depths between 1000 and 5000 metres \citep{carey1997}. However, this does not agree with field observations, where tephra layers seen at the bottom of the ocean represent almost instantaneous geophysical events. In other words, the main factor influencing tephra deposition patterns and behaviour is the atmospheric conditions; the settling through the ocean happens relatively quickly in comparison.

\cite{carey1997} presented a new model which uses the idea of convective sedimentation to explain the rapid acceleration that particles undergo when settling through the ocean. Essentially, small tephra particles settling in a body of water can either do so individually, or collectively as a plume. These plumes are generated when the bulk density of the tephra-water mixture becomes large enough, yielding settling velocities much greater than those expected of single particles which settle at the predicted Stokes velocity. \cite{carey1997} performed several experiments to illustrate this, and a numerical model based upon these is given in \cite{goldin2008} which extends the popular \texttt{K-FIX} two-phase flow code to run simulations of a continuous liquid phase with immersed solid particles.

In this example, \fluidity\ is used to replicate the conditions under which plumes of tephra particles form as well as the settling velocities of the individual particles and plumes. The example demonstrates the following functionality:
\begin{itemize}
\item 2D flow
...
The simulation illustrated here took $\sim$1 hour to run in parallel over four processes on two (dual-core) Intel Xeon E5430 2.66 GHz processors.

\subsection{Problem specification}
The simulation uses a 0.3 x 0.7 metre domain, replicating the cross-section of the water tank used in the experiments, with an initial characteristic element size of 0.01 metres. No normal flow boundary conditions are imposed along with a zero velocity initial condition. The parameters for density, viscosity, particle diameter and gravity are $\rho_p = 2340\ \mathrm{kgm^{-3}}$, $\rho_l = 1000\ \mathrm{kgm^{-3}}$, $\mu_l = 0.001\ \mathrm{Pa\cdot s}$, $d = 48\times 10^{-6}\ \mathrm{m}$, and $\mathbf{g} = 9.8\ \mathrm{ms^{-2}}$ respectively (with the subscripts $p$ and $l$ denoting the properties of the particle and liquid phase).

The experiments by Carey introduced tephra particles into the tank from above using a delivery system and a particle disperser; eventually, a layer of tephra formed on top of the water. This is represented here using an inflow boundary condition at the top of the domain to allow tephra to flux in at a rate of $0.000472\ \mathrm{kgm^{-2}s^{-1}}$.

The results of the experiments showed that tephra particles initially settled at an average Stokes velocity of $0.2\ \mathrm{cms^{-1}}$. When the concentration was high enough, plumes formed and travelled to the bottom of the tank at an average velocity of $2\ \mathrm{cms^{-1}}$ --- 10 times greater than that of single particles.

\subsection{Results}
Several timesteps of the example simulation (when run on a mesh with characteristic element size $\Delta x = 0.0025\ \mathrm{m}$) can be seen in Figure \ref{fig:tephra_settling}. Similar images can be generated by visualising the vtu files using Paraview.
...
\begin{figure}[H]
   ...
   \caption{Results from the \texttt{tephra\_settling} example at time $t = 10, 30, 50, 80, 110$ seconds. }
   \label{fig:tephra_settling}
\end{figure}

The following graph (reproducible by typing \textit{make postprocess} at the command line) illustrates how the tephra particles initially settle at approximately $0.0017\ \mathrm{ms^{-1}}$, as predicted by Stokes' law. As more tephra fluxes in, the layer becomes unstable and plumes begin to form, resulting in settling velocities over 10 times greater than that of an individual particle.

\begin{figure}[H]
	\centering
	      \includegraphics[width=0.5\columnwidth]{./examples_images/tephra_settling/tephra_velocity.pdf}
