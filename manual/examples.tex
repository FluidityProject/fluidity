...
\subsection{Overview}
Thigs examples demonstrates the capabilities of simulating wetting and drying processes in \fluidity\ .
...
\subsection{Configuration}
The simulation configuration resembles the experimental setup as close as possible. 
The considered domain is a basin with walls on the sides but the left where the water level can be enforced. 
The basin measures $5.448\mbox{m} \times 3.402\mbox{m}$ and the bathymetry and coastal topography closely correspond to measurement data, see Figure \ref{fig:monai_inputwave}.
...
The mesh used for the simulation is vertically aligned, one element deep and horizontally unstructured mesh consisting of $111,513$ tetrahedral elements with increased resolution near the inundation areas.
...
To prevent wave breaking in the simulation, this coefficient is increased to $0.2\mbox{sm}^{-\frac{1}{3}}$ in a rectangular area with a side length of $0.5$m centred at $3.4$m $\times 1.7$m (which is the centre of the island in the domain) and a fourth order stabilization is applied. 
...
\item Remove the fourth order stabilization (active the "remove\_stabilisation\_term" in /material\_phase::water/scalar\_field::Pressure/prognostic/reference\_node). 
      The simulation will run for some while and the terminate. If you look at the results, can you figure out the reason why it blows up? 
      (Hint: In \fluidity\ the water elevation is a function of the horizontal positions mapping to the height, i.e. in a plane each $x$, $y$ coordinate has a water elevation value $z$ assigned to it)
\end{enumerate}
