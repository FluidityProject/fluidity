...
\subsection{Problem specification}
The simulation uses a 0.3 x 0.7 metre domain, replicating the cross-section of the water tank used in the experiments, with an initial characteristic element length of 0.01 metres. No normal flow boundary conditions are imposed along with a zero velocity initial condition. The parameters for density, viscosity, particle diameter and gravity are $\rho_p = 2340\ \mathrm{kg/m^3}$, $\rho_l = 1000\ \mathrm{kg/m^3}$, $\mu_l = 0.001\ \mathrm{Pa\cdot s}$, $d = 48\times 10^{-6}\ \mathrm{m}$, and $\mathbf{g} = 9.8\ \mathrm{m/s^2}$ respectively (with the subscripts p and l denoting the properties of the particle and liquid phase).

The experiments by Carey introduced tephra particles into the tank from above using a delivery system and a particle disperser; eventually, a layer of tephra formed on top of the water. This is represented using a Neumann boundary condition at the top of the domain to allow tephra to flux in at a rate of $0.000472\ \mathrm{kg/m^2/s}$.

The results of the experiments showed that tephra particles initially settled at an average Stokes velocity of 0.2 cm/s. When the concentration was high enough, plumes formed and travelled to the bottom of the tank at an average velocity of 2 cm/s --- 10 times greater than that of single particles.
