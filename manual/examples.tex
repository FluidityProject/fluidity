...
\begin{figure}[ht]
  \centering
  \pdffig[width=1.0\textwidth]{./examples_images/lock_exchange/front_speed}
...
\begin{itemize}
\item Run from the checkpoint to look at the Froude number later in the simulation and calculate an average value, section \ref{sect:configuring_fluidity_checkpointing} (don't forget to rename the vtus with rename\_checkpoint.py, table \ref{tab:fltools});
\item Run from the checkpoint to look at mixing later in the simulation, section \ref{sect:configuring_fluidity_checkpointing};
