...
\begin{equation}
\overline{\alpha} = \int_{-\infty}^\infty G_m\left( \overrightarrow{r} \right) \alpha \left( \overrightarrow{x} - \overrightarrow{r} \right) \text{d} \overrightarrow{r}
\label{eqn:les_filtering}
\end{equation}
and
\begin{equation}
\alpha = \overline{\alpha} + \alpha^\prime
\label{eqn:les_decomposition}
\end{equation}
where $\alpha^\prime$ denotes the subgrid-scale fluctuation. Applying
...
\end{equation}
\par
In comparison with the unfiltered Navier-Stokes equations, equations \eqref{eqn:filtered_momentum}
feature an additional term. This is reflected in the options that must be adjusted in order to
perform large eddy simulations using \fluidity. In particular, the user must activate the option
under spud path \lstinline+.../spatial_discretisation/continuous_galerkin/les_model+. Further 
options under this path allow the user to select a particular subgrid scale model and configure it.
See chapter \ref{chap:configuration} for how to configure \fluidity\ options.
...
\end{equation}
where $C_s$ is the \emph{Smagorinsky constant} and $\left | \mathcal{\overline{S}} \right |$ is the local strain
...
\subsubsection{Fourth-Order Dissipation}
