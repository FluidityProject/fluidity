...
The fourth term on the right, containing the Reynolds stress tensor $-\rho\mathbf{u}'\cdot\mathbf{u}'$, represents the effect of turbulent fluctuations on the steady flow and is modelled as:

\begin{equation}\label{retens}
 -\rho\mathbf{u}'\cdot\mathbf{u}' = \nu_T \left(\nabla\mathbf{u} + (\nabla\mathbf{u})^{\mathrm{T}}\right) - \frac{2}{3} k,
\end{equation}
