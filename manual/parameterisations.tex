...
The model is valid for high Reynolds numbers, in which $\nu_T >> \nu$, although it may be used for lower Reynolds number with the following boundary conditions:

\begin{eqnarray}
\bmu &=& 0, \slabel{ubc}\\
k &=& 0, \slabel{kbc}\\
\epsilon &=& \nu \ppxx[y]{k} = 2 \nu \left ( \ppx[y]{k^{1/2}} \right ). \slabel{epsbc}\\
\end{eqnarray}

In the viscous sublayer of the boundary layer this is not valid, and so boundary conditions are important. 
