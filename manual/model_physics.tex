...
This chapter covers the underlying physics of the Navier Stokes and associated field equations modelled by {\fluidity}.
...
\end{equation}
Note that the second-order terms defined above is often termed Laplacian diffusion.
...
Co-oscillating tides are forced as cosine waves of specified phase and amplitude along designated boundaries:
\begin{equation}
...
\end{subeqnarray}
where $\tote\equiv\inte+\modu^2/2$ is the total specific energy. \eqref{conmass} is exactly the conservative form of the continuity equation given in \eqref{mass_conservation_2}, \eqref{conmom} is equation \eqref{viscous_fluids_1} and \eqref{conenergy} is obtained from making the substitutions $w=\inte+p/\rho$ and $\tote\equiv\inte+\modu^2/2$ in \eqref{viscous_fluids_1}.
...
\subsection{Astronomical tides}\label{Sect:AST}
\index{astronomical tides}
\label{astronomical}
...
The astonomical tidal potential can be modified to account for the deformation of the solid earth
