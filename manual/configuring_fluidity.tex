...
\subsubsection{Mesh properties}

\begin{itemize}
\item Mesh shape
\item Mesh Continuity
...
\begin{description}
\item[Bassi Rebay] The classical scheme of Bassi and Rebay (see section \ref{BassiRebay}).
\item[Compact DG] The compact discontinuous Galerkin scheme from Peraire and Persson (\cite{peraire2008}).
...
The momentum equation can be discretised conservatively by setting the BETA factor equal to 1 (corresponding to a divergence form of the equation). If BETA is set to zero, the discretisation is left non-conservative. An intermediate value can alternatively be selected. Please refer to section \ref{Sect:ND_momentum_equation} for a comprehensive discussion on the influence of this parameter.
...
to a \option{diagnostic/algorithm::Internal}-type field. The list of fields to switch is:
\begin{enumerate}
...
$k-\epsilon$ option in the GLS model (see \ref{Sect:GLS}) which is only for oceans. There are only 2 sub-options,
both of which are recommended to be on:
\option{\ldots/calculate\_boundaries} and \option{\ldots/limit\_lengthscale}.
...
Fully implicit temporal discretisation (\option{\ldots/temporal\_discretisation/theta=1}) \linebreak
 is recommended. 

Finally, a fields that are altered by the k-epsilon model, such as the Viscosity need to be switched
to a \option{diagnostic/algorithm::Internal}-type field. The list of fields to switch is:
\begin{enumerate}
...
tensor field, and as a diagnostic scalar field (e.g. for plotting using statplot). However, the ordinary viscosity must still be prescribed under \linebreak
...
The model is designed for low Reynolds number flows ($\nu_T \approx \nu$) with the following Dirichlet boundary conditions:
...
However, it has been tested successfully for higher Reynolds numbers: $\nu_T >> \nu$.
$k$ should in practice be set to a very small nonzero number to prevent the code crashing.
\eqref{epsbc} can be implemented by selecting the following boundary condition type: \linebreak
\option{\ldots/TurbulentDissipation/prognostic/boundary\_conditions/k\_epsilon}.
...
A Crank-Nicolson temporal discretisation with a non-linear relaxation is recommended for the temporal discretisation [REF SECTION]. This is selected by setting:
\begin{itemize}
...
\item \option{\ldots/spatial\_discretisation/conservative\_advection} set to 0 (non-conservative), [REF SECTION]
\end{itemize}
...
For the spatial discretisation a control-volumes discretisation with a finite-element face value discretisation and Sweby limiter are recommended which are selected with the options [REF SECTIONS]:
\begin{itemize}
\item{\ldots/spatial\_discretisation/control\_volumes/face\_value::FiniteElement}
...
\begin{itemize}
\item{\ldots/prognostic/spatial\_discretisation/control\_volumes/face\_value::FiniteElement/limit\_face\_value/limiter::Sweby/project\_upwind\_value\_from\_point/store\_upwind\_elements}
\end{itemize}
...
For the temporal discretisation a Crank-Nicolson scheme is recommended, with the control volume options of 3 advection iterations and limit theta [REF SECTIONS]. These are set with the options:
\begin{itemize}
\item{\ldots/temporal\_discretisation/theta} set to 0.5
\item{\ldots/temporal\_discretisation/control\_volumes/number\_advection\_iterations}
\item{\ldots/prognostic/temporal\_discretisation/control\_volumes/limit\_theta}
\end{itemize}
...
\begin{itemize}
\item{\ldots/spatial\_discretisation/discontinuous\_galerkin/advection\_scheme/lax\_friedrichs}, cf. [REF SECTION]
\item{\ldots/spatial\_discretisation/discontinuous\_galerkin/advection\_scheme/project\_velocity\_to\_continuous/mesh::CoordinateMesh}, cf. [REF SECTION]
\item{\ldots/spatial\_discretisation/discontinuous\_galerkin/advection\_scheme/integrate\_advection\_by\_parts/once}, cf. [REF SECTION]
\item{\ldots/spatial\_discretisation/discontinuous\_galerkin/diffusion\_scheme/compact\_discontinuous\_galerkin}, cf. \ref{sec:NM_DG_diffusion}
\item{\ldots/spatial\_discretisation/discontinuous\_galerkin/slope\_limiter::Vertex\_Based}, cf. \ref{Sect:ND_DG_slope_limiters}
\end{itemize}

For the temporal discretisation a Crank-Nicolson scheme with subcycling is recommended. This can be set with:
\begin{itemize}
\item{\ldots/temporal\_discretisation/theta} set to 0.5
\item{\ldots/temporal\_discretisation/discontinuous\_galerkin/maximum\_courant\_number\_per\_subcycle} set to an appropriate value.
\end{itemize}
...
\section{Industrial CFD Problems}
\label{Sect:industrial_CFD}
