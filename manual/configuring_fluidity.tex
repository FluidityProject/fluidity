...
Finally, to transport a scalar field (for example \option{Tracer}) through the porous media using the Darcy velocity requires:
\begin{itemize}
\item the \option{Tracer} field to be discretised with control volumes via the option \option{\ldots/scalar\_field::Tracer/prognostic/spatial\_discretisation/control\_volumes},
\item the \option{Tracer} field porosity field name to use set via the option \option{\ldots/scalar\_field::Tracer/prognostic/spatial\_discretisation/control\_volumes/porosity::Porosity},
\item the \option{ControlVolumeCFLNumber} field porosity field name to use set via the option \option{\ldots/scalar\_field::ControlVolumeCFLNumber/diagnostic/porosity::Porosity},
\item the metric advection porosity field name to use set via the option \option{/mesh\_adaptivity/hr\_adaptivity/metric\_advection/spatial\_discretisation/control\_volumes/porosity::Porosity},
