...
symmetric in cases where it should be. \website
...
\option{\ldots/equation\_of\_state/compressible/miegrunneisen} defines a simple compressible equation of state that can be used to describe gases, liquids and solids, known as the Mie-Gruneisen (or stiffened gas) EOS (see section~\ref{Sect:StiffenedGas}).
...
For both P1-P1 and P1DG-P2
\begin{itemize}
\item \option{\ldots/spatial\_discretisation/continuous\_galerkin} is recommended for the spatial discretisation. 
\item \option{\ldots/scheme/poisson\_pressure\_solution} is recommended to be chosen as either \option{never} or \option{only\_first\_timestep}, [REF SECTION]
\end{itemize}
...
The temporal discretisation recommendations are:
\begin{itemize}
\item Crank-Nicolson, [REF SECTION]: \\
\option{\ldots/temporal\_discretisation/theta} set to 0.5
\item with a non-linear relaxation, [REF SECTION]: \\
\option{\ldots/temporal\_discretisation/relaxation} to 0.5
\end{itemize}

If using P1-P1 the spatial discretisation recommendations are:
\begin{itemize}
\item no stabilisation, cf. \ref{Sect:ND_advective_stabilisation_CG}: \\ 
\option{\ldots/spatial\_discretisation/continuous\_galerkin/stabilisation/no\_stabilisation}
\item lumping the mass matrix, cf. \ref{Sect:ND_cg_mass_lumping}: \\
\option{\ldots/spatial\_discretisation/continuous\_galerkin/mass\_terms/lump\_mass\_matrix}
\item and non-conservative advection, [REF SECTION]: \\ 
\option{\ldots/spatial\_discretisation/conservative\_advection} set to 0
\end{itemize}

If using P1DG-P2, the spatial discretisation recommendations are:
\begin{itemize}
\item a compact discontinuous galerkin viscosity scheme, cf. \ref{sec:NM_DG_diffusion}: \\
\option{\ldots/spatial\_discretisation/discontinuous\_galerkin/viscosity\_scheme/compact\_discontinuous\_galerkin}
\item an upwinding advection scheme, cf. \ref{Sect:ND_discontinuous_galerkin_advection}: \\
\option{\ldots/spatial\_discretisation/discontinuous\_galerkin/advection\_scheme/upwind}
\item advection is integrated by parts twice, cf. \ref{Sect:ND_discontinuous_galerkin_advection}: \\
\option{\ldots/spatial\_discretisation/discontinuous\_galerkin/advection\_scheme/integrate\_advection\_by\_parts/twice}
\item and non-conservative advection, [REF SECTION]: \\ 
\option{\ldots/spatial\_discretisation/conservative\_advection} set to 0
\end{itemize}
...
For the spatial discretisation a control-volumes discretisation is recommended with the following options [REF SECTIONS]:
\begin{itemize}
\item a finite-element face value discretisation: \\
\option{\ldots/spatial\_discretisation/control\_volumes/face\_value::FiniteElement}
\item with Sweby limiter: \\
\option{\ldots/prognostic/spatial\_discretisation/control\_volumes/face\_value::FiniteElement/limit\_face\_value/limiter::Sweby}
\item and an Element-Gradient diffusion scheme: \\
\option{\ldots/spatial\_discretisation/control\_volumes/diffusion\_scheme::ElementGradient}
\end{itemize}
To help increase speed it is possible to store the upwind elements so they do not have to be recalculated every time step (only after adapts) [REF SECTION]. This is enable with the option:
\begin{itemize}
...
\item Crank-Nicolson, [REF SECTION]: \\
\option{\ldots/temporal\_discretisation/theta} set to 0.5
\item with 3 control-volume advection iterations, [REF SECTION]: \\ 
\option{\ldots/temporal\_discretisation/control\_volumes/number\_advection\_iterations}
\item and limit theta enabled, [REF SECTION]: \\
\option{\ldots/prognostic/temporal\_discretisation/control\_volumes/limit\_theta}
\end{itemize}
...
The spatial discretisation recommendations are:
\begin{itemize}
\item a Lax-Friedrichs advection scheme, cf. [REF SECTION]: \\
\option{\ldots/spatial\_discretisation/discontinuous\_galerkin/advection\_scheme/lax\_friedrichs}
\item velocity projected to continuous space (the CoordinateMesh for example), cf. [REF SECTION]: \\
\option{\ldots/spatial\_discretisation/discontinuous\_galerkin/advection\_scheme/project\_velocity\_to\_continuous/mesh::CoordinateMesh}
\item advection is integrated by parts once, cf. \ref{Sect:ND_discontinuous_galerkin_advection} [REF SECTION CHECK]: \\
\option{\ldots/spatial\_discretisation/discontinuous\_galerkin/advection\_scheme/integrate\_advection\_by\_parts/once}
\item a compact-discontinuous-Galerkin diffusion scheme, cf. \ref{sec:NM_DG_diffusion}: \\
\option{\ldots/spatial\_discretisation/discontinuous\_galerkin/diffusion\_scheme/compact\_discontinuous\_galerkin}
\item with a vertex-based slope limiter, cf. \ref{Sect:ND_DG_slope_limiters}: \\
\option{\ldots/spatial\_discretisation/discontinuous\_galerkin/slope\_limiter::Vertex\_Based}
\end{itemize}
...
\item Crank-Nicolson [REF SECTION]: \\
\option{\ldots/temporal\_discretisation/theta} set to 0.5
\item with subcycling [REF SECTION]: \\
\option{\ldots/temporal\_discretisation/discontinuous\_galerkin/maximum\_courant\_number\_per\_subcycle} set to an appropriate value.
\end{itemize}


\subsubsection{Geostrophic Pressure}
\label{Sect:GFD_config_geopressure}
If enabled a `geopressure' solver is used, \ref{Sect:config_geostrophic_balance}. The specified mesh should be the \option{GeostrophicPressureMesh}, cf. \ref{sect:GFD_config_geometry}. The terms included in the right-hand side of the geopressure solver are selected under the option \option{\ldots/prognostic/spatial\_discretisation/geostrophic\_pressure\_option}.
...
compressible EOS: Mie-Grunneisen (see section \ref{Sect:StiffenedGas}).
...
\begin{itemize}
\item \option{\ldots/spatial\_discretisation/\ldots/lump\_mass\_matrix} should be turned on.
\item For a Continuous Galerkin discretisation in order to couple the velocity components together \\* \option{\ldots/spatial\_discretisation/continuous\_galerkin/stress\_terms/stress\_form} is required.
\end{itemize}
