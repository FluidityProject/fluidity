...
If particles are selected, the user must specify \option{/particle\_group}, each of which can contain multiple \option{/particle\_subgroup}. Particle groups also define particle IO options, such as \option{/dump\_period}, \option{/cpu\_dump\_period} and \option{/wall\_time\_dump\_period}. Particle spawning and deleting options are also set per particle group, under the option \option{/particle\_spawning}. Each subgroup within a particle group will be governed by the same specified set of parameters. Particle subgroups define an array of particles, each of which is initialized with a specified \option{/number\_of\_particles} and \option{/initial\_position}. Particle attributes are defined per particle subgroup under the option \option{/attributes}. These attributes allow users to calculate and track variables on particles and can be set either as a constant, from python, or from python\_fields, which incorporates any field values having the option \option{/include\_in\_particles} enabled. Old field values and old particle attributes can also be utilized under the python\_fields category, assuming the corresponding \option{/store\_old\_field} or \option{/store\_old\_attribute} option is enabled. Particle attributes can be set as either scalars, vectors or tensors, allowing for a wide variety of uses and applications. Particle attributes can be interpolated onto a (Eulerian) grid to generate a diagnostic field, through the diagnostic algorithm \option{/from\_particles}.

Particle spawning and deleting options are all defined under the \option{/particle\_spawning} option for each particle group. Here the mesh used for spawning and deleting must be specified, and the minimum and maximum thresholds for the number of particles per control volume can be set via the options \option{/min\_cv\_threshold} and \option{/max\_cv\_threshold} respectively. The location of spawned particles can also be specified under the option \option{/spawn\_location}, where particles can be spawned either within a user defined local coordinate \option{/radius} of the parent particle, ensuring the spawned particle is within the same element and control volume as the parent, or at \option{/random} within the control volume, prioritizing elements with a lower number of particles. Attribute values of spawned particles are calculated using one of two algorithms under the option \option{spawn\_attributes}. These algorithms either \option{/copy\_parent\_attributes} when spawning, or \option{/calculate\_average} attribute values across the control volume and assign them to spawned particles. Finally, the option \option{/subgroup\_spawning\_caps} can be used when it is desired to only spawn particles from the dominant subgroup in a control volume, and help maintain material conservation. The option \option{/percentage} under this is a user set parameter which specifies the percentage a subgroup must attain within a control volume in order for particles from that subgroup to spawn. As an example, if the percentage parameter is set to 99\%, a particle subgroup must make up at least 99\% of the particles in a control volume for spawning of the subgroup to occur (when the minimum particle threshold of that control volume is broken). If no subgroup meets this criteria, no spawning will occur in the control volume.
...
The option \option{/delete\_outside\_domain} will cause any particles found outside the domain boundaries to be deleted. Information for these particles will still be contained within the .h5part files for all previous timesteps.
