...
\end{equation}
The above drag force correlation was developed for spherical and solid dispersed phase but fluid dispersed particles with slight impurities can be modelled using the same. \cite{schiller1935drag} drag has been used for the modelling of dilute bubble columns successfully. 
...
Additionally, it is also possible to cap the dispersed phase diameter, which is useful when using a scalar field for the diameter. For instance, a very small diameter can result in a very large drag force which can result in the code to crash.
