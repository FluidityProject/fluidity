...
\begin{description}
\item[FirstOrderUpwind] - see section \ref{sec:cv_fou}.  Note that first order upwinding does not require nonlinear advection iterations as the low order pivot solution uses first order upwinding itself.  However in this case it is necessary that the implicitness factor, $\theta$, is the same as the pivot implicitness factor, $\theta_p$ (see sections \ref{sec:cvtemp} and \ref{sect:configuring_fluidity_temporal_discretisation}).
...
LES models are available as options under \option{\ldots/Velocity/prognostic/spatial\_discretisation/continuous\_galerkin/les\_model}. See \ref{Sect:LES} for details of the various LES models available. These models require a prescribed viscosity (\ldots/Velocity/prognostic/tensor\_field::Viscosity/prescribed), to which an eddy viscosity is added to account for subgrid-scale turbulence. The LES models are currently restricted to use in incompressible flow cases, where the discrete velocity is divergence-free and the eddy viscosity tensor is traceless.
...
\subsubsection{GLS sub-grid scale parameterisation}\label{Sect:BCs:special:gls}
...
\subsubsection{k-epsilon sub-grid scale parameterisation}\label{Sect:BCs:special:kepsilon}
...
The mesh that you use must be a two plus one mesh which is unstructured in the horizontal and structured in the vertical.  It can either be constructed in gmsh and read into fluidity, or a two-dimensional mesh can be made in gmsh and extruded within fluidity (see \ref{sect:extruded_meshes}). In addition, these mesh settings are recommended:
\begin{itemize}
...
global maximum number, but can be altered to be the number per process. The mesh is coarsened everywhere until this is acheived.
...
\begin{itemize}
\item minimum number of nodes - sets the minimum possible number of nodes in the domain. In parallel this is the
global minimum number. The mesh is refined until this is acheived.
\item adaptive timestep at adapt - used in conjuction with adaptive timestep (see section \ref{section:config_adaptive_timestep}), this
option resets the timestep back to the minumum value under \option{\ldots/adaptive\_timestep/minimum\_timestep} immediately follwing
a mesh adapt.
\item maximum node increase - The maximum ration by which the number of nodes is allowed to increase. A value of 1.1
indicates the number of nodes can increase by at most 10\%.
\item node locking - Allows the locking of nodes via a python function that cannot be moved by adaptivity.
\item metric advection - advect the metric along with the flow, ensuring the resolution can be pushed ahead of any flow, rather than lagging behind.
...
\item preserve mesh regions - ensures that regions in your mesh, specified by region IDs, are preserved through adaptivity.
\item vertically structured adaptivity - Also known as 2D+1 adaptivity, this option performs adaptivity in two stages - first the horizontal mesh
is adapted, then the mesh is extruded using the adaptivity metric in the 3rd dimension. You must use an extruded mesh with this option.
\item adaptivity library - choose which adaptivity library to use. In 2D you are restricted to libmba2d. In 3D you can choose either libmba3D or libadaptivity (default).
\item adapt iterations - this options controls the number of intermediate adapt iteration during parallel adaptive simulations. The default value is 3. Higher values may give you 
better meshes, especially when the number of elements per process is low.
\end{itemize}


\subsubsection{Zoltan options}
\label{sect:configuring_fluidity_zoltan_options}
\index{adaptivity options}
\index{zoltan}
