...
\begin{itemize}
\item The pressure term in the internal energy equation can be included by enabling the \option{include\_pressure\_term} option under the InternalEnergy field in Diamond.
\item The heat flux term can be included via the InternalEnergy's Diffusivity field. Users will need to specify an isotropic value for $\frac{k}{C_v}$, where $k$ is the effective conductivity and $C_v$ is the specific heat at constant volume.
\end{itemize}
