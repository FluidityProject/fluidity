...
recomennded discretisation options for such simulations. It is based on the use
...
smaller, regional or coastal domains. For the first case, see the additional
...
\ref{sec:extruded_meshes}). To enable the \PoDGPt element pair you will need
the two following additional meshes that are derived from the three-dimensional
CoordinateMesh:
\begin{itemize}
\item The VelocityMesh must be made discontinuous galerkin, by setting \option{/geometry/mesh(VelocityMesh)/from\_mesh/mesh\_shape/mesh\_continuity} to discontinuous.  
...
\option{vertical\_lumping} option for the \option{mg} preconditioner work.

\subsection{Time stepping}
\begin{itemize}
\item The timestep is typically chosen based on a knowledge of the time scales
  ...
  the timescale of the physical phenomena of interest in mind.
...
\item To ensure that the equations are indeed stable for large timesteps choose
  ...
  order to improve the temporal accuracy of the model.
...
\item To enable the last option you also need to add a
  \option{DG\_CourantNumber} diagnostic field under the
...
\item Under \option{\dots/solver}, select \option{cg} for the
  ...
  Under the \option{\ldots/preconditioner::mg} switch on
...
\option{/matertial\_phase} to be able to visualise the Density field (for
...
\begin{itemize}
  ...
  \item If the fields contain discontinuities (shocks or fronts) a
    \option{streamline\_upwind} can be chosen under
    \option{\ldots/spatial\_discretisation/stabilisation}.
...
Brunt-V\"ais\"al\"a frequency becomes a, very strict, limitation on the timestep. To
avoid this, add the option
\option{\ldots/vertical\_stabilization/implicity\_buoyancy} under the Velocity
field.
...
in section \ref{sec_spherical_earth}. Note, that when using
...
under-resolved. This is
achieved by having another mesh, typically called \option{BaseMesh}, that has the
...
\ref{sec:large-scale-oceans} should be followed. The option
...
\begin{itemize}
  \item Separate VelocityMesh and PressureMesh are not necessarry. Instead you
  ...
  \item Under Velocity select
    \option{\ldots/spatial\_discretisation/continuous\_galerkin}. Under that
    you have to have \option{mass\_terms/lump\_mass\_matrix}. You may need
    to add \option{stabilisation/streamline\_upwind} to stabilize non-smooth
    flows.
  \item The options for Pressure are the same as before.
\end{itemize}
...
To solve for the shallow water equations \eqref{swe} select equation type
...
and spatial discretisation options is mostly the same as for other geophysical
flow problems (see sections \ref{section:large-scale-ocean} and
\ref{sec:gfd_problems}). We recommend the \PoDGPt pressure element pair
