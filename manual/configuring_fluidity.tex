...
mesh: the CoordinateMesh. Some settings, fields or other settings have
...
\item \option{compact\_discontinous\_galerkin}: The compact
...
\item \option{interior\_penalty}: Symmetric interior penalty (IP)
  ...
  \option{use\_face\_integral} which computes a lengthscale from the
...
\option{\ldots/calculate\_boundaries} is recommended. Finally, you can enable a number of optional diagnostic fields.
...
Finally, fields that are altered by the GLS model, such as the Viscosity, should be switched
...
$k-\epsilon$ option in the GLS model (see \ref{Sect:GLS}) which is only for oceans. The user can choose to limit the length-scale
 near the wall, which is recommended: \option{\ldots/limit\_lengthscale}.
...
\begin{align}
source &= -min(0.0, diss-prod), \nonumber \\
absorption &= max(0.0, diss-prod), \nonumber \\
\end{align}
where $src$ is the explicitly calculated source term and $abs$ is the explicitly calculated absorption term. Please note, it is not recommended to use the implicit option unless the simulation is very unstable.
...
is recommended. The \option{control\_volume} discretisation option should be selected, with \linebreak
...
\option{\ldots/subgridscale\_parameterisation::k-epsilon} to use the eddy diffusivity calculated in the model. It should be noted that the eddy diffusivity and eddy viscosity are related by the Schmidt number for the kinetic energy field, \option{sigma\_k}, which is usually set to the value in Table \ref{tab:kepsco}.
...
The lengthscale can be limited near selected walls using the \option{limit\_lengthscale} option (see \eqref{yap}).
It is limited throughout the domain by the value set in \option{L\_max}, which should be the maximum possible size of an eddy (a geometric constraint, e.g. inlet height).
However, it has no effect on the solution since the length scale is purely diagnostic [pending revision].

\subsubsection{Initial and Boundary Conditions}[PENDING REVISION]
...
In the low Reynolds number case, \eqref{epsbc1} can be implemented by selecting the following boundary condition type: \option{\ldots/boundary\_conditions/k\_epsilon/wall\_functions/low\_Re}. \eqref{kbc1} is not currently set using this option, and it must be set using a Dirichlet condition on the k field. In practice it should be nonzero and small, perhaps of order $O(1/Re)$.

In the high Reynolds number case, \eqref{kbc2} and \eqref{epsbc2} can be implemented by selecting the following boundary condition type: \option{\ldots/boundary\_conditions/k\_epsilon/wall\_functions/high\_Re}. The velocity condition, \eqref{ubc2}, is not implemented in the model and may not be necessary. If using a sufficiently fine near-wall mesh, the correct velocity profile should be generated anyway.
...
2 non-linear iterations are recommended and can be specified by setting \option{\ldots/timestepping/nonlinear\_iterations} to 2, \ref{}
...
\item \option{\ldots/scheme/poisson\_pressure\_solution} can be chosen as either \option{never} or \option{only\_first\_timestep}, [REF SECTION]
\end{itemize}
...
\item psuedo-consistant interpolation - not recommended at present.
\item Galerkin interoplation - Conservative and non-dissipative, but requires the construction of a supermesh
