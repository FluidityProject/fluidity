...
mesh. In the simplest case, this can be used to specify that one mesh is the
same as another mesh. For example, since \option{CoordinateMesh} is linear
and continuous, it is possible to specify that \option{VelocityMesh} will
also be continuous and linear by selecting \option{from\_mesh} under
\option{/geometry/mesh::VelocityMesh} and selecting
\option{mesh::CoordinateMesh}\ under that.
...
Periodic meshes are those that are ``virtually'' connected in one or more directions. To make a periodic mesh you must first
create a triangle file where the edges that are periodic can be mapped exactly by a simple transformation. For example,
if the mesh is periodic in the X-direction, the two edges must have nodes at exactly the same height on each side. This
can be easily accomplished using the create\_aligned\_mesh script in the scripts folder.

In Diamond, a periodic mesh can now be formed. Add a new mesh under \option{/geometry/mesh}. 
Select \option{\ldots/from\_mesh/mesh::CoordinateMesh} and then turn on\onlypdf\linebreak
\option{\ldots/from\_mesh/periodic\_boundary\_conditions} for each dimension that is periodic. There are three fields that need to be completed: the surface IDs of one side, the surface IDs of the opposite side and a python function (\option{coordinate\_map}) which contains the necessary mapping function

Meshes that are required to be periodic can now be derived from this periodic mesh.

Running a periodic mesh in parallel is relatively straight forward. To begin with fludity must be compiled with Zoltan. To check if the Zoltan libaries are installed type:
...
\begin{lstlisting}[language=bash]
sudo apt-get install libzoltan_dev
\end{lstlisting}
...
The output is a new flml called  foo\_periodised.flml. Then run flredecomp (section \ref{sect:flredecomp}) to decompose the mesh for the number of processors required. For example, to run a periodic simulation on four processors simply type:

\begin{lstlisting}[language=bash]
mpiexec -np 4 ./myfluidity/bin/flredecomp \
...
\begin{lstlisting}[language=bash]
mpiexec -np [target number of processors] \
   ./myfluidity/bin/flredecomp -i [input number of processors] \
        -o [target number of processors] [input flml] [output flml]
\end{lstlisting}

The output of running flredecomp is a series of mesh and vtu files as well as the new flml; in this case foo\_periodised\_flredecomp.flml. It is this flml that you use to run a periodic parallel mesh. 

\subsubsection{Extruded meshes}
\index{mesh!extruded}

\subsubsection{Required meshes}
\index{mesh!required}
...
VelocityMesh is the right choice but it can be necessary to change this if,
for example, the domain is periodic.
...
The following sections describe the options below the
...
\item[Bassi Rebay]
\item[Compact DG]
\item[Arbitrary upwind]
\item[Interior penalty]
\end{description}
...
Setting the equation of state is optional.  If unset the material is assumed to be incompressible (see section~\ref{sect:equation_of_state}).  There are two ``fluids'' equations of state available and one ``multimaterial'' EOS:

\begin{description}
\item\option{\ldots/equation\_of\_state/fluids/linear} Is a simple linear equation of state for quasi-incompressible materials, where density is a function of temperature and salinity (or any other compositional volume fraction).
...
This EOS (\option{\ldots/equation\_of\_state/linear}) can be used when simulating incompressible flow to account for the effect on buoyancy of small changes in density caused by variations in temperature and/or salinity (for more details see section~\ref{Sect:IncompressibleFlow}).

\begin{description}
\item\option{\ldots/linear/reference\_density} Specifies the reference density of the fluid in SI units. If only this option is set the fluid is incompressible.
\item\option{\ldots/linear/temperature\_dependency} Activate this option to make the fluid density depend on temperature.
\item\option{\ldots/linear/temperature\_dependency/reference\_temperature} Defines the reference (or background) temperature of the fluid $T_0$; \ie in the absence of any other influence, the temperature at which $\rho=\rho_0$.
\item\option{\ldots/linear/temperature\_dependency/thermal\_expansion\_coefficient} Specifies the volumetric thermal expansion coefficient (Eq.~\ref{thermalexpansioncoeff}) $\alpha$, controlling the change in density for a given change in temperature.
\item\option{\ldots/linear/salinity\_dependency} Activate this option to make the fluid density depend on salinity.
\item\option{\ldots/linear/salinity\_dependency/reference\_salinity} Defines the reference (or background) salinity of the fluid $S_0$; \ie in the absence of any other influence, the salinity at which $\rho=\rho_0$.
\item\option{\ldots/linear/salinity\_dependency/saline\_contraction\_coefficient} Specifies the saline contraction coefficient $\beta$, controlling the change in density for a given change in salinity.
\end{description}
