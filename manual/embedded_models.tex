...
is then used to calculate the hindered sinking velocity, $v_{si}$ based upon the unhindered 
sinking velocity $v_{s0}$ and the total concentration of sediment, $c$.

\begin{equation}\label{eq:hindered_sinking_velocity}
v_{si} = v_{s0}(1-c)^{2.39}
\end{equation}
...
field: the SedimentDeposition field. Each sediment class has an equivalent Flux field.
...
diameter (mm). The SedimentDepositon field effectively mixes the deposited sediment, so order of deposition
is not preserved.
