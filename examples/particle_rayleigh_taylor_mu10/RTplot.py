...
scaling_factor = numpy.sqrt(1.0 / 0.9142)

# First plot 24x24 case:
pylab.plot(
    stat(statfile24)["ElapsedTime"]["value"][:],
    stat(statfile24)["Buoyant"]["Velocity%magnitude"]["l2norm"][:] * scaling_factor,
    "r",
    linestyle="-",
    lw=4.0,
    label="Particle-24",
...
pylab.plot(
    stat(statfile48)["ElapsedTime"]["value"][:],
    stat(statfile48)["Buoyant"]["Velocity%magnitude"]["l2norm"][:] * scaling_factor,
    "b",
    linestyle="-",
    lw=4.0,
    label="Particle-48",
...
pylab.plot(
    stat(statfile24)["ElapsedTime"]["value"][:],
    stat(statfile24)["Buoyant"]["Entrainment"]["integral"][:] / scaling_factor,
    "r",
    linestyle="-",
    lw=4.0,
    label="Particle-24",
...
pylab.plot(
    stat(statfile48)["ElapsedTime"]["value"][:],
    stat(statfile48)["Buoyant"]["Entrainment"]["integral"][:] / scaling_factor,
    "b",
    linestyle="-",
    lw=4.0,
    label="Particle-48",
