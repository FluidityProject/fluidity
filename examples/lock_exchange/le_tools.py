...
def GetFiles(directory):
    ...
    vtus = glob.glob(directory + "*.pvtu")
    if len(vtus) == 0:
        vtus = glob.glob(directory + "*.vtu")
    vtus = [item for item in vtus if all(x not in item for x in ['check',
                                                                 'Mesh'])]

    return sorted(vtus, key=key)
...
#
    for i in range(1, len(X) - 1):
        U.append(LeastSquares(t[i - 1:i + 2],
                 X[i - 1:i + 2], [0.0, 1.0])[0][0][1])
#
...
def LeastSquares(x_values, y_values, p0):
    ...
    plsq = scipy.optimize.leastsq(residuals, numpy.array(
        p0), args=(numpy.array(x_values), numpy.array(y_values)))

    lsq = []
    for val in x_values:
        lsq.append(predictedform(plsq[0], val))
...
def GetAverageRange(X, lower_lim, domainheight):
    ...
    try:
        start_val = numpy.where(numpy.array(X) > lower_lim)[0][0]
        end_val = numpy.where(numpy.array(X) > 0.4 - domainheight)[0][0]
        average = True
    except IndexError:
        start_val = 0
        end_val = 0
...
def GetstatFiles(directory):
    ...
    for sf in stat_files:
        if 'original' in sf:
            stat_files.remove(sf)
    for sf in stat_files:
        stat = stat_parser(sf)
        time_end.append(stat['ElapsedTime']['value'][-1])
    time_end, stat_files = zip(*sorted(zip(time_end, stat_files)))
    for i in range(len(stat_files) - 1):
        ...
        try:
            time_index_end.append(numpy.nonzero(
...
def Getconstantsfromflml(flmlname):
    ...
    rho_zero = float(Getflmlvalue(flmlname, f"""/fluidity_options/\
material_phase[@name={material_phase_name}]/equation_of_state/fluids/linear/\
reference_density"""))
    T_zero = float(Getflmlvalue(flmlname, f"""/fluidity_options/material_phase\
[@name={material_phase_name}]/equation_of_state/fluids/linear/\
temperature_dependency/reference_temperature"""))
    alpha = float(Getflmlvalue(flmlname, f"""/fluidity_options/material_phase\
[@name={material_phase_name}]/equation_of_state/fluids/linear/\
temperature_dependency/thermal_expansion_coefficient"""))
    g = float(Getflmlvalue(
        flmlname, '/fluidity_options/physical_parameters/gravity/magnitude'))
...
]/scalar_field[@name="Temperature"]/prognostic/stat/include_mixing_stats\
