            ...
            for ii in other.indices:
                if isinstance(ii, index):
                    if ii.id in product.sum_indices:
                        product.sum_indices[ii.id] = product.sum_indices[ii.id] + 1
                    else:
        ...
        for integral in other:
            if integral.trial:
                raise AttributeError(
                    "Cannot add a form with a trial function " + "to a field."
                )
    ...
    def __iadd__(self, other):
        ...
        for integral in other:
            if not (integral.trial):
                raise AttributeError(
                    "Cannot add a form with no trial function " + "to a matrix."
                )
