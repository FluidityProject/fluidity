...
def ReadMsh(filename):
    ...
    for i in range(elementcount):
        ...
        if element[1] == "1":
            ...
            sys.stderr.write(
                "Warning - Unknown element of type "
                + element[1]
                + ' encountered in gmsh msh file "'
                + filename
                + '"\n'
            )
            pass
    ...
    if (
        not len(elements["linear_triangles"]) == 0
        or not len(elements["linear_tets"]) == 0
    ):
        if (
            not len(elements["linear_quads"]) == 0
            or not len(elements["linear_hexes"]) == 0
        ):
            sys.stderr.write(
                'Warning - gmsh msh file "'
                + filename
...
def GenerateClPos(clfunc, nodes, elements={}):
    ...
    for key in elements.keys():
        ...
        for element in elements[key]:
            newElements[key].append(
                [element[0], [clfunc(coord) for coord in ElementCoords(nodes, element)]]
            )
    ...
    for key in elements.keys():
        if key == "linear_edges":
            ...
            sys.stderr.write(
                'Warning: Unknown element of type "'
                + str(key)
                + '" encountered in element dictionary\n'
            )
            pass
