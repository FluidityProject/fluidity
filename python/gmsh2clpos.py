...
def ReadMsh(filename):
  """
  Reads a gmsh msh file. Returns a list of node coordinates, and a dictionary of
  elements each with data element equal to the physical group ID.
  ...
  for i in range(elementcount):
    ...
    if element[1] == "1":
      ...
      sys.stderr.write("Warning - Unknown element of type " + element[1] + " encountered in gmsh msh file \"" + filename + "\"\n")
      pass
  assert(mshfile.readline().strip()=="$EndElements")

  if not len(elements["linear_triangles"]) == 0 or not len(elements["linear_tets"]) == 0:
    if not len(elements["linear_quads"]) == 0 or not len(elements["linear_hexes"]) == 0:
      sys.stderr.write("Warning - gmsh msh file \"" + filename + "\" contains a mix of triangles/tets and quads/hexes\n")
...
def GenerateClPos(clfunc, nodes, elements = {}):
  """
  Replaces the data in all supplied elements with characteristic lengths as
  defined by the supplied characteristic length function clfunc. clfunc must be of
  the form:
  ...
  for key in elements.keys():
    ...
    for element in elements[key]:
      newElements[key].append([element[0], [clfunc(coord) for coord in ElementCoords(nodes, element)]])

  return newElements

def WriteClPos(filename, clfunc, nodes, elements):
  """
  Generates a gmsh post processing file suitable for use as a background mesh,
  with characteristic lengths as defined by the supplied characteristic length
  function clfunc. See GenerateClPos.
  """
  ...
  for key in elements.keys():
    if key == "linear_edges":
      for linear_edge in elements["linear_edges"]:
        posfile.write("  SL" + ElementCoordsAndData(nodes, linear_edge) + ";\n")
    elif key == "linear_triangles":
      for linear_triangle in elements["linear_triangles"]:
        posfile.write("  ST" + ElementCoordsAndData(nodes, linear_triangle) + ";\n")
    elif key == "linear_quads":
      for linear_quad in elements["linear_quads"]:
        posfile.write("  SQ" + ElementCoordsAndData(nodes, linear_quad) + ";\n")
    elif key == "linear_tets":
      for linear_tet in elements["linear_tets"]:
        posfile.write("  SS" + ElementCoordsAndData(nodes, linear_tet) + ";\n")
    elif key == "linear_hexes":
      for linear_hex in elements["linear_hexes"]:
        posfile.write("  SH" + ElementCoordsAndData(nodes, linear_hex) + ";\n")
    else:
      sys.stderr.write("Warning: Unknown element of type \"" + str(key) + "\" encountered in element dictionary\n")
      pass
...
def Gmsh2ClPos(input_filename, clfunc, output_filename):
  """
  Reads the gmsh mesh file specified by input_filename and generates a gmsh post
  processing file suitable for use as a background mesh.
  """
