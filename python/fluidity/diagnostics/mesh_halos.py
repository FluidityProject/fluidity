...
class Halo:
  ...
  def SetSends(self, sends, process = None):
    if process is None:
      ...
      self._sends = [[None for i in range(len(sends[i]))] for i in range(nProcesses)]
  ...
  def SetReceives(self, receives, process = None):
    if process is None:
      ...
      self._receives = [[None for i in range(len(receives[i]))] for i in range(nProcesses)]
...
class Halos:
  ...
  def HaloCompatible(self, halo):    
    return halo.GetProcess() == self.GetProcess() and halo.GetNProcesses() == self.GetNProcesses()
...
class mesh_halosUnittests(unittest.TestCase):
  ...
  def testHaloTrailingReceivesOrdered(self):
    self.assertTrue(Halo(process = 0, nProcesses = 1, nOwnedNodes = 2, sends = [[0, 1]], receives = [[2, 3]]).TrailingReceivesOrdered())
    self.assertTrue(Halo(process = 0, nProcesses = 1, nOwnedNodes = 2, sends = [[1, 0]], receives = [[3, 2]]).TrailingReceivesOrdered())
    self.assertFalse(Halo(process = 0, nProcesses = 1, sends = [[0, 1]], receives = [[2, 3]]).TrailingReceivesOrdered())
    self.assertFalse(Halo(process = 0, nProcesses = 1, nOwnedNodes = 2, sends = [[1, 2]], receives = [[2, 3]]).TrailingReceivesOrdered())
    self.assertFalse(Halo(process = 0, nProcesses = 1, nOwnedNodes = 2, sends = [[0, 1]], receives = [[1, 2]]).TrailingReceivesOrdered())
    self.assertFalse(Halo(process = 0, nProcesses = 1, nOwnedNodes = 2, sends = [[0, 1]], receives = [[3, 4]]).TrailingReceivesOrdered())
    self.assertFalse(Halo(process = 0, nProcesses = 1, nOwnedNodes = 2, sends = [[0, 1]], receives = [[2, 2]]).TrailingReceivesOrdered())
...
class mesh_halosDataUnittests(unittest.TestCase):
  def testReadHalos(self):
    filename = os.path.join(os.path.dirname(__file__), os.path.pardir, "test-data", "CoarseCorner_0.halo")
    halos = ReadHalos(filename)
