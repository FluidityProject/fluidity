...
def ModelPvtuToVtu(pvtu):
    ...
    locations = numpy.array([locations[i] for i in node_ids])
...
def AddtoVtuField(vtu, add, fieldName, scale=None):
    ...
    if optimise.DebuggingEnabled():
        assert vtktools.VtuMatchLocations(vtu, add)
...
def AddtoVtu(vtu, add, scale=None):
    ...
    if vtktools.VtuMatchLocations(vtu, add):
        for fieldName in vtu.GetFieldNames():
...
def TimeAveragedVtu(filenames, timeFieldName="Time", baseMesh=None, baseVtu=None):
    ...
    for i, filename in enumerate(filenames):
        ...
        if nextInputVtu is None:
            inputVtu = vtktools.vtu(filename)
        else:
        ...
        if len(filenames) == 1:
            ...
            if i < len(filenames) - 1:
                nextInputVtu = vtktools.vtu(filenames[i + 1])
        ...
        if i == 0:
            if not vtktools.VtuMatchLocations(inputVtu, result):
