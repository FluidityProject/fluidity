    ...
    def __init__(self, dim=None, nodeCount=None, gmshElementTypeId=None):
        if gmshElementTypeId is None:
            ...
            elements.ElementType.__init__(
                self,
                elementTypeId=self._gmshElementTypeIdToElementTypeId[gmshElementTypeId],
            )
...
def WriteMsh(mesh, filename, binary=True):
    ...
    if binary:
        ...
        fileHandle.write(b"$EndMeshFormat\n")

        # Write the Nodes section

        fileHandle.write(b"$Nodes\n")
        ...
        fileHandle.write(b"$EndNodes\n")

        # Write the Elements section

        fileHandle.write(b"$Elements\n")
        fileHandle.write(
            utils.FormLine(
        ...
        fileHandle.write(b"$EndElements\n")
    else:
        # ASCII format
        ...
        for i, ele in enumerate(mesh.GetSurfaceElements() + mesh.GetVolumeElements()):
            ...
            fileHandle.write(
                utils.FormLine(
                    [
                        i + 1,
                        gmshType.GetGmshElementTypeId(),
                        len(ids),
                        ids,
                        utils.OffsetList(ToGmshNodeOrder(ele.GetNodes(), eleType), 1),
                    ]
                )
            )
...
class gmshtoolsUnittests(unittest.TestCase):
    def testGmshElementType(self):
        ...
        self.assertEqual(type.GetGmshElementTypeId(), GMSH_QUAD)
        type.SetDim(3)
        self.assertEqual(type.GetGmshElementTypeId(), GMSH_TETRAHEDRON)
        type.SetNodeCount(8)
        self.assertEqual(type.GetGmshElementTypeId(), GMSH_HEXAHEDRON)
