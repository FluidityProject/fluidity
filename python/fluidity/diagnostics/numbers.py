...
def ThermalBoundaryLayerThickness(g, alpha, deltaT, nu, kappa, H, D = None):
  ...
  return math.pow(RayleighNumber(g = g, alpha = alpha, deltaT = deltaT, nu = nu, kappa = kappa, H = H), -0.25) * D
...
class numbersUnittests(unittest.TestCase):
  def testThermalBoundaryLayerThickness(self):
    self.assertAlmostEquals(ThermalBoundaryLayerThickness(g = 1.0 / 16.0, alpha = 1.0, deltaT = 1.0, nu = 1.0, kappa = 1.0, H = 1.0), 2.0)
    H = math.pow(1.0 / 16.0, 1.0 / 3.0)
    self.assertAlmostEquals(ThermalBoundaryLayerThickness(g = 1.0, alpha = 1.0, deltaT = 1.0, nu = 1.0, kappa = 1.0, H = H), 2.0 * H)
    self.assertAlmostEquals(ThermalBoundaryLayerThickness(g = 1.0, alpha = 1.0, deltaT = 1.0, nu = 1.0, kappa = 1.0, H = H, D = 0.5 * H), H)
    
    return
    
  def testBoundaryEkmanLayerThickness(self):
    self.assertAlmostEquals(EkmanBoundaryLayerThickness(omega = 4.0, nu = 1.0, H = 1.0), 0.5)
    self.assertAlmostEquals(EkmanBoundaryLayerThickness(omega = 4.0, nu = 1.0, H = 1.0, D = 2.0), 1.0)
