...
def pczdna(state, parameters):
    ...
    Dnew=state.scalar_fields["IteratedDetritus"]
    coords=state.vector_fields["Coordinates"]
    ...
    for n in range(P.node_count):
        ...
        theta = C_n/N_n
        alpha = alpha_c * theta * zeta

        # Light limited phytoplankton growth rate.
        J=(v*alpha*I_n)/(v**2+alpha**2*I_n**2)**0.5

        # Nitrate limiting factor.
        Q_N=(N_n*(-psi * A_n))/(k_N+N_n)
        ...
        # Zooplankton grazing of phytoplankton.
        G_P=(g * epsilon * p_P * P_n**2 * Z_n)/(k**2 + p_P*P_n**2 + p_D*D_n**2)

        # Zooplankton grazing of detritus.
        G_D=(g * epsilon * (1-p_P) * D_n**2 * Z_n)/(k**2 + p_P*P_n**2 + p_D*D_n**2)

        # Death rate of phytoplankton.
        De_P=mu_P*P_n*P_n/(P_n+k_p)

        # Death rate of zooplankton.
        De_Z=mu_Z*Z_n**3/(Z_n_k_z)
        ...
        if (abs(coords.node_val(n)[2]) < photicZoneDepth):
            ...
            C_source.addto(n, (R_P*J*P_n*(Q_N+Q_A) + (-G_P-De_P))*zeta/theta)
            Z_source.addto(n, gamma*(beta_P*G_P+beta_D*G_D) - De_Z)
        ...
        # below
            ...
            D_source.addto(n, -lambda_bio*(P_n + Z_n) - mu_D*D_n**2*lambda_D)
            A_source.addto(n, -lambda_A*A_n)
            N_source.addto(n, -lambda_A*A_n - lambda_d*D_n)
...
def check_pczdna_parameters(parameters):
    ...
    if not parameters.has_key("p_D"):
        stderr.write("PCZNDA parameter p_D missing.\n")
        stderr.write("p_D is the relative grazing preference of zooplankton for detritus.\n\n")
        valid = False
