...
subroutine test_vtk_precision
  ...
  call report_test("[Clean]", abs(read_v_field%val(1)%ptr(1)) > 0.0, .false., "[Failed to clean output]")
  ...
  do i = 1, mesh_dim(mesh_field)
    call report_test("[Clean]", any(abs(read_v_field%val(1)%ptr) > 0.0), .false., "[Failed to clean output]")
  end do
  ...
  call vtk_read_state(filename, state = read_state)
  read_v_field => extract_vector_field(read_state, "Coordinate")
  call report_test("[Coordinate field, tiny, real_4 precision]", read_v_field%val(1)%ptr(1) < tiny(0.0_S), .false., "[Insufficient precision]")
  read_s_field => extract_scalar_field(read_state, "TestScalarField")
  read_v_field => extract_vector_field(read_state, "TestVectorField")
  call report_test("[Scalar field, tiny, real_4 precision]", any(read_s_field%val < tiny(0.0_S)), .false., "[Insufficient precision]")
  call report_test("[Vector field, tiny, real_4 precision]", any(read_v_field%val(1)%ptr < tiny(0.0_S)), .false., "[Insufficient precision]")
  call deallocate(read_state)
  nullify(read_s_field)
  nullify(read_v_field)
  ...
  call vtk_read_state(filename, state = read_state)
  read_v_field => extract_vector_field(read_state, "Coordinate")
  call report_test("[Coordinate field, tiny, real_8 precision]", read_v_field%val(1)%ptr(1) < tiny(0.0_D), .false., "[Insufficient precision]")
  read_s_field => extract_scalar_field(read_state, "TestScalarField")
  read_v_field => extract_vector_field(read_state, "TestVectorField")
  call report_test("[Scalar field, tiny, real_8 precision]", any(read_s_field%val < tiny(0.0_D)), .false., "[Insufficient precision]")
  call report_test("[Vector field, tiny, real_8 precision]", any(read_v_field%val(1)%ptr < tiny(0.0_D)), .false., "[Insufficient precision]")
  call deallocate(read_state)
  nullify(read_s_field)
  nullify(read_v_field)
#endif
