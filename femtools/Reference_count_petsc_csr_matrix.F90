  subroutine addref_petsc_csr_matrix(object)
    ...
    if (associated(object%refcount)) then
       ...
       id = id + 1
       object%refcount=>new_refcount("petsc_csr_matrix", object%name)
       object%refcount%id = id
    end if
    
  end subroutine addref_petsc_csr_matrix
  
  subroutine incref_petsc_csr_matrix(object)
    !!< Increment the reference count of object. If there are no references
    !!< then error.
    use parallel_tools, only: abort_if_in_parallel_region
    type(petsc_csr_matrix), intent(in), target :: object
    ...
    if (.not.associated(object%refcount)) then
       FLAbort ("Attempt to incref petsc_csr_matrix "//trim(object%name)//" which has no references")
    end if
  ...
  end subroutine incref_petsc_csr_matrix  
  
  subroutine decref_petsc_csr_matrix(object)
    ...
    use parallel_tools, only: abort_if_in_parallel_region
    type(petsc_csr_matrix), intent(inout) :: object
    ...
    if (object%refcount%count<=0) then
       ...
       if (object%refcount%count<0) then
          ...
          ewrite(0,'(a, i0)') "Reference count of &
               &petsc_csr_matrix "//trim(object%name)//&
  ...
  end subroutine decref_petsc_csr_matrix

  pure function has_references_petsc_csr_matrix(object) result (has_references)
    !!< Return true if there are any references to object
    type(petsc_csr_matrix), intent(in) :: object
    logical :: has_references
    
    has_references=associated(object%refcount)

  end function has_references_petsc_csr_matrix
