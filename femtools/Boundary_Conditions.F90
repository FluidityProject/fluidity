...
contains
  ...
  !! boundary condition warnings surpressed when dealing with sub_state
  logical, intent(in), optional:: is_sub_state
    ...
    if (.not. IsParallel() .and. .not. all(boundary_id_used)  .and. .not.(is_sub_state)) then
  ...
  subroutine add_vector_boundary_condition(field, name, type, boundary_ids, &
      applies, option_path, is_sub_state)
