...
implicit none
  ...
  function dot_product_vector(fieldA, fieldB) result(val)
    ...
    if ((fieldA%field_type == FIELD_TYPE_NORMAL .or. fieldA%field_type == FIELD_TYPE_SPHERICAL_COORDINATES) .and. &
        (fieldB%field_type == FIELD_TYPE_NORMAL .or. fieldB%field_type == FIELD_TYPE_SPHERICAL_COORDINATES)) then
       do d=1,fieldA%dim
