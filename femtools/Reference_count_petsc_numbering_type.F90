  subroutine addref_petsc_numbering_type(object)
    ...
    if (associated(object%refcount)) then
       ...
       id = id + 1
       object%refcount=>new_refcount("petsc_numbering_type", object%name)
       object%refcount%id = id
    end if
    
  end subroutine addref_petsc_numbering_type
  
  subroutine incref_petsc_numbering_type(object)
    !!< Increment the reference count of object. If there are no references
    !!< then error.
    use parallel_tools, only: abort_if_in_parallel_region
    type(petsc_numbering_type), intent(in), target :: object
    ...
    if (.not.associated(object%refcount)) then
       FLAbort ("Attempt to incref petsc_numbering_type "//trim(object%name)//" which has no references")
    end if
  ...
  end subroutine incref_petsc_numbering_type  
  
  subroutine decref_petsc_numbering_type(object)
    ...
    use parallel_tools, only: abort_if_in_parallel_region
    type(petsc_numbering_type), intent(inout) :: object
    ...
    if (object%refcount%count<=0) then
       ...
       if (object%refcount%count<0) then
          ...
          ewrite(0,'(a, i0)') "Reference count of &
               &petsc_numbering_type "//trim(object%name)//&
  ...
  end subroutine decref_petsc_numbering_type

  pure function has_references_petsc_numbering_type(object) result (has_references)
    !!< Return true if there are any references to object
    type(petsc_numbering_type), intent(in) :: object
    logical :: has_references
    
    has_references=associated(object%refcount)

  end function has_references_petsc_numbering_type
