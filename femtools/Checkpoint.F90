...
contains
  ...
  subroutine checkpoint_simulation(state, prefix, postfix, cp_no, protect_simulation_name, &
    keep_initial_data, ignore_detectors, number_of_partitions, solid)
    ...
    call checkpoint_state(state, lprefix, postfix = lpostfix, cp_no = cp_no, &
      keep_initial_data = keep_initial_data, number_of_partitions=number_of_partitions, solid=solid)
...
!!!!! Writing of position detectors before checkpointing in serial  !!!!!
  ...
  subroutine checkpoint_state(state, prefix, postfix, cp_no, keep_initial_data, number_of_partitions, solid)
    !!< Checkpoint state.
    ...
    call checkpoint_meshes(state, prefix, postfix, cp_no, keep_initial_data=keep_initial_data, number_of_partitions=number_of_partitions, solid=solid)
    call checkpoint_fields(state, prefix, postfix, cp_no, keep_initial_data=keep_initial_data, number_of_partitions=number_of_partitions, solid=solid)

  end subroutine checkpoint_state

  subroutine checkpoint_meshes(state, prefix, postfix, cp_no, keep_initial_data, number_of_partitions, solid)
  ...
  subroutine checkpoint_fields(state, prefix, postfix, cp_no, keep_initial_data, number_of_partitions, solid)
      ...
      do j = 1, size(state(i)%meshes)
        ...
        if(n_ps_fields_on_mesh + n_pv_fields_on_mesh + n_pt_fields_on_mesh > 0) then
          call vtk_write_fields(vtu_filename, position = positions, model = mesh, &
            ...
            & tfields = pt_fields_on_mesh(:n_pt_fields_on_mesh), number_of_partitions=number_of_partitions, solid=solid, stat=stat)
        end if
