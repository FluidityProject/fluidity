...
contains
  ...
  function dshape_dot_vector_tensor_shape(dshape, vector, tensor, shape, detwei) result (R)
    ...
    real, dimension(size(tensor,1),size(tensor,2),size(dshape,1),shape%ndof) ::&
         & R
    ...
    forall(iloc=1:dshape_loc,jloc=1:shape%ndof,i=1:size(tensor,1),j=1:size(tensor,2))
       ! Main mass matrix.
       R(i,j,iloc,jloc)=&
  ...
  function shape_vector_dot_dshape_tensor(shape, vector, dshape, tensor, detwei) result (R)
    ...
    real, dimension(size(tensor,1), size(tensor,2), shape%ndof, size(dshape,1)) :: R
    ...
    forall(iloc=1:shape%ndof,jloc=1:dshape_loc,i=1:size(tensor,1),j=1:size(tensor,2))
