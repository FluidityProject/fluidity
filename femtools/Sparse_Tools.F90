...
SUBROUTINE posinm_legacy(TOTELE,NNodes1,NLoc1, &
     COLM, LenCOLM, NIMEM, &
     FINDRM, CENTRM, &
...
!
!
SUBROUTINE POSINM_COLOUR(TOTELE,NNodes1,NLoc1, &
     COLM, LenCOLM, NIMEM, &
     FINDRM, CENTRM, &
     NNodes2, NLoc2, NDGLNO2, &
     NDGLNO1)
  ...
  RETURN
END SUBROUTINE POSINM_COLOUR

  !! This is the Discontinuous Galerkin version of this routine. It takes
  !! into account the links between adjacent boundaries
  !
!!$  SUBROUTINE POSINM_DG(sparsity, TOTELE, NNodes1, NLoc1, NDGLNO1,&
!!$       NNodes2, NLoc2, NDGLNO2, boundary_list, FLoc, boundary_glno, diag)
!!$    type(csr_sparsity), intent(out) :: sparsity
!!$    INTEGER, INTENT(IN)::NNodes1, NNodes2, TOTELE, NLoc1, NLoc2, FLoc
!!$    INTEGER, target, INTENT(IN)::NDGLNO1(TOTELE*NLoc1), NDGLNO2(TOTELE*NLoc2)
!!$    type(csr_matrix), intent(in) :: boundary_list
!!$    ! Following line is a hack to get around an obscure ifort bug with the
!!$    ! entries function.
!!$    INTEGER, INTENT(IN), target::boundary_glno(count(boundary_list%sparsity%colm/=0)*FLoc)
!!$    logical, intent(in), optional :: diag
!!$
!!$    INTEGER ELE,GLOBI,GLOBJ,LOCI,LOCJ,I,IROW,J,IHOLD,IFINDR, PTR
!!$    integer :: ele_j, ele_j2, boundary_idx, boundary_j_idx, lstat
!!$
!!$    ! Count of nonzero entries.
!!$    integer :: entries
!!$    ! Count of diagonal entries.
...
!!$    integer, dimension(:), pointer :: neigh, neigh2
!!$
!!$    ! Pointers to current boundaries.
!!$    integer, dimension(:), pointer :: boundary, boundary_j
!!$
!!$    ! Pointers to elements.
!!$    integer, dimension(:), pointer :: glob_ele, glob_ele_j, glob_ele_j2
!!$
!!$    ! Whether val and diag should be allocated. If diag is false then the
!!$    ! diagonal will be totally excluded from the matrix.
!!$    logical :: ldiag
...
!!$       i_loop: DO LOCI=1,NLoc2
!!$          GLOBI=NDGLNO2((ELE-1)*NLoc2+LOCI)
!!$
!!$          j_loop: DO LOCJ=1,NLoc1
!!$             GLOBJ=NDGLNO1((ELE-1)*NLoc1+LOCJ)
!!$
!!$             ! Count diagonal entries.
!!$             if (GLOBI==GLOBJ) diag_cnt=diag_cnt+1
!!$             
!!$             call row_insert(lMatrix(GLOBI), globj, entries)
!!$
!!$          END DO j_loop
!!$       END DO i_loop
!!$
!!$       ! Insert entries for adjacent elements. Note that between
!!$       ! elements a and b the boundary adjacent to element a is boundary(b,a).
!!$                 
!!$       neigh=>row_m_ptr(boundary_list,ELE)
!!$
!!$       neigh_loop: do ele_j=1,size(neigh)
!!$          ! Skip external boundaries.
!!$          if (neigh(ele_j)==0) cycle neigh_loop
!!$             
!!$          ! Find boundary numbers for the inside of the boundary.
!!$          boundary_idx=ival(boundary_list,ele,neigh(ele_j))
!!$
!!$          ! Find the local node numbers associated with the boundary.
!!$          boundary=>boundary_glno((boundary_idx-1)*floc+1:boundary_idx*floc)
!!$
!!$          ! Find the global numbers associated with the elements.
!!$          glob_ele=>ndglno2((ele-1)*nloc2+1:ele*nloc2)
!!$          glob_ele_j=>ndglno1((neigh(ele_j)-1)*nloc1+1:neigh(ele_j)*nloc1)
!!$
!!$          ! List of neighbours of this neighbour.
!!$          neigh2=>row_m_ptr(boundary_list,neigh(ele_j))          
!!$
!!$          boundary_i_loop:do LOCI=1,NLoc2
!!$             globi=glob_ele(loci)
!!$
!!$             neigh_j_loop: do locj=1,NLoc1
!!$                globj=glob_ele_j(locj)
!!$
!!$                call row_insert(lMatrix(GLOBI), globj, entries)
!!$
!!$             end do neigh_j_loop
!!$
!!$             ! Now insert entries from the boundaries adjacent to the neighbour
!!$             ! elements.
!!$
!!$             ! Only insert entries if this node lies on the relevant boundary.
!!$             if (.not.any(loci==boundary)) cycle boundary_i_loop
!!$
!!$             neigh2_loop:do ele_j2=1,size(neigh2)
!!$                ! Skip external boundaries.
!!$                if (neigh2(ele_j2)==0) cycle neigh2_loop
!!$                
!!$                ! Boundary number for outside boundary.
!!$                boundary_j_idx=ival(boundary_list,neigh2(ele_j2), neigh(ele_j))
!!$                ! Local node numbers on that boundary.
!!$                boundary_j=>boundary_glno((boundary_j_idx-1)*floc+1:boundary_j_idx*floc)
!!$                ! Find the global numbers associated with the elements.
!!$                glob_ele_j2=>ndglno1((neigh2(ele_j2)-1)*nloc1+1&
!!$                     &               :neigh2(ele_j2)*nloc1)
!!$                
!!$                boundary_j_loop: do locj=1,NLoc1
!!$                   ! Only add entries if this node lies on the relevant
!!$                   ! boundary. 
!!$                   if (.not.any(locj==boundary_j)) cycle boundary_j_loop
!!$                   
!!$                   globj=glob_ele_j2(locj)
!!$                
!!$                   call row_insert(lMatrix(GLOBI), globj, entries)
!!$
!!$                end do boundary_j_loop
!!$                
!!$             end do neigh2_loop
!!$          end do boundary_i_loop
!!$       end do neigh_loop
!!$       
!!$    end DO ele_loop
!!$
...
!!$    DEALLOCATE( lMatrix )
!!$
!!$    ewrite(1, *) "END SUBROUTINE POSINM_dg"
!!$
!!$  END SUBROUTINE POSINM_DG
