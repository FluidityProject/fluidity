...
module stocastic
  ...
  public :: get_random_points_in_mesh, initialise_stocastic_module,&
...
contains
  ...
  subroutine initialise_stocastic_module()
    if (initialised) return
    call init_random_seed()
    initialised=.true.
  end subroutine initialise_stocastic_module
...
! and we choose not to waste them.
  ...
  function get_random_points_in_mesh(X,npoints,pdf) result(pts)
    ...
    if (element_count(X)==0) then
       FLAbort("Trying to allocate stocastic detectors on a domain of zero measure. Check any surface ids are actually present in the mesh")
    end if
