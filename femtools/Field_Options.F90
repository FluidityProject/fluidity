...
contains
  ...
  type(vector_field) positions
    ...
    if (have_option(trim(mesh%option_path)//"/exclude_from_mesh_adaptivity")) then
      ...
      if (mesh_dim(mesh)+1==mesh_dim(coordinate_field)) then
         if (surface_element_count(coordinate_field)==element_count(mesh)) then
            ! it's a surface mesh
            positions=extract_vector_field(state, "SurfaceCoordinate")
            call incref(positions)
         else
  ...
  integer function equation_type_index(option_path)
    ...
    case ( "KOmega" )
      equation_type_index = FIELD_EQUATION_KOMEGA
    case default
