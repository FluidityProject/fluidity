...
integer, private, parameter:: ISOLATED=0, COUPLED=-1
  ...
  real(kind=PetscRealKind) :: PetscZero=0.0_PetscRealKind, PetscOne=1.0_PetscRealKind
...
Mat, dimension(:), optional, intent(in):: external_prolongators
...
logical, optional, intent(in) :: has_null_space
  ...
  logical isparallel, initialized
    ...
    if (isparallel .or. present_and_true(has_null_space)) then
      ...
      call KSPSetTolerances(ksp_smoother, 1.0e-100_PetscRealKind, 1e-8_PetscRealKind, 1e10_PetscRealKind, 300, ierr)
    end if
...
integer, intent(in):: iterations
  ...
  call PCSORSetOmega(pc, PetscOne, ierr)
...
Vec, intent(out):: eigvec
  ...
  do i=1, MAX_ITERATIONS
    ...
    call VecScale(x_kp1, PetscOne/norm2, ierr)
  ...
  ! copied from Futils.F90 to avoid dependency
