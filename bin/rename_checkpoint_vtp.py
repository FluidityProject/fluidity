...
def main():
    ...
    if basename[-4:]==".vtu" or basename[-4:]==".vtp":
       base_filename = os.path.basename(basename[:-4])
    elif basename[-5:]==".pvtu" or basename[-5:]==".pvtp":
       base_filename = os.path.basename(basename[:-5])
    else:
       base_filename = os.path.basename(basename)


    filelist_vtu = glob.glob(base_filename+"_checkpoint_*[0-9].vtu")+glob.glob(base_filename+"_checkpoint_*[0-9]/"+base_filename+"_checkpoint_*[0-9].vtu")+glob.glob(base_filename+"_checkpoint_*[0-9].pvtu")
    filelist_vtp = glob.glob(base_filename+"_checkpoint_*[0-9].vtp")+glob.glob(base_filename+"_checkpoint_*[0-9]/"+base_filename+"_checkpoint_*[0-9].vtp")+glob.glob(base_filename+"_checkpoint_*[0-9].pvtp")
    filelist_boundary = glob.glob("boundary_*[0-9].vtu")

    rename_vtu(base_filename, filelist_vtu, index, verbose=verbose)
    rename_vtp(base_filename, filelist_vtp, index, verbose=verbose)
    rename_boundary(filelist_boundary, index, verbose=verbose)
...
def rename_vtp(base_filename, filelist, index, verbose=False):

    """Rename the VTP or PVTP files based on base_filename and "checkpoint" """
        
    for i in range(len(filelist)):
      if filelist[i][-4:]==".vtp":
        filesplit  = filelist[i].split(".vtp")[0].split(base_filename+"_checkpoint_")[-1].split("_")
        # serial vtps
        if(len(filesplit)==1):
          newindex = index + int(filesplit[0])
          newfilename = base_filename+"_"+str(newindex)+".vtp"
          if(os.path.exists(newfilename)):
            if(verbose): print "backing up", newfilename, "to", newfilename+".bak"
            shutil.move(newfilename, newfilename+".bak")
          if(verbose): print "moving", filelist[i], "to", newfilename
        ...
        # parallel vtps
          ...
          newfilename = checkpoint_directory+base_filename+"_"+str(newindex)+"_"+filesplit[1]+".vtp"
          if(os.path.exists(newfilename)):
            if(verbose): print "backing up", newfilename, "to", newfilename+".bak"
            shutil.move(newfilename, newfilename+".bak")
          if(verbose): print "moving", filelist[i], "to", newfilename
          shutil.move(filelist[i], newfilename)
      elif filelist[i][-5:]==".pvtp":
        filesplit  = filelist[i].split(".pvtp")[0].split(base_filename+"_checkpoint_")[-1].split("_")
        # parallel pvtps
        if(len(filesplit)==1):
          newindex = index + int(filesplit[0])
          newfilename = base_filename+"_"+str(newindex)+".pvtp"
          # if in format where all vtps live in a directory assocaited with a pvtp move that directory
          # also set directory name so when rewriting pvtp can add correct directory name
          checkpoint_directory = '.'
          if(os.path.exists(filelist[i].split('.pvtp')[0])):
            if(os.path.exists(base_filename+"_"+str(newindex))):
              if(verbose): print "backing up", base_filename+"_"+str(newindex)+"/*.vtp", "to", base_filename+"_"+str(newindex)+"/*.vtp.bak"
              for f in glob.glob(base_filename+"_"+str(newindex)+"/*.vtp"): shutil.move(f, f+".bak")
              if(verbose): print "moving", filelist[i].split('.pvtp')[0]+"/*.vtp", "to", base_filename+"_"+str(newindex)+"/*.vtp"
              for f in glob.glob(filelist[i].split('.pvtp')[0]+"/*.vtp"):  shutil.move(f, base_filename+"_"+str(newindex)+'/'+f.split('/')[1])
            else:
              os.mkdir(base_filename+"_"+str(newindex))
              if(verbose): print "moving", filelist[i].split('.pvtp')[0]+"/*.vtp", "to", base_filename+"_"+str(newindex)+"/*.vtp"
              for f in glob.glob(filelist[i].split('.pvtp')[0]+"/*.vtp"):  shutil.move(f, base_filename+"_"+str(newindex)+'/'+f.split('/')[1])
            try:
              if(verbose): print "removing directory", filelist[i].split('.pvtp')[0]
              os.rmdir(filelist[i].split('.pvtp')[0])
            except OSError:
              if(verbose): print filelist[i].split('.pvtp')[0], "not removed, directory may not be empty"
            checkpoint_directory = base_filename+"_"+str(newindex)
          if(os.path.exists(newfilename)):
            if(verbose): print "backing up", newfilename, "to", newfilename+".bak"
            shutil.move(newfilename, newfilename+".bak")
          if(verbose): print "moving", filelist[i], "to", newfilename
          shutil.move(filelist[i], newfilename)
          # must also adjust content of pvtp so it points at moved parallel vtps
          pvtpfile = file(newfilename,'r')
          lines = pvtpfile.readlines()
          pvtpfile.close()
          for line in range(len(lines)):
            lineindex=lines[line].find(base_filename+"_checkpoint_"+filesplit[0])
            if(lineindex!=-1):
              processorsplit = lines[line][lineindex:].split(".vtp")[0].split(base_filename+"_checkpoint_")[-1].split("_")
              if(len(processorsplit)==2):
                newline = lines[line][:lineindex]+checkpoint_directory+'/'+base_filename+"_"+str(newindex)+"_"+str(processorsplit[-1])+".vtp"+lines[line][lineindex:].split(".vtp")[-1]
                lines[line] = newline
          pvtpfile = file(newfilename,'w')
          pvtpfile.writelines(lines)
          pvtpfile.close()

def rename_boundary(filelist, index, verbose=False):

    """Rename the boundary vtu files based on the filelist and associated index with vtu and vtp files"""       
    for i in range(len(filelist)):
