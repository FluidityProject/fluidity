...
subroutine simple_xy_rd(FILE_NAME, NX, NY, data_in, x_data, y_data)
  use netcdf
  implicit none
  ...
  real, dimension(:,:), allocatable, intent(inout) :: data_in
  real, dimension(:), allocatable, intent(inout) :: x_data
  real, dimension(:), allocatable, intent(inout) :: y_data

  ! This will be the netCDF ID for the file and data variable.
  integer :: ncid, varid, x_varid, y_varid
  ...
  call check( nf90_inq_varid(ncid, "y", x_varid) )
  call check( nf90_inq_varid(ncid, "x", y_varid) )

  ! Read the data.
  call check( nf90_get_var(ncid, varid, data_in) )
  call check( nf90_get_var(ncid, x_varid, x_data) )
  call check( nf90_get_var(ncid, y_varid, y_data) )
